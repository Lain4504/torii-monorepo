    ...
    useEffect(() => {
        if (offering) {
            reset({
                ...
                originalPrice: Number(offering.originalPrice || offering.price),
                ...
                classIds: offering.classes?.map(c => c.id) || [],
            })
        } else {
            reset({
                code: "",
                        ...
                        <form id="offering-form" onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                            <FieldGroup>
                                ...
                                <FieldSet>
                                    ...
                                    <FieldDescription>Chọn các lớp học (LIVE/VOD) sẽ được kích hoạt khi mua gói này.</FieldDescription>
                                    <div className="grid grid-cols-1 gap-2 mt-2">
                                        {classes?.map((cls) => (
                                            <div
                                                ...
                                                className={cn(
                                                    ...
                                                    selectedClassIds.includes(cls.id) ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-muted"
                                                )}
                                                onClick={() => toggleClass(cls.id)}
                                            >
                                                <div className="flex flex-col">
                                                    <span className="text-sm font-medium">{cls.name}</span>
                                                    <span className="text-xs text-muted-foreground">{cls.code} • {cls.mode}</span>
                                                </div>
                                        ...
                                        {!classes?.length && <div className="text-sm text-muted-foreground italic">Không có lớp học nào khả dụng.</div>}
                                    </div>
                                </FieldSet>
                            </FieldGroup>
                        </form>
