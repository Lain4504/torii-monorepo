...
import { useAcademyClassCurriculum, useAcademyClass } from '@/lib/api/services/academy-classes'
...
export function ClassDiscussionTab({ classId }: ClassDiscussionTabProps) {
  ...
  const { data: curriculum } = useAcademyClassCurriculum(classId)
  ...
  const renderReplyTree = (reply: CommentResponseDTO, depth: number) => {
      ...
      <div key={reply.id} className={depth === 0 ? '' : 'mt-4'}>
        ...
        {canRecurse && (
          <div className="ml-4 border-l border-primary/10 pl-4 mt-3">
            {reply.replies?.map((r) => renderReplyTree(r as any, depth + 1))}
          </div>
        )}
      </div>
    )
                  ...
                  <CardContent className="p-4 pt-0">
                    ...
                    {(topic.replies?.length ?? 0) > 0 && (
                      <div className="mt-4">
                        ...
                        <div className="space-y-4">
                          {(topic.replies ?? []).map((r) => renderReplyTree(r as any, 0))}
                        </div>
                      </div>
                    )}
