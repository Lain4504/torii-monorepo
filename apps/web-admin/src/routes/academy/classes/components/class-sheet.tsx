...
export function ClassSheet({ open, onOpenChange, academyClass }: ClassSheetProps) {
    ...
    useEffect(() => {
        if (academyClass) {
            reset({
                ...
                syllabusId: (academyClass as any).syllabusId || "", // Type safety issue in current AcademyClass type
                code: academyClass.code,
                name: academyClass.name,
                mode: academyClass.mode,
                instructorId: academyClass.liveClass?.instructorId || null,
                openingDate: academyClass.liveClass?.openingDate ? new Date(academyClass.liveClass.openingDate).toISOString().split('T')[0] : null,
                closingDate: academyClass.liveClass?.closingDate ? new Date(academyClass.liveClass.closingDate).toISOString().split('T')[0] : null,
                maxStudents: (academyClass.liveClass?.maxStudents || academyClass.vodClass?.maxStudents) ?? 0,
            })
        } else {
            reset({
                courseProfileId: "",
    ...
    return (
        <Sheet open={open} onOpenChange={onOpenChange}>
            <SheetContent className="w-full sm:max-w-[800px] flex flex-col p-0">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <Controller
                                                    ...
                                                    render={({ field }) => (
                                                        <Select
                                                            onValueChange={(val) => {
                                                                field.onChange(val);
                                                                setValue("syllabusId", ""); // Reset syllabus when profile changes
                                                            }}
                                                            value={field.value}
                                                            disabled={isEditing}
                                                        >
                                            ...
                                            <Field>
                                                ...
                                                <Controller
                                                    ...
                                                    control={control}
                                                    render={({ field }) => (
                                                        <Select onValueChange={field.onChange} value={field.value} disabled={!selectedCourseProfileId || isEditing}>
                                                            <SelectTrigger>
                                                                <SelectValue placeholder={!selectedCourseProfileId ? "Chọn Profile trước" : "Chọn phiên bản giáo trình"} />
                                                            </SelectTrigger>
                                                            <SelectContent>
                                                                {syllabuses?.map(s => (
                                                                    <SelectItem key={s.id} value={s.id}>{s.versionLabel} {s.name ? `- ${s.name}` : ''}</SelectItem>
                                                                ))}
                                                            </SelectContent>
                                                        </Select>
                                                    )}
                ...
                <SheetFooter className="px-6 py-4 border-t gap-2 bg-muted/20">
