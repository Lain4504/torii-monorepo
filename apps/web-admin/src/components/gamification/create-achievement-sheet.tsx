...
export function CreateAchievementSheet({ open, onOpenChange }: { open: boolean, onOpenChange: (open: boolean) => void }) {
    ...
    const onSubmit: SubmitHandler<AchievementFormValues> = (values) => {
        createAchievement(values, {
            onSuccess: () => {
                toast.success("Đã tạo thành tích mới!");
                onOpenChange(false);
                reset();
            },
            onError: (error: any) => {
                toast.error("Không thể tạo thành tích: " + error.message);
            }
        });
    }
                                ...
                                <FieldSet>
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <Field>
                                            ...
                                            <Input placeholder="Ví dụ: Chiến binh chuyên cần" {...register("title")} />
                                    ...
                                    <Field>
                                        ...
                                        <Textarea placeholder="Học viên sẽ thấy gì khi đạt được..." {...register("description")} />
