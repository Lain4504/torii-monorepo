...
export function DashboardSidebar({ className, isCollapsed, toggleCollapse }: SidebarProps) {
  ...
  const NavGroup = ({ title, items }: { title: string; items: NavItem[] }) => (
    <div className="py-2">
      {!isCollapsed && (
        <h4 className="px-6 py-2 text-[10px] font-serif font-bold italic uppercase tracking-widest text-muted-foreground/40">
          {title}
        </h4>
      )}
          ...
          <Link to="/" className={cn("flex items-center gap-3", isCollapsed ? "justify-center w-full" : "")}>
            ...
            {!isCollapsed && (
              <div className="flex flex-col">
                <span className="text-lg font-serif font-bold italic tracking-tight text-foreground">Torii <span className="text-primary">Admin</span></span>
              ...
              <>
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="flex flex-col min-w-0">
                    ...
                    <span className="truncate text-[10px] font-serif font-bold italic text-muted-foreground/40 uppercase tracking-wider">{user?.role || t('sidebar.defaultRole')}</span>
                  </div>
                </div>
              </>
            ) : (
