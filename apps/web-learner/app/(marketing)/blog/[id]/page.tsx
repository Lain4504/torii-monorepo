...
export default function BlogDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true)
                const [blogData, latestData, topData] = await Promise.all([
                    blogApi.findById(id),
                    blogApi.findAll({ page: 1, limit: 5 }),
                    blogApi.findAll({ page: 1, limit: 5, sortBy: 'viewCount', sortOrder: 'desc' })
                ])
                setBlog(blogData)
    ...
    const headings = blog.content ?
        Array.from(blog.content.matchAll(/<h([2-3])>(.*?)<\/h\1>/g)).map(m => ({
                ...
                <div className="flex">
                    <Breadcrumb>
                        <BreadcrumbList className="gap-2 sm:gap-3 text-xs font-bold text-muted-foreground">
                            ...
                            <BreadcrumbItem>
                                <BreadcrumbLink href="/blog" className="hover:text-primary transition-colors">Bài viết</BreadcrumbLink>
                            </BreadcrumbItem>
                            <BreadcrumbSeparator className="scale-75 opacity-40">
                                <ChevronRight className="w-4 h-4" />
                            </BreadcrumbSeparator>
                        ...
                        <div className="pt-16 border-t">
                            <CommentSection
                                ...
                                onCommentCountChange={(delta) => {
                                    setBlog(prev => prev ? { ...prev, commentCount: (prev.commentCount || 0) + delta } : null)
                                }}
                            />
                        </div>
                    </div>
