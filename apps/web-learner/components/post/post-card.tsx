...
export function PostCard({ post }: PostCardProps) {
    ...
    return (
        <Link href={`/post/${post.id}`} className="block h-full cursor-pointer group">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center gap-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                            <span className="flex items-center gap-1.5">
                                ...
                                {format(new Date(post.publishedAt || post.createdAt), "dd 'thg' MM, yyyy", { locale: vi })}
                            </span>
                        ...
                        <p className="text-sm md:text-base text-muted-foreground line-clamp-2 leading-relaxed">
                            {post.excerpt || 'Khám phá những kiến thức thú vị về tiếng Nhật và văn hóa Nhật Bản...'}
                        </p>
                    </div>
                        ...
                        <div className="flex items-center gap-3">
                            <Avatar className="w-8 h-8 border border-border">
                                ...
                                <AvatarFallback className="text-[10px] font-bold bg-primary/10 text-primary">
                                    {post.author?.displayName?.charAt(0) || 'T'}
                                </AvatarFallback>
                            </Avatar>
                            <span className="text-xs font-bold text-foreground overflow-hidden text-ellipsis whitespace-nowrap max-w-[100px] md:max-w-[150px]">
                                {post.author?.displayName || 'Torii Writer'}
                            </span>
                        </div>
