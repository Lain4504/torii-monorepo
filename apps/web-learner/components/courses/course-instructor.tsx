                    ...
                    <div className="flex flex-col md:flex-row gap-10 items-start">
                        ...
                        <div className="flex-1 space-y-6">
                            <div className="space-y-2">
                                ...
                                <h3 className="text-3xl font-serif font-bold italic tracking-tight text-foreground uppercase">Torii Academic Faculty</h3>
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 italic">Elite linguistic instructors & JLPT specialists</p>
                            </div>
        ...
        <div className="space-y-10 animate-in fade-in duration-700">
            ...
            <div className="grid gap-6 md:gap-8">
                {instructors.map((instructor) => (
                    <div key={instructor.id} className="rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 bg-muted/20 border border-border/40 group hover:border-primary/20 transition-all duration-500">
                        <div className="flex flex-col md:flex-row gap-6 md:gap-10 items-start">
                            <div className="flex-shrink-0 flex flex-row md:flex-col items-center gap-4 w-full md:w-auto">
                                <div className="w-20 h-20 md:w-32 md:h-32 rounded-[1.2rem] md:rounded-[2rem] p-1 md:p-1.5 border-2 border-primary/20 bg-background shadow-2xl relative group-hover:scale-105 transition-transform duration-500">
                                    <Avatar className="w-full h-full rounded-[1rem] md:rounded-[1.5rem]">
                                        <AvatarImage src={instructor.user.avatarUrl ?? undefined} className="object-cover" />
                                        <AvatarFallback className="bg-primary/10 text-primary text-xl md:text-3xl font-black">
                                ...
                                <div className="flex flex-col items-start md:items-center gap-1">
                                    <div className="flex gap-0.5 text-amber-500">
                                        ...
                                        <span className="text-[10px] md:text-[11px] font-black text-foreground ml-1">5.0</span>
                                    </div>
                                    <span className="text-[8px] md:text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Instructor Rating</span>
                                </div>
                            </div>

                            <div className="flex-1 space-y-4 md:space-y-6 w-full">
                                <div className="space-y-2">
                                    <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary/5 text-primary rounded-full text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em]">
                                        <span>Certified Instructor</span>
                                    </div>
                                    <h3 className="text-2xl md:text-4xl font-serif font-bold italic tracking-tight text-foreground uppercase group-hover:text-primary transition-colors leading-tight">
                                        {instructor.user.displayName}
                                    </h3>
                                    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/40 italic">Senior Academic Lead at Torii Nihongo</p>
                                </div>

                                <div className="flex flex-wrap gap-y-4 gap-x-8">
                                    <div className="flex items-center gap-3 group/stat">
                                        <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-background flex items-center justify-center text-muted-foreground group-hover/stat:text-primary transition-colors">
                                            <Users className="w-3.5 h-3.5 md:w-4 md:h-4" />
                                        </div>
                                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Kinh nghiệm dày dặn</span>
                                    </div>
                                    <div className="flex items-center gap-3 group/stat">
                                        <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-background flex items-center justify-center text-muted-foreground group-hover/stat:text-primary transition-colors">
                                            <Award className="w-3.5 h-3.5 md:w-4 md:h-4" />
                                        </div>
                                        <span className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-muted-foreground/60">Hệ thống bài giảng JLPT</span>
                                    </div>
                                </div>

                                <p className="text-xs md:text-sm font-bold text-muted-foreground/80 leading-relaxed italic border-l-2 border-primary/20 pl-4 md:pl-6 break-words">
                                    Mang đến trải nghiệm học tiếng Nhật hiện đại, đơn giản và cực kỳ hiệu quả thông qua lộ trình cá nhân hóa.
                                </p>

                                <Button variant="outline" className="h-10 md:h-12 px-6 md:px-8 rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest border-border/40 hover:bg-muted hover:border-primary/20 transition-all cursor-pointer group/btn active:scale-95 w-full md:w-auto">
                                    Xem hồ sơ chi tiết
                                    <ChevronRight className="ml-2 w-3.5 h-3.5 md:w-4 md:h-4 group-hover/btn:translate-x-1 transition-transform" />
                                </Button>
                            </div>
                        </div>
                    </div>
