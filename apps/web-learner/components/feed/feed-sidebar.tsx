...
export function FeedSidebar({ onSortChange, onSearch }: FeedSidebarProps) {
    ...
    return (
        <div className="space-y-8">
            {/* Search Widget */}
            <Card className="border-border/50 shadow-none bg-card rounded-2xl overflow-hidden group">
                <CardContent className="p-4">
                    <div className="relative">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground transition-colors group-focus-within:text-primary" />
                        <Input
                            placeholder="Tìm kiếm câu hỏi..."
                            className="pl-11 h-12 rounded-xl bg-muted/30 border-border/40 focus:border-primary/40 focus:bg-background transition-all shadow-none font-medium"
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') {
            ...
            <Link href={isProfilePage ? '/dashboard/feed' : (user ? `/user/${(user as any).id}` : '/login')} className="block group">
                <Item variant="outline" className="p-5 border-border/50 transition-all duration-300 shadow-none bg-card rounded-2xl hover:border-primary/50">
                    <ItemMedia className="size-11 rounded-xl bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-all shadow-sm">
                    ...
                    <ItemContent className="space-y-0.5">
                        ...
                        <ItemDescription className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.1em] opacity-60">
            ...
            <Card className="border-border/50 shadow-none bg-card overflow-hidden rounded-2xl">
                <CardHeader className="pb-4 px-6 pt-6 space-y-2">
                    <CardTitle className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60 leading-none">Lọc bài viết</CardTitle>
                ...
                <CardContent className="p-3 pt-4">
                    <div className="space-y-1.5">
                        <Button
                            variant="ghost"
                            className="w-full justify-start gap-3 px-4 h-11 rounded-xl font-bold text-sm text-muted-foreground hover:text-primary hover:bg-primary/5 transition-all group"
                        ...
                        <Button variant="ghost" className="w-full justify-start gap-3 px-4 h-11 rounded-xl font-bold text-sm text-muted-foreground hover:text-foreground/80 transition-all group">
            ...
            <Card className="border-border/50 shadow-none bg-card overflow-hidden rounded-2xl">
                <CardHeader className="pb-4 px-6 pt-6 space-y-2">
                    <CardTitle className="text-[10px] font-bold uppercase tracking-[0.2em] text-orange-500/80 flex items-center gap-2 leading-none">
                ...
                <CardContent className="p-0 pt-3">
                    <div className="divide-y divide-border/5">
                        {loadingHotFeeds ? (
                            <div className="p-12 text-center text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] animate-pulse">
                                Đang tải bài viết...
                            </div>
                        ) : hotFeeds.length > 0 ? (
                            hotFeeds.map((feed, i) => (
                                <Link
                                    key={feed.id}
                                    href={`/feed/${feed.id}`}
                                    className="block p-5 hover:bg-muted/30 cursor-pointer transition-all duration-300 group"
                                >
                                    <div className="flex gap-4">
                                        ...
                                        <div className="flex-1 space-y-2.5">
                                            ...
                                            <div className="flex items-center gap-5 text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.1em]">
                            ...
                            <div className="p-12 text-center text-[10px] font-bold text-muted-foreground/60 uppercase tracking-[0.2em]">
                                Chưa có câu hỏi nào
                            </div>
                        )}
                    </div>
