...
describe('StorageService', () => {
  ...
  describe('generatePresignedUploadUrl', () => {
    it('should generate a presigned upload URL and create a pending record', async () => {
      ...
      mockSharedStorageService.generatePresignedUploadUrl.mockResolvedValue(
        expectedUploadUrl,
      );
      ...
      expect(
        sharedStorageService.generatePresignedUploadUrl,
      ).toHaveBeenCalledWith(expectedKey, dto.contentType);
      expect(sharedStorageService.getPublicUrl).toHaveBeenCalledWith(
        expectedKey,
      );
      expect(storageRepository.update).toHaveBeenCalledWith('mock-uuid-1234', {
        fileUrl: expectedPublicUrl,
      });
  ...
  describe('confirmUpload', () => {
    it('should confirm upload if file exists in shared storage', async () => {
      ...
      const fileAsset = {
        id: 'mock-uuid-1234',
        fileUrl:
          'https://storage.example.com/uploads/profile/mock-uuid-1234.png',
      };
      ...
      expect(sharedStorageService.extractKeyFromUrl).toHaveBeenCalledWith(
        fileAsset.fileUrl,
      );
      expect(sharedStorageService.exists).toHaveBeenCalledWith(extractedKey);
      expect(storageRepository.update).toHaveBeenCalledWith(dto.fileId, {
        status: 'uploaded',
      });
    ...
    it('should throw NotFoundException if file asset not found', async () => {
      ...
      await expect(
        service.confirmUpload({ fileId: 'non-existent' }),
    ...
    it('should handle extraction errors gracefully and use URL as key', async () => {
      ...
      expect(sharedStorageService.exists).toHaveBeenCalledWith(
        fileAssetHttp.fileUrl,
      );
    });
  });
  ...
  describe('deleteFile', () => {
    ...
    it('should throw BadRequestException if database deletion fails due to constraint (Restrict)', async () => {
      ...
      await expect(service.deleteFile(dto)).rejects.toThrow(
        BadRequestException,
      );
    ...
    it('should succeed even if S3 deletion fails after DB record is removed', async () => {
      ...
      mockStorageRepository.findById.mockResolvedValue(fileAsset);
      mockStorageRepository.delete.mockResolvedValue(undefined);
      mockSharedStorageService.delete.mockRejectedValue(
    ...
    it('should throw NotFoundException if file asset not found', async () => {
      ...
      await expect(
        service.deleteFile({ fileId: 'non-existent' }),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getSignedUrl', () => {
    ...
    it('should throw NotFoundException if file asset not found', async () => {
      mockStorageRepository.findById.mockResolvedValue(null);
      await expect(
        service.getSignedUrl({ fileId: 'non-existent' }),
      ).rejects.toThrow(NotFoundException);
    });
  });
});
