...
describe('AuthService', () => {
    ...
    describe('adminLogin', () => {
        ...
        it('should login admin successfully if they have permissions', async () => {
            usersRepository.findByEmail.mockResolvedValue({ ...mockUser, role: UserRole.ADMIN });
            (argon2.verify as jest.Mock).mockResolvedValue(true);
            authorizationService.getUserPermissions.mockResolvedValue({ permissions: ['*'] });
            twoFactorAuthService.get2FAStatus.mockResolvedValue({ isEnabled: false });
            sessionService.createSession.mockResolvedValue({ sessionId: 's1', refreshToken: 'r1' });
            jwtTokenProvider.generateToken.mockResolvedValue('access-token');
    ...
    describe('updateUser', () => {
        it('should update user display name and metadata', async () => {
            ...
            expect(usersRepository.update).toHaveBeenCalledWith(mockUser.id, expect.objectContaining({
    ...
    describe('updateAvatar', () => {
        it('should update avatar and delete old one', async () => {
            ...
            usersRepository.findById.mockResolvedValue({ ...mockUser, avatarUrl: oldAvatarUrl });

            // Mock NATS send using RxJS of()
            natsClient.send.mockImplementation((pattern) => {
                if (pattern.cmd === 'storage.findById') {
                    return of({ status: 'uploaded', fileUrl: 'http://storage.test/new-avatar.png' });
                }
            ...
            usersRepository.update.mockResolvedValue({ ...mockUser, avatarUrl: 'http://storage.test/new-avatar.png' });
            authorizationService.getUserPermissions.mockResolvedValue({ permissions: [] });

            const result = await service.updateAvatar(mockUser.id, fileId);

            expect(result.avatarUrl).toBe('http://storage.test/new-avatar.png');
            expect(natsClient.send).toHaveBeenCalledWith({ cmd: 'storage.deleteFile' }, expect.any(Object));
        });
    });
    ...
    describe('invited user flow', () => {
        ...
        it('should verify invite token', async () => {
            ...
            usersRepository.findById.mockResolvedValue({ ...mockUser, password: null });
        ...
        it('should set password for invited user', async () => {
            ...
            usersRepository.update.mockResolvedValue({ ...mockUser, password: 'new-hashed-password' });
            ...
            expect(natsClient.emit).toHaveBeenCalledWith({ cmd: 'send_email' }, expect.objectContaining({ type: 'welcome' }));
        });
    });
    ...
    describe('OAuth and Providers', () => {
        ...
        it('should register/login with Google successfully', async () => {
            ...
            usersRepository.findByEmail.mockResolvedValue(null);
            usersRepository.create.mockResolvedValue({ ...mockUser, email: googleUser.email });
            authorizationService.getUserPermissions.mockResolvedValue({ permissions: [] });
            sessionService.createSession.mockResolvedValue({ sessionId: 's1', refreshToken: 'r1' });
            jwtTokenProvider.generateToken.mockResolvedValue('access-token');
        ...
        it('should link Google account if not already linked', async () => {
            ...
            expect(userIdentityRepository.create).toHaveBeenCalledWith(expect.objectContaining({
                provider: 'google',
        ...
        it('should throw error when unlinking last provider', async () => {
            ...
            await expect(service.unlinkProvider(mockUser.id, 'email')).rejects.toThrow(BadRequestException);
        });

        it('should return linked providers', async () => {
            userIdentityRepository.findByUserId.mockResolvedValue([{ provider: 'google', createdAt: new Date() }]);
            usersRepository.findById.mockResolvedValue(mockUser);
    ...
    describe('Account Management', () => {
        it('should get current user with permissions', async () => {
            ...
            authorizationService.getUserPermissions.mockResolvedValue({ permissions: ['read:all'] });
    ...
    describe('Verification and OTP Resend', () => {
        ...
        it('should resend verification email', async () => {
            usersRepository.findByEmail.mockResolvedValue({ ...mockUser, verifiedAt: null });
            redis.get.mockResolvedValue(null); // Rate limit check

            await service.resendVerification(mockUser.email);

            expect(natsClient.emit).toHaveBeenCalledWith({ cmd: 'send_email' }, expect.objectContaining({ type: 'verification' }));
        });
        ...
        it('should resend OTP', async () => {
            ...
            expect(natsClient.emit).toHaveBeenCalledWith({ cmd: 'send_email' }, expect.objectContaining({ type: 'otp' }));
        });
    });
});
