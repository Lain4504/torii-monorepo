...
export class ClassReviewHandler {
    ...
    create(
        @Payload()
        data: { classId: string; userId: string; dto: ClassReviewCreateDto },
    ) {
    ...
    update(
        @Payload()
        data: { id: string; userId: string; dto: ClassReviewUpdateDto; isAdmin?: boolean },
    ) {
        return this.reviews.updateReview(data.id, data.userId, data.dto, data.isAdmin);
    }
    ...
    moderate(
        @Payload()
        data: { id: string; moderatorId: string; dto: ClassReviewModerateDto },
    ) {
