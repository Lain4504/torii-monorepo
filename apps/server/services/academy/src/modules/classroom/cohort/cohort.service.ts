...
import { AcademyCohortCreateDTO, AcademyCohortUpdateDTO, AcademyCohortQueryDTO } from '@workspace/schemas';
  ...
  async findById(id: string) {
    const item = await this.prisma.cohort.findUnique({
      ...
      include: { 
        ...
        liveClasses: { include: { instructor: { select: { id: true, displayName: true } }, _count: { select: { enrollments: true } } } } 
      },
    });
  ...
  async update(id: string, data: AcademyCohortUpdateDTO) {
    return this.prisma.cohort.update({
      ...
      data: {
        ...
        rejectionReason: (data as any).rejectionReason,
        submittedForApprovalAt: data.status === 'PENDING_APPROVAL' ? new Date() : undefined,
        enrollmentOpenAt: data.enrollmentOpenAt ? new Date(data.enrollmentOpenAt) : undefined,
        enrollmentCloseAt: data.enrollmentCloseAt ? new Date(data.enrollmentCloseAt) : undefined,
