...
export class LiveClassHandler {
  ...
  findAssignments(@Payload() data: { classId: string }) { return this.service.findAssignments(data.classId); }

  @MessagePattern({ cmd: 'academy.liveClass.addAssignment' })
  addAssignment(@Payload() data: any) { return this.service.addAssignment(data); }
}
