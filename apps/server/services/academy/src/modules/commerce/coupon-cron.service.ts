...
export class CouponCronService {
    ...
    async handleCouponExpirations() {
        ...
        try {
            ...
            if (expiredByDate.count > 0) {
                this.logger.log(`Deactivated ${expiredByDate.count} coupons due to expiration date reached.`);
            }
            ...
            this.logger.error('Error during scheduled coupon expiration check:', error);
        }
    }
    ...
    private async deactivateCouponsReachedLimit(): Promise<number> {
        ...
        if (idsToDeactivate.length > 0) {
            ...
            this.logger.log(`Deactivated ${idsToDeactivate.length} coupons due to usage limit reached.`);
