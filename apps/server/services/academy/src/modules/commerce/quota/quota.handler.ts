...
export class QuotaHandler {
    ...
    async adminGetAllPlans() {
        try {
            ...
            this.logger.error(`Error in admin.billing.subscription.getAllPlans: ${error.message}`, error.stack);
            return [];
        }
    }
    ...
    async adminUpdatePlan(@Payload() data: { id: string; plan: any }) {
        try {
            return await this.aiSubscriptionService.admin_updatePlan(data.id, data.plan);
        } catch (error: any) {
            this.logger.error(`Error in admin.billing.subscription.updatePlan: ${error.message}`, error.stack);
            throw error;
        }
    }

    @MessagePattern({ cmd: 'admin.billing.subscription.getUserSubscriptions' })
    async adminGetUserSubscriptions(@Payload() params: { page: number; limit: number; search?: string }) {
        try {
            return await this.aiSubscriptionService.admin_getUserSubscriptions(params);
        } catch (error: any) {
            this.logger.error(`Error in admin.billing.subscription.getUserSubscriptions: ${error.message}`, error.stack);
