...
export class FeatureQuotaService {
  ...
  async checkAndConsumeQuota(userId: string, featureType: 'roleplay' | 'live') {
    ...
    const trialLimitField =
      featureType === 'roleplay'
        ? 'aiRoleplayTrialLimit'
        : 'liveMeetingTrialLimit';
    ...
    if (trialRemaining > 0) {
      ...
      this.logger.log(
        `User ${userId} consumed 1 trial use for ${featureType}. Remaining: ${trialRemaining - 1}`,
      );
      return {
        allowed: true,
        remainingTrial: trialRemaining - 1,
        chargedCoins: false,
      };
    }
    ...
    this.logger.log(
      `User ${userId} has no trial uses left for ${featureType}. Allowing with token-based billing.`,
    );
  ...
  async getQuotaStatus(userId: string, featureType: 'roleplay' | 'live') {
    ...
    if (!userBalance) {
      ...
      const defaultTrial =
        featureType === 'roleplay'
          ? trialLimits.aiRoleplay
          : trialLimits.liveMeeting;
   ...
   */
  async recordTokenUsageAndDeduct(
    userId: string,
    taskType: string,
    usage: any,
  ) {
    ...
    const serviceKey =
      taskType === 'live_voice' ? 'live_voice' : 'ai_text_chat';
    const serviceConfig = insights.services?.[serviceKey];

    if (!serviceConfig) {
      this.logger.warn(
        `No service config found for '${serviceKey}' in insights.services. Skipping deduction for user ${userId}.`,
      );
      return;
    }
    ...
    const modelKey =
      ...
      (taskType === 'live_voice'
        ...
        : 'gemini-2.0-flash');
    const pricing =
      serviceConfig.pricing?.[modelKey] || serviceConfig.pricing?.['default'];

    if (!pricing) {
      this.logger.warn(
        `No pricing found for service '${serviceKey}' (model: ${modelKey}). Skipping deduction for user ${userId}.`,
      );
      return;
    }
    ...
    const inputCostCoins =
      (promptTokens / 1_000_000) * inputPriceUSD * coinRate;
    const outputCostCoins =
      (completionTokens / 1_000_000) * outputPriceUSD * coinRate;
    ...
    if (totalCostCoins <= 0) {
      this.logger.warn(
        `[billing] Computed 0 coins for user ${userId} (${taskType}). Check pricing config.`,
      );
      return;
    }
    ...
    try {
      ...
      this.logger.error(
        `Failed to deduct ${totalCostCoins} coins for user ${userId}: ${error.message}`,
      );
    }
  }
}
