...
export class UserBalanceHandler {
  ...
  async addBalance(
    ...
    return this.userBalanceService.addBalance(
      data.userId,
      data.amount,
      data.reason,
      data.type,
  ...
  async deductBalance(
    ...
    return this.userBalanceService.deductBalance(
      data.userId,
      data.amount,
      data.reason,
      data.type,
  ...
  async checkAndConsumeUsage(
    @Payload() data: { userId: string; featureType: 'roleplay' | 'live' },
  ) {
    return this.featureQuotaService.checkAndConsumeQuota(
      data.userId,
      data.featureType,
    );
  }
  ...
  async getQuotaStatus(
    ...
    return this.featureQuotaService.getQuotaStatus(
      data.userId,
      data.featureType,
    );
  }
  ...
  async recordTokenUsage(
    @Payload() data: { userId: string; taskType: string; usage: any },
  ) {
    return this.featureQuotaService.recordTokenUsageAndDeduct(
      data.userId,
      data.taskType,
      data.usage,
    );
  ...
  async recordTokenUsageEvent(
    @Payload() data: { userId: string; taskType: string; usage: any },
  ) {
    this.logger.log(
      `[event] recordTokenUsage for user=${data.userId} taskType=${data.taskType}`,
    );
    await this.featureQuotaService.recordTokenUsageAndDeduct(
      data.userId,
      data.taskType,
      data.usage,
    );
