...
export class OrderService implements IOrderService {
    ...
    try {
      ...
      if (count > 0) {
        ...
        for (const order of expiredOrders) {
          if (order.enrollmentId && order.metadata?.courseRunId) {
            this.natsClient
              .send(
                { cmd: 'learning.enrollment.delete' },
                {
                  userId: order.userId,
                  courseRunId: order.metadata.courseRunId,
                },
              )
      ...
      this.logger.error(
        `Error in auto-cancel orders logic: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getStats(
    ...
    try {
      ...
      const stats = await this.orderRepository.aggregate(whereClause, {
        _sum: { amount: true },
        _count: true,
      });
      ...
      this.logger.error(
        `Error fetching order stats: ${error.message}`,
        error.stack,
      );
  ...
  async findById(id: string): Promise<OrderResponseDTO | null> {
    try {
      ...
      this.logger.error(
        `Error fetching order ${id}: ${error.message}`,
        error.stack,
      );
      return null;
    }
  ...
  async create(
    ...
    this.logger.log(
      `[OrderService] Creating order for user: ${userId} with input: ${JSON.stringify(input)}`,
    );
    ...
    if (
      input.orderType === OrderType.TOP_UP &&
      input.paymentMethod !== PaymentMethod.PAYOS
    ) {
      throw new BadRequestException(
        'Nạp tiền vào tài khoản chỉ hỗ trợ qua phương thức PayOS',
      );
    }

    if (
      input.orderType === OrderType.COURSE_PURCHASE &&
      input.paymentMethod !== PaymentMethod.BALANCE
    ) {
      throw new BadRequestException(
        'Mua khóa học chỉ hỗ trợ thanh toán bằng Coin (Số dư ví Torii)',
      );
    }
    ...
    if (input.orderType === OrderType.COURSE_PURCHASE && courseRunId) {
      ...
      if (courseRunId) {
        try {
          ...
          this.logger.error(
            `Error calling learning.courserun.findById: ${error.message}`,
          );
        }
      }
      ...
      if (fetchMasterId) {
        try {
          ...
          this.logger.error(
            `Error calling learning.coursemaster.findById: ${error.message}`,
          );
        }
      }
      ...
      if (courseRun) {
        ...
        this.logger.log(
          `Creating order for run: ${courseRunId}, finalAmount=${amount}`,
        );
      } else {
        throw new BadRequestException(
          'CourseRunId is required for course purchases',
        );
      }

      this.logger.log(
        `Creating order for course: run=${courseRunId}, finalAmount=${amount}`,
      );
    } else if (input.orderType === OrderType.TOP_UP) {
      amount = Number((input as any).amount);
      if (!amount || amount <= 0) {
        throw new BadRequestException(
          'Amount is required for top-up and must be positive',
        );
      }
      this.logger.log(
        `Creating top-up order for user ${userId}: amount=${amount}`,
      );
    } else if (!courseRunId && input.orderType === OrderType.COURSE_PURCHASE) {
      throw new BadRequestException(
        'CourseRunId is required for course_purchase order type',
      );
    }

    // Handle Gift Order Validation
    if (
      input.orderType === OrderType.GIFT ||
      (input.metadata && input.metadata.isGift)
    ) {
      const recipientEmail = input.metadata?.recipientEmail;
      if (!recipientEmail) {
        throw new BadRequestException(
          'Recipient email is required for gift orders',
        );
      }

      if (
        recipientEmail ===
        (await this.orderRepository.getUserById(userId))?.email
      ) {
      ...
      try {
        ...
        if (!identityResponse || !identityResponse.user) {
          throw new BadRequestException(
            `Recipient with email ${recipientEmail} not found`,
          );
        }
    ...
    if (input.couponCode) {
      this.logger.log(
        `[OrderService] Attempting to redeem coupon: ${input.couponCode}`,
      );
      try {
        ...
        if (couponDiscount >= amount) {
          ...
          throw new BadRequestException(
            'Mã giảm giá không hợp lệ cho đơn hàng này (Giá trị khuyến mãi phải nhỏ hơn giá khóa học)',
          );
        }
        ...
        this.logger.log(
          `Coupon ${input.couponCode} applied. Discount: ${couponDiscount}. Final Amount: ${amount}`,
        );
      } catch (error: any) {
        this.logger.warn(
          `Failed to apply coupon ${input.couponCode}: ${error.message}`,
        );
        throw error instanceof BadRequestException
          ? error
          : new BadRequestException(`Coupon error: ${error.message}`);
      }
    } else {
    ...
    try {
      const metadata = {
        ...
        ...(discountAmount !== undefined && {
          discountAmount: (discountAmount || 0) + couponDiscount,
        }), // Combine course discount and coupon discount
      ...
      if (
        ...
        try {
          ...
          this.logger.log(
            `Created pending enrollment ${enrollmentId} for order`,
          );
        } catch (e: any) {
          this.logger.error(
            `Failed to create pending enrollment: ${e.message}`,
          );
          // Should we block order creation? strict consistency says yes.
          throw new BadRequestException(
            `Failed to initialize enrollment: ${e.message}`,
          );
        }
      }
      ...
      if (enrollmentId) {
        this.natsClient.emit(
          { cmd: 'learning.enrollment.updateOrderId' },
          {
            id: enrollmentId,
            orderId: created.id,
          },
      ...
      if (input.paymentMethod === PaymentMethod.PAYOS) {
        try {
          ...
          const returnUrl =
            ...
            (input as any).returnUrl ||
            `${webLearnerUrl}/checkout/return?order_id=${created.id}`;
          const cancelUrl =
            input.metadata?.cancelUrl ||
            (input as any).cancelUrl ||
            `${webLearnerUrl}/checkout/return?order_id=${created.id}`;
          ...
          this.logger.log(
            `Created PayOS payment link for order ${created.id}: ${paymentLinkData.checkoutUrl}`,
          );
      ...
      if (
        input.paymentMethod === PaymentMethod.BALANCE &&
        created.orderType === OrderType.COURSE_PURCHASE
      ) {
  ...
  async confirm(
    ...
    try {
      const transactionId =
        input.transactionId || `PAY-${Date.now()}-${orderId.substring(0, 8)}`;
      ...
      if (
        (order.orderType === OrderType.COURSE_PURCHASE ||
          order.orderType === OrderType.GIFT) &&
        metadata?.courseMasterId
      ) {
        try {
          // Check if it's a gift
          const isGift =
            order.orderType === OrderType.GIFT || !!metadata.isGift;
          const targetUserId = isGift ? metadata.recipientId : order.userId;

          if (isGift && !targetUserId) {
            this.logger.error(
              `Gift order ${orderId} missing recipientId in metadata`,
            );
            throw new BadRequestException(
              'Gift order missing recipient information',
            );
          }
          ...
          if (isGift) {
            ...
            this.logger.log(
              `Gift enrollment created for user ${targetUserId} and run ${metadata.courseRunId} (ID: ${enrollment?.id})`,
            );
            ...
            if (enrollmentId) {
              await lastValueFrom(
                this.natsClient.send(
                  { cmd: 'learning.enrollment.activate' },
                  { enrollmentId },
                ),
              );
            ...
            if (!enrollmentId) {
              ...
              this.logger.warn(
                `Order ${orderId} missing enrollmentId, attempting to create/find...`,
              );
              const enrollmentPayload = {
                userId: targetUserId,
                courseRunId: metadata.courseRunId,
              ...
              this.logger.log(
                `Activated enrollment ${enrollmentId} for order ${orderId}`,
              );
            }
          }
          ...
          try {
            ...
            if (course && user) {
              ...
              this.logger.log(
                `order_payment_success event emitted for order ${orderId}`,
              );

              if (isGift && targetUserId) {
                const recipientUser =
                  await this.orderRepository.getUserById(targetUserId);
                if (recipientUser) {
                  this.natsClient.emit(
                    { cmd: 'course_gift_received' },
                    {
                  ...
                  this.logger.log(
                    `course_gift_received event emitted for recipient ${recipientUser.email}`,
                  );
                }
              }
            }
            ...
            this.logger.error(
              `Failed to emit payment success event: ${eventError.message}`,
            );
          }
        } catch (enrollError: any) {
          this.logger.warn(
            `Failed to create/activate enrollment after payment: ${enrollError.message}`,
          );
        }
      }
      ...
      if (order.orderType === OrderType.TOP_UP) {
        try {
          ...
          this.logger.log(
            `User ${order.userId} account credited with ${order.amount} balance`,
          );
        } catch (balanceError: any) {
          this.logger.error(
            `Failed to credit balance for top-up: ${balanceError.message}`,
          );
        }
      }
  ...
  async handleWebhook(webhookData: any): Promise<any> {
    ...
    if (!orderCode) {
      this.logger.error(
        `Webhook data missing orderCode: ${JSON.stringify(webhookData)}`,
      );
    ...
    const order =
      await this.orderRepository.findByTransactionId(payOsOrderCode);

    if (!order) {
      this.logger.warn(
        `Order not found for PayOS OrderCode: ${payOsOrderCode}`,
      );
    ...
    if (code !== '00') {
      this.logger.warn(
        `PayOS transaction failed/cancelled: ${desc} (Code: ${code}, OrderCode: ${orderCode})`,
      );
    ...
    try {
      await this.orderRepository.createPayment({
        ...
        status:
          order.status === OrderStatus.COMPLETED ? 'duplicate' : 'success',
    ...
    if (Number(amount) < Number(order.amount)) {
      this.logger.warn(
        `Payment amount mismatch for ${order.id}. Expected ${order.amount}, got ${amount}`,
      );
  ...
  async cancel(
    ...
    this.logger.log(
      `Cancelling order ${id} with status ${order.status} by user ${userId} (role: ${userRole})`,
    );
  ...
  async refund(id: string, reason?: string): Promise<OrderResponseDTO> {
    ...
    if (courseRunId) {
      try {
        ...
        this.logger.error(
          `Failed to process un-enrollment/refund logic properly: ${error.message}`,
        );
  ...
  async exportOrders(query: OrderQueryDTO): Promise<any[]> {
    try {
      ...
      return orders.map((order) => ({
        ...
        couponDiscount: order.metadata ? order.metadata.couponDiscount || 0 : 0,
        originalCourseAmount: order.metadata
          ? order.metadata.originalAmount || Number(order.amount)
          : Number(order.amount),
  ...
  async payWithBalance(
    ...
    if (
      ...
      throw new BadRequestException(
        'Balance payment is only available for course purchases',
      );
    }
    ...
    try {
      ...
      this.logger.error(
        `Balance payment failed for order ${orderId}: ${error.message}`,
      );
