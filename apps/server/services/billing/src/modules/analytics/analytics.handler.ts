...
export class AnalyticsHandler {
  ...
  async getOverview() {
    ...
    const growthData = sales.reduce((acc: any, sale) => {
      ...
      const month = sale.completedAt.toLocaleString('default', {
        month: 'short',
      });
    ...
    const revenueByLevel = sales.reduce((acc: any, sale) => {
      const level =
        sale.enrollment?.courseRun?.courseMaster?.jlptLevel || 'Other';
    ...
    return {
      ...
      growthData: Object.entries(growthData).map(([name, total]) => ({
        name,
        total,
      })),
      revenueByLevel: Object.entries(revenueByLevel).map(([level, amount]) => ({
        level,
        amount,
      })),
    };
