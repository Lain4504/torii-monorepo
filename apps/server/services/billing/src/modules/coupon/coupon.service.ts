...
export class CouponService {
  ...
  async validateCoupon(
    ...
    if (coupon.status !== CouponStatus.ACTIVE) {
      this.logger.warn(
        `Coupon ${code} verification failed: Status is ${coupon.status}`,
      );
    ...
    if (now < coupon.validFrom || now > coupon.validUntil) {
      this.logger.warn(
        `Coupon ${code} verification failed: Date mismatch. Now: ${now}, From: ${coupon.validFrom}, Until: ${coupon.validUntil}`,
      );
    ...
    if (coupon.usageLimit !== null && coupon.usageCount >= coupon.usageLimit) {
      this.logger.warn(
        `Coupon ${code} verification failed: Usage limit reached (${coupon.usageCount}/${coupon.usageLimit})`,
      );
    ...
    if (coupon.minOrderAmount && orderAmount < Number(coupon.minOrderAmount)) {
      this.logger.warn(
        `Coupon ${code} verification failed: Min order amount ${coupon.minOrderAmount} > ${orderAmount}`,
      );
      return {
        isValid: false,
    ...
    if (isRestricted) {
      ...
      // Then check master inclusion
        ...
        courseMasterId &&
        coupon.applicableCourseMasterIds.includes(courseMasterId)
      ) {
        isFound = true;
      }
    ...
    if (courseRunId && coupon.excludedRunIds.includes(courseRunId)) {
      return {
        isValid: false,
        message: 'Coupon is excluded for this specific run',
      };
    }
    if (
      courseMasterId &&
      coupon.excludedCourseMasterIds.includes(courseMasterId)
    ) {
    ...
    if (coupon.userId && coupon.userId !== userId) {
      this.logger.warn(
        `Coupon ${code} verification failed: Ownership mismatch. Coupon belongs to ${coupon.userId}, but used by ${userId}`,
      );
      return { isValid: false, message: 'Mã giảm giá không thuộc về bạn' };
    }

    // Check user usage limit
    const userUsage = await this.couponRepository.checkUserUsage(
      userId,
      coupon.id,
    );
    if (coupon.userUsageLimit && userUsage >= coupon.userUsageLimit) {
      this.logger.warn(
        `Coupon ${code} verification failed: User ${userId} usage limit reached (${userUsage}/${coupon.userUsageLimit})`,
      );
      return {
        isValid: false,
    ...
    const { finalPrice, discountAmount } = this.calculateDiscount(
      orderAmount,
      coupon,
    );
  ...
  async redeemCoupon(
    ...
    if (!acquired) {
      ...
      throw new ConflictException(
        'Coupon is currently being processed by another user. Please try again.',
      );
    }
    ...
    try {
      ...
      if (!validation.coupon) {
        throw new InternalServerErrorException(
          'Validation passed but coupon missing',
        );
      }
  ...
  async releaseCoupon(couponId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to release coupon ${couponId}: ${error.message}`,
      );
    }
  }
  ...
  async getCouponsForUser(userId: string) {
    ...
    for (const coupon of coupons) {
      ...
      if (coupon.userUsageLimit) {
        const usage = await this.couponRepository.checkUserUsage(
          userId,
          coupon.id,
        );
