...
describe('PayOSService', () => {
  ...
  describe('Logger tests', () => {
    it('should log warning if configuration is missing', async () => {
      ...
      expect(loggerSpy).toHaveBeenCalledWith(
        expect.stringContaining('PayOS configuration is missing'),
      );
  ...
  describe('Initialization', () => {
    ...
    it('should handle missing configuration gracefully', async () => {
      ...
      await expect(
        serviceNoConfig.createPaymentLink({} as any),
      ).rejects.toThrow(BadRequestException);
      expect(() => serviceNoConfig.verifyPaymentWebhookData({})).toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('createPaymentLink', () => {
    ...
    it('should successfully create a payment link', async () => {
      ...
      expect(mockPayOSInstance.createPaymentLink).toHaveBeenCalledWith(
        paymentData,
      );
    ...
    it('should throw BadRequestException when PayOS returns an error', async () => {
      ...
      await expect(service.createPaymentLink(paymentData)).rejects.toThrow(
        'PayOS error: Invalid amount',
      );
    });

    it('should log error when createPaymentLink fails', async () => {
      const loggerSpy = jest.spyOn(Logger.prototype, 'error');
      mockPayOSInstance.createPaymentLink.mockRejectedValue(
      ...
      expect(loggerSpy).toHaveBeenCalledWith(
        expect.stringContaining(
          'Error creating PayOS payment link: Internal Server Error',
        ),
        expect.any(String),
      );
      loggerSpy.mockRestore();
  ...
  describe('verifyPaymentWebhookData', () => {
    ...
    it('should successfully verify webhook data', () => {
      mockPayOSInstance.verifyPaymentWebhookData.mockReturnValue(
        webhookData.data,
      );

      const result = service.verifyPaymentWebhookData(webhookData);

      expect(mockPayOSInstance.verifyPaymentWebhookData).toHaveBeenCalledWith(
        webhookData,
      );
    ...
    it('should propagate error if verifyPaymentWebhookData throws', () => {
      ...
      expect(() => service.verifyPaymentWebhookData(webhookData)).toThrow(
        'Checksum mismatch',
      );
    });
  });
