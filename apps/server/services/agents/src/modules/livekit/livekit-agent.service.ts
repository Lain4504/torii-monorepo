...
export class LivekitAgentService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (!googleApiKey || !livekitUrl) {
      this.logger.log(
        'Missing Gemini API Key or LiveKit URL in config. LiveKit Agent will not start.',
      );
      this.logger.warn(
        'Missing Gemini API Key or LiveKit URL in config. LiveKit Agent will not start.',
      );
  ...
  async joinRoom(roomName: string, participantIdentity?: string) {
    ...
    if (lastJoinTime && Date.now() - lastJoinTime < 300000) {
      this.logger.log(
        `Join request ignored: Agent already joining/active in room ${roomName}`,
      );
    ...
    if (participantIdentity) {
      ...
      if (lastUserJoin && Date.now() - lastUserJoin < 300000) {
        this.logger.log(
          `Join request ignored: User ${participantIdentity} already has an active agent`,
        );
        return { success: true, alreadyJoining: true };
      }
    ...
    try {
      this.logger.log(
        `Dispatching local job for room: ${roomName} (participant: ${participantIdentity})`,
      );
      ...
      this.activeJoins.delete(roomName);
      if (participantIdentity)
        this.activeJoins.delete(`user:${participantIdentity}`);
      throw e;
    }
  }
  ...
  clearJoinLock(roomName: string, userId?: string) {
    this.logger.log(
      `Clearing join locks for room: ${roomName}, user: ${userId}`,
    );
    this.activeJoins.delete(roomName);
    if (userId) {
