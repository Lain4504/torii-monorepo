...
const VALID_RUN_TRANSITIONS: Record<CourseRunStatus, CourseRunStatus[]> = {
  ...
  [CourseRunStatus.PENDING_REVIEW]: [
    CourseRunStatus.APPROVED,
    CourseRunStatus.CANCELLED,
    CourseRunStatus.CHANGES_REQUIRED,
...
export class CourseRunService {
  ...
  async create(
    ...
    if (!this.hasPermission(requester, 'course.update')) {
      throw new ForbiddenException(
        'You do not have permission to create course runs',
      );
    }

    const courseMaster = await this.courseRepository.findById(
      dto.courseMasterId,
    );
    if (!courseMaster) {
      throw new NotFoundException(
        `Course master with id ${dto.courseMasterId} not found`,
      );
    }
    ...
    if ((courseMaster as any).type === 'vod') {
      const existingRunCount = await this.courseRunRepository.count({
    ...
    if (!latestVersion) {
      throw new BadRequestException(
        'Cannot create a run for an empty or unpublished course master. Please publish the course master first.',
      );
    }
  ...
  async update(
    ...
    await this.emitAuditLog(
      ...
      'courserun.update',
      id,
      `Updated course run: ${updated.title}`,
      existing,
      updated,
    );
  ...
  async findMyRuns(
    ...
    if (!requester || !requester.sub) {
      throw new ForbiddenException(
        'User is not authenticated properly to access my course runs',
      );
    }
  ...
  async updateStatus(
    ...
    if (status === CourseRunStatus.ENROLLING && !isVod) {
      if (!existing.startDate) {
        throw new BadRequestException(
          'Cannot open enrollment without a start date set',
        );
      }
    }
  ...
  async reviewRunContent(
    ...
    if (
      ...
      requester.role !== UserRole.STAFF_LMS &&
  ...
  async delete(requester: Requester, id: string): Promise<void> {
    ...
    await this.emitAuditLog(
      ...
      'courserun.delete',
      id,
      `Deleted course run: ${existing.title}`,
    );
  }
  ...
  private hasPermission(requester: Requester, permission: string): boolean {
    ...
    return (
      role === UserRole.ADMIN ||
      role === UserRole.STAFF ||
      role === UserRole.STAFF_LMS ||
      (requester.permissions?.includes(permission) ?? false)
    );
  }
  ...
  private checkOwnership(requester: Requester, run: any) {
    ...
    throw new ForbiddenException(
      'You do not have permission to manage this course run.',
    );
  }
