...
export class CourseRunScheduler {
  ...
  async handleExpiredEnrollment() {
    this.logger.log(
      '🕐 Auto-handle expired enrollment course runs triggered (hourly)',
    );
    try {
      const courseRuns =
        await this.courseRunRepository.findExpiredEnrollmentCourseRuns();
      ...
      for (const run of courseRuns) {
        try {
          ...
          if (run.totalStudents < minStudents) {
            ...
            this.logger.log(
              `🚫 Course run ${run.id} cancelled due to insufficient enrollment (${run.totalStudents}/${run.minStudents})`,
            );
            cancelledCount++;
          } else {
          ...
          this.logger.error(
            `Failed to process course run ${run.id}: ${error?.message}`,
          );
        }
      }

      this.logger.log(
        `✅ Auto-handle expired enrollment completed: ${cancelledCount} cancelled, ${stayCount} kept active`,
      );
    } catch (error: any) {
      this.logger.error(
        `❌ Error in course run scheduler: ${error?.message}`,
        error?.stack,
      );
    }
  }
