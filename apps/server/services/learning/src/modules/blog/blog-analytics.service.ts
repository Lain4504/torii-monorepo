...
export class BlogAnalyticsService {
  ...
  async generateDailyBlogInteractionStats(): Promise<void> {
    try {
      ...
      // Log for debugging
      this.logger.log(
        `Generating daily summary for: ${dateString} (Asia/Ho_Chi_Minh)`,
      );
      this.logger.log(`  Yesterday start (UTC): ${startUTC.toISOString()}`);
      this.logger.log(`  Yesterday end (UTC): ${endUTC.toISOString()}`);
      this.logger.log(
        `  Yesterday start (Asia/HCM): ${yesterdayStart.toISOString()}`,
      );
      this.logger.log(
        `  Yesterday end (Asia/HCM): ${yesterdayEnd.toISOString()}`,
      );
      ...
      this.logger.log(
        `Found ${allPublishedBlogs.length} published blogs to check`,
      );
      ...
      this.logger.log(
        `Filtered to ${validBlogs.length} blogs (published on or before ${dateString})`,
      );
