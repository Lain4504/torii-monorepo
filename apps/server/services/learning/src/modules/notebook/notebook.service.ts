...
export class NotebookService implements INotebookService {
    ...
    async createNotebook(data: CreateNotebookDTO & { userId: string }): Promise<NotebookResponseDTO> {
        ...
        try {
            ...
            if (existing > 0) {
                throw new RpcException({ status: 400, message: 'Bạn đã có sổ tay với tên này rồi' });
            }
            ...
            throw new RpcException({ status: 400, message: `Failed to create notebook: ${error?.message}` });
        }
    }

    async findAllNotebooks(params: { userId: string; search?: string }): Promise<NotebookResponseDTO[]> {
    ...
    async deleteNotebook(id: string, userId: string): Promise<void> {
        try {
            ...
            if (error instanceof RpcException) throw error;
            throw new RpcException({ status: 500, message: `Failed to delete notebook: ${error?.message}` });
        }
    }

    // ── NoteEntry CRUD ───────────────────────────────────────────

    async addEntry(notebookId: string, data: CreateNoteEntryDTO, userId: string): Promise<NoteEntryResponseDTO> {
        try {
    ...
    async updateEntry(notebookId: string, entryId: string, data: UpdateNoteEntryDTO, userId: string): Promise<NoteEntryResponseDTO> {
        try {
            await this.verifyOwnership(notebookId, userId);

            const entry = await this.notebookRepo.findEntryById(entryId);
            if (!entry || entry.notebookId !== notebookId) {
                throw new RpcException({ status: 404, message: 'Note entry not found' });
            }

            const updated = await this.notebookRepo.updateEntry(entryId, {
                ...(data.word !== undefined && { word: data.word.trim() }),
                ...(data.phonetic !== undefined && { phonetic: data.phonetic?.trim() || null }),
    ...
    async deleteEntry(notebookId: string, entryId: string, userId: string): Promise<void> {
        try {
            await this.verifyOwnership(notebookId, userId);
            ...
            await this.notebookRepo.update(notebookId, { entryCount: { decrement: 1 } });
        } catch (error: any) {
            if (error instanceof RpcException) throw error;
            throw new RpcException({ status: 500, message: `Failed to delete entry: ${error?.message}` });
        }
    }

    async bulkCreateEntries(notebookId: string, data: BulkCreateEntriesDTO, userId: string): Promise<{ count: number }> {
        try {
            await this.verifyOwnership(notebookId, userId);
            ...
            // Update entryCount
            await this.notebookRepo.update(notebookId, { entryCount: { increment: count } });

            return { count };
        } catch (error: any) {
            if (error instanceof RpcException) throw error;
            throw new RpcException({ status: 400, message: `Failed to bulk create entries: ${error?.message}` });
        }
    }
}
