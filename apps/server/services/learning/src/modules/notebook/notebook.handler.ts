...
export class NotebookHandler {
    ...
    async create(@Payload() data: { name: string; description?: string; isPublic?: boolean; userId: string }) {
    ...
    async findPublic(@Payload() data: { search?: string; excludeUserId?: string }) {
    ...
    async update(@Payload() data: { id: string; input: any; userId: string }) {
        return this.notebookService.updateNotebook(data.id, data.input, data.userId);
    }
    ...
    async addEntry(@Payload() data: { notebookId: string; entry: any; userId: string }) {
        return this.notebookService.addEntry(data.notebookId, data.entry, data.userId);
    }

    @MessagePattern({ cmd: 'learning.notebook.entry.update' })
    async updateEntry(@Payload() data: { notebookId: string; entryId: string; entry: any; userId: string }) {
        return this.notebookService.updateEntry(data.notebookId, data.entryId, data.entry, data.userId);
    }

    @MessagePattern({ cmd: 'learning.notebook.entry.delete' })
    async deleteEntry(@Payload() data: { notebookId: string; entryId: string; userId: string }) {
        await this.notebookService.deleteEntry(data.notebookId, data.entryId, data.userId);
        return { success: true };
    }

    @MessagePattern({ cmd: 'learning.notebook.entry.bulkCreate' })
    async bulkCreateEntries(@Payload() data: { notebookId: string; entries: any[]; userId: string }) {
        return this.notebookService.bulkCreateEntries(data.notebookId, { entries: data.entries }, data.userId);
    }
}
