...
export class SubmissionHandler {
  ...
  async saveDraft(
    @Payload()
    data: SubmitAssignmentDto & { assignmentId: string; requester: Requester },
  ) {
  ...
  async grade(
    @Payload() data: GradeSubmissionDto & { id: string; requester: Requester },
  ) {
  ...
  async returnSubmission(
    @Payload() data: ReturnSubmissionDto & { id: string; requester: Requester },
  ) {
    const { id, requester, ...dto } = data;
  ...
  async getMySubmission(
    ...
    return this.submissionService.getMySubmission(
      userId,
      assignmentId,
      courseRunId,
    );
  ...
  async findAll(
    @Payload() data: { assignmentId: string; courseRunId?: string },
  ) {
