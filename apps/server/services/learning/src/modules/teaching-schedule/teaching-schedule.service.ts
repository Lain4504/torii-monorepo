...
export class TeachingScheduleService implements ITeachingScheduleService {
  ...
  async assignSchedule(
    requester: Requester,
    dto: TeachingScheduleCreateDTO,
  ): Promise<TeachingScheduleResponseDTO> {
    if (!this.hasPermission(requester, 'live_class.schedule')) {
      throw new ForbiddenException(
        'Only authorized staff can assign teaching schedules',
      );
    }
    ...
    const validation = await this.courseMasterService.validateForScheduling(
      run.courseMasterId,
    );
    if (!validation.isReady) {
      throw new ConflictException(
        validation.message || 'Khóa học chưa sẵn sàng để gán lịch',
      );
    }
    ...
    return this.mapper.map<any, TeachingScheduleResponseDTO>(
      schedule,
      'TeachingSchedule',
      'TeachingScheduleResponseDTO',
    );
  ...
  async findByRun(courseRunId: string): Promise<TeachingScheduleResponseDTO[]> {
    ...
    return schedules.map((s) =>
      this.mapper.map<any, TeachingScheduleResponseDTO>(
        s,
        'TeachingSchedule',
        'TeachingScheduleResponseDTO',
      ),
  ...
  async removeSchedule(requester: Requester, id: string): Promise<void> {
    if (!this.hasPermission(requester, 'live_class.schedule')) {
      throw new ForbiddenException(
        'Only authorized staff can remove teaching schedules',
      );
    }
  ...
  async createRequest(
    ...
    return this.mapper.map<any, ScheduleRequestResponseDTO>(
      request,
  ...
  async getPendingRequests(
    ...
    const canManageSchedules = this.hasPermission(
      requester,
      'live_class.schedule',
    );
    ...
    return requests.map((r) =>
      this.mapper.map<any, ScheduleRequestResponseDTO>(
        r,
        'LiveSessionScheduleRequest',
        'ScheduleRequestResponseDTO',
      ),
  ...
  async handleRequest(
    ...
    if (!this.hasPermission(requester, 'live_class.schedule')) {
      throw new ForbiddenException(
        'Only authorized staff can handle schedule requests',
      );
    }
    ...
    if (action === 'approve') {
      ...
      if (!availability.available) {
        throw new ConflictException(
          'Giảng viên đã có lịch dạy trùng, không thể phê duyệt',
        );
      }
