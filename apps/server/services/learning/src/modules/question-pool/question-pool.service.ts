...
export class QuestionPoolService implements IQuestionPoolService {
  ...
  private toQuestionPoolDto(pool: QuestionPool): QuestionPoolResponseDTO {
    return this.mapper.map<QuestionPool, QuestionPoolResponseDTO>(
      pool,
      'QuestionPool',
      'QuestionPoolResponseDTO',
    );
  ...
  private checkPermission(requester: Requester, action: string): void {
    ...
    if (!hasPermission) {
      throw new ForbiddenException(
        `Only authorized staff can ${action} question pools`,
      );
    }
  }
  ...
  async findAll(
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to fetch pools: ${error?.message || 'Unknown error'}`,
      );
    }
  }
  ...
  async create(
    ...
    try {
      ...
      throw new BadRequestException(
        `Failed to create pool: ${error?.message || 'Unknown error'}`,
      );
    }
  }
  ...
  async update(
    ...
    try {
      ...
      if (dto.courseMasterId !== undefined)
        updateData.courseMasterId = dto.courseMasterId;
      ...
      this.logger.error(
        `Error updating pool ${poolId}: ${error.message}`,
        error.stack,
      );
      throw new BadRequestException(
        `Failed to update pool: ${error?.message || 'Unknown error'}`,
      );
    }
  }
  ...
  async delete(
    ...
    try {
      ...
      this.logger.error(
        `Error deleting pool ${poolId}: ${error.message}`,
        error.stack,
      );
      throw new BadRequestException(
        `Failed to delete pool: ${error?.message || 'Unknown error'}`,
      );
    }
  }
  ...
  async getByCourse(
    ...
    const pools =
      await this.questionPoolRepository.findByCourse(courseMasterId);
