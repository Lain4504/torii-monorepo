...
export class LiveSessionHandler {
  ...
  constructor(
    @Inject(LIVE_SESSION_SERVICE_TOKEN)
    private readonly liveSessionService: ILiveSessionService,
  ...
  async handleRoomEnded(@Payload() data: { roomId: string; roomSID: string }) {
    ...
    try {
      ...
      const session = await (this.liveSessionService as any).syncEndedSession(
        data.roomId,
      );
      if (session) {
      ...
      this.logger.error(
        `Failed to sync ended session or calc attendance: ${error.message}`,
      );
    }
  }
  ...
  async handleUserJoined(@Payload() data: { roomId: string; userId: string }) {
    ...
    try {
      ...
      const session = await this.liveSessionService.findByMeetingId(
        data.roomId,
      );
      if (session) {
  ...
  async create(
    @Payload() data: LiveSessionCreateDTO & { requester: Requester },
  ) {
  ...
  async bulkCreate(
    @Payload() data: LiveSessionBulkCreateDTO & { requester: Requester },
  ) {
    const { requester, ...dto } = data;
  ...
  async update(
    @Payload()
    data: LiveSessionUpdateDTO & { id: string; requester: Requester },
  ) {
