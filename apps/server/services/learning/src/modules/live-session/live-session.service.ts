...
export class LiveSessionService implements ILiveSessionService {
  ...
  async findById(id: string): Promise<LiveSessionResponseDTO> {
    ...
    return this.mapper.map<any, LiveSessionResponseDTO>(
      session,
      'LiveSession',
      'LiveSessionResponseDTO',
    );
  ...
  async findByRunId(courseRunId: string): Promise<LiveSessionResponseDTO[]> {
    ...
    return sessions.map((s) =>
      this.mapper.map<any, LiveSessionResponseDTO>(
        s,
        'LiveSession',
        'LiveSessionResponseDTO',
      ),
  ...
  async bulkCreate(
    ...
    if (!this.hasPermission(requester, 'live_class.schedule')) {
      throw new ForbiddenException(
        'Only authorized staff can schedule live sessions',
      );
    }
    ...
    if (!run) {
      throw new NotFoundException(
        `CourseRun with id ${dto.courseRunId} not found`,
      );
    }
    ...
    if (!course) {
      throw new NotFoundException(
        `Course with id ${run.courseMasterId} not found`,
      );
    }

    if (course.type !== 'live') {
      throw new BadRequestException(
        'Live sessions can only be scheduled for live course runs',
      );
    }
    ...
    for (let i = 0; i < dto.dates.length; i++) {
      ...
      const session = await this.liveSessionRepository.create({
        ...
        lecturer: dto.lecturerId
          ? { connect: { id: dto.lecturerId } }
          : undefined,
      });
      createdSessions.push(session);
    }

    return createdSessions.map((s) =>
      this.mapper.map<any, LiveSessionResponseDTO>(
        s,
        'LiveSession',
        'LiveSessionResponseDTO',
  ...
  async create(
    ...
    if (!course || course.type !== 'live') {
      throw new BadRequestException(
        'Live sessions can only be created for live course runs',
      );
    }
  ...
  async update(
    requester: Requester,
    id: string,
    dto: LiveSessionUpdateDTO,
  ): Promise<LiveSessionResponseDTO> {
    // Only authorized users can update live sessions
    if (!this.hasPermission(requester, 'live_class.schedule')) {
      throw new ForbiddenException(
        'Only authorized staff can update live sessions',
      );
    }
  ...
  async delete(requester: Requester, id: string): Promise<{ message: string }> {
    ...
    if (!this.hasPermission(requester, 'live_class.schedule')) {
      throw new ForbiddenException(
        'Only authorized staff can delete live sessions',
      );
    }
  ...
  async startSession(
    ...
    const hasManagePermission = this.hasPermission(
      requester,
      'live_class.manage',
    );
      ...
      const createRoomReq = create(CreateRoomReqSchema, {
        ...
        metadata: create(RoomMetadataSchema, {
          ...
          roomFeatures: create(RoomCreateFeaturesSchema, {
            ...
            externalMediaPlayerFeatures: create(
              ExternalMediaPlayerFeaturesSchema,
              {
                isAllow: true,
              },
            ...
            displayExternalLinkFeatures: create(
              DisplayExternalLinkFeaturesSchema,
              {
                isAllow: true,
              },
            ...
            insightsFeatures: create(InsightsFeaturesSchema, {
              ...
              chatTranslationFeatures: create(
                InsightsChatTranslationFeaturesSchema,
                {
                  isAllow: true,
                },
  ...
  async endSession(
    ...
    if (existing.meetingId) {
      try {
        ...
        this.logger.log(
          `Ended WebRTC room for live session: ${existing.meetingId}`,
        );
      } catch (error) {
  ...
  async joinSession(
    requester: Requester,
    id: string,
  ): Promise<LiveSessionJoinResponseDTO> {
    ...
    const hasViewRestricted = this.hasPermission(
      requester,
    ...
    try {
      ...
      const roomInfo = await lastValueFrom(
        this.natsClient.send(
          ...
          { roomId: session.meetingId, isRunning: true },
        ),
      );
      ...
      throw new BadRequestException(
        `Failed to join live session: ${error.message}`,
      );
    }
  }

  async syncEndedSession(
    meetingId: string,
