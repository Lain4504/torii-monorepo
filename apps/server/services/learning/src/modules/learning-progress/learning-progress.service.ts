...
export class LearningProgressService implements ILearningProgressService {
  ...
  async getMyCourses(userId: string) {
    ...
    const enrollments = await this.enrollmentRepo.findMany({
      where: {
        userId,
        completionStatus: {
          in: [EnrollmentStatus.IN_PROGRESS, EnrollmentStatus.COMPLETED] as any,
        },
      },
      include: {
        courseRun: {
    ...
    const data = await Promise.all(
      enrollments.map(async (e: any) => {
        ...
        if (e.versionId) {
          try {
            const courseVersion = await this.courseRepo.getVersionById(
              e.versionId,
            );
            ...
            this.logger.warn(
              `Failed to get snapshot for enrollment ${e.id}: ${err}`,
            );
          }
        }
        ...
        if (totalLessons === 0) {
          totalLessons = await this.lessonRepo.count({
            module: {
              courseMasterId: e.courseRun.courseMaster.id,
              deletedAt: null,
            },
            deletedAt: null,
            status: 'published',
        ...
        if (totalLessons > 0) {
          ...
          if (Math.abs(calculated - progress) > 5) {
            ...
            this.enrollmentRepo
              .update(e.id, { completionPercentage: progress })
              .catch(console.error);
          }
        } else {
  ...
  async trackLessonProgress(
    ...
    const enrollment = await this.enrollmentRepo.findByUserAndCourseMaster(
      userId,
      courseMasterId,
    );
    ...
    const watchCompleted = seconds / totalSeconds > 0.9;
    ...
    if (watchCompleted && (lesson as any).requiresPassingGrade) {
      ...
      if (runQuizzes.length > 0) {
        ...
        if (!allPassed) {
          this.logger.warn(
            `User ${userId} tried to complete lesson ${lessonId} without passing all quizzes`,
          );
    ...
    if (totalLessons > 0) {
      const completedCount = await this.progressRepo.countCompletedLessons(
        enrollment.id,
      );
      percentage = Math.min(
        100,
    ...
    if (percentage >= 100) {
      this.certificateService
        .issueCertificate(userId, courseMasterId, enrollment.id)
        .catch((err: any) => {
          this.logger.error(
    ...
    try {
      ...
      if (isCompleted) {
        ...
        this.logger.log(
          `Emitted LESSON_COMPLETE event for user ${userId}, lesson ${lessonId}`,
        );
      }
    } catch (error) {
  ...
  async getUserLearningStats(userId: string) {
    ...
    const completedCourses = enrollments.filter(
      (e: any) => e.completionStatus === EnrollmentStatus.COMPLETED,
    ).length;
    const inProgressCourses = enrollments.filter(
      (e: any) => e.completionStatus === EnrollmentStatus.IN_PROGRESS,
    ).length;

    const enrollmentIds = enrollments.map((e: any) => e.id);
    const totalSecondsWatched =
      await this.progressRepo.getTotalLearningSeconds(enrollmentIds);
    const totalLearningHours =
      Math.round((totalSecondsWatched / 3600) * 10) / 10;

    let averageProgress = 0;
    if (totalCourses > 0) {
      const totalPercentage = enrollments.reduce(
        (acc: number, curr: any) =>
          acc + (Number(curr.completionPercentage) || 0),
        0,
      );
  ...
  async getLearningHistory(userId: string) {
    ...
    return history.map((item: any) => ({
      ...
      expiresAt: item.enrollment.expiresAt
        ? item.enrollment.expiresAt.toISOString()
        : null,
    }));
  }
}
