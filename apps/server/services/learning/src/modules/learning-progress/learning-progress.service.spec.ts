...
describe('LearningProgress & Full E-Learning Flow Simulation', () => {
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: LEARNING_PROGRESS_REPOSITORY_TOKEN, useValue: mockProgressRepo },
    ...
    describe('Full Flow Simulation & Tracking Logic', () => {
        ...
        it('Phase 1: Student sees their enrolled snapshot regardless of live edits', async () => {
            ...
            mockCourseRepo.getVersionById.mockResolvedValue({
                ...
                curriculumSnapshot: [
                    {
                        id: 'mod-1',
                        lessons: [{ id: 'l1' }, { id: 'l2' }, { id: 'l3' }, { id: 'l4' }, { id: 'l5' }] // 5 lessons
                    }
                ]
            });
        ...
        it('Phase 2: Student tracks progress properly within their version limit', async () => {
            ...
            // Mock lesson lookup
            mockLessonRepo.findById.mockResolvedValue({ id: lessonId, moduleId: 'mod-1', requiresPassingGrade: false });
            mockModuleRepo.findById.mockResolvedValue({ id: 'mod-1', courseMasterId: courseMasterId });
            ...
            // Gamification Events
            expect(mockNatsClient.emit).toHaveBeenCalledWith('user.activity', expect.objectContaining({
        ...
        it('Phase 3: Student hits expiration date limit', async () => {
            ...
            await expect(service.trackLessonProgress(studentId, lessonId, 100, 100))
        ...
        it('Phase 4: Course Completion triggers Certificate generation automatically', async () => {
            mockLessonRepo.findById.mockResolvedValue({ id: lessonId, moduleId: 'mod-1', requiresPassingGrade: false });
            mockModuleRepo.findById.mockResolvedValue({ id: 'mod-1', courseMasterId });
            ...
            expect(mockEnrollmentRepo.update).toHaveBeenCalledWith(
                enrollmentId,
                expect.objectContaining({ completionPercentage: 100, completionStatus: EnrollmentStatus.COMPLETED })
            );

            // Ensure certificate was issued
            expect(mockCertificateService.issueCertificate).toHaveBeenCalledWith(studentId, courseMasterId, enrollmentId);
        });

        it('Phase 5: Prevent lesson completion if Quizzes are required but not passed', async () => {
            // Mock lesson requiring passing grade
            mockLessonRepo.findById.mockResolvedValue({ id: lessonId, moduleId: 'mod-1', requiresPassingGrade: true });
            mockModuleRepo.findById.mockResolvedValue({ id: 'mod-1', courseMasterId });
