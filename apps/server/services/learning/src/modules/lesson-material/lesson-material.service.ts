...
import {
  LESSON_MATERIAL_REPOSITORY_TOKEN,
...
export class LessonMaterialService implements ILessonMaterialService {
  ...
  private async checkReadAccess(
    ...
    if (!requester?.sub) {
      throw new ForbiddenException(
        'You must be logged in to access lesson materials',
      );
    }
    ...
    const courseMasterId = (lesson as any).module?.courseMasterId;
    if (!courseMasterId) {
      this.logger.warn(
        `Could not resolve courseMasterId for lesson ${lessonId}`,
      );
      throw new ForbiddenException(
        'Access denied: course information unavailable',
      );
    }
    ...
    if (!enrolled) {
      throw new ForbiddenException(
        'You must be enrolled in this course to access its materials',
      );
    }
  }
  ...
  async uploadMaterial(
    ...
    try {
      ...
      const material = await this.lessonMaterialRepository.create({
        ...
        type: dto.type,
        title:
          dto.title || fileAsset.metadata?.originalName || 'Untitled Material',
        orderIndex: 0, // Will be updated if reordering is implemented
        createdBy: requester.sub,
      });

      this.logger.log(
        `Material uploaded for lesson ${dto.lessonId} by user ${requester.sub}`,
      );

      return this.mapper.map<any, LessonMaterialResponseDTO>(
        material,
        'LessonMaterial',
        'LessonMaterialResponseDTO',
      );
  ...
  async findByLessonId(
    ...
    const materials =
      await this.lessonMaterialRepository.findByLessonId(lessonId);
    return materials.map((material) =>
      this.mapper.map<any, LessonMaterialResponseDTO>(
        material,
        'LessonMaterial',
        'LessonMaterialResponseDTO',
      ),
  ...
  async updateMaterial(
    ...
    try {
      ...
      const updated = await this.lessonMaterialRepository.update(
        materialId,
        updateData,
      );
      return this.mapper.map<any, LessonMaterialResponseDTO>(
  ...
  async deleteMaterial(
    ...
    try {
      ...
      this.logger.log(
        `Material ${materialId} deleted by user ${requester.sub}`,
      );
