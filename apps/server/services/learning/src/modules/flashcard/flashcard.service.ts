...
export class FlashcardService implements IFlashcardService {
  ...
  async generateFlashcardsFromAI(
    userId: string,
    deckId: string,
    topic: string,
    level: string,
    ...
    try {
      ...
      this.logger.log(
        `AI generated ${result.flashcards.length} flashcards for topic ${topic}`,
      );
      ...
      this.logger.error(
        `Error generating flashcards from AI: ${error.message}`,
        error.stack,
      );
      ...
      const flashcard = await this.flashcardRepository.create({
        ...
        difficulty:
          data.difficulty !== undefined
            ? fromDifficultyLevel(data.difficulty)
            : 'medium',
        ...
        sourceDocument: (data as any).sourceDocumentId
          ? { connect: { id: (data as any).sourceDocumentId } }
          : undefined,
        generationMethod:
          (data as any).generationMethod || FlashcardGenerationMethod.MANUAL,
  ...
  async getFlashcards(
    params: FlashcardQueryDTO & { userId: string },
    ...
    try {
      ...
      this.logger.error(
        `Error getting flashcards: ${error.message}`,
        error.stack,
      );
      throw new RpcException({
        status: 400,
  ...
  async updateFlashcard(
    ...
    try {
      ...
      if ((data as any).furigana !== undefined)
        updateData.furigana = (data as any).furigana;
      if ((data as any).kanji !== undefined)
        updateData.kanji = (data as any).kanji;
      if ((data as any).partOfSpeech !== undefined)
        updateData.partOfSpeech = (data as any).partOfSpeech;
      if ((data as any).wordJlptLevel !== undefined)
        updateData.wordJlptLevel = (data as any).wordJlptLevel;
      if ((data as any).meanings !== undefined)
        updateData.meanings = (data as any).meanings;
      if ((data as any).notes !== undefined)
        updateData.notes = (data as any).notes;
      if ((data as any).isArchived !== undefined)
        updateData.isArchived = (data as any).isArchived;
  ...
  async getFlashcardById(id: string): Promise<FlashcardResponseDTO> {
    try {
      ...
      if (error instanceof RpcException) throw error;
      this.logger.error(
        `Error getting flashcard by id: ${error.message}`,
        error.stack,
      );
      throw new RpcException({
        status: 400,
  ...
  async bulkOperations(
    data: BulkFlashcardOperationsDTO & { userId: string },
  ): Promise<BulkFlashcardOperationsResponseDTO> {
    let successCount = 0;
    let failedCount = 0;
