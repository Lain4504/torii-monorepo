    ...
    try {
      ...
      for (const item of expiredItems) {
        try {
          ...
          this.logger.error(
            `Failed to expire enrollment ${item.id}: ${error?.message}`,
          );
        }
      }

      this.logger.log(
        `✅ Auto-expire completed: ${updatedCount}/${expiredItems.length} enrollments marked as EXPIRED`,
      );
    } catch (error: any) {
      this.logger.error(
        `❌ Error in enrollment expiration cronjob: ${error?.message}`,
        error?.stack,
      );
    }
  }
