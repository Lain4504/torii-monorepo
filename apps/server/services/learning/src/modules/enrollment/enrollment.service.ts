...
export class EnrollmentService implements IEnrollmentService {
  ...
  async activateEnrollment(
    ...
    try {
      ...
      this.logger.error(
        `Error activating enrollment: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getLearnerStats(userId: string): Promise<{
    ...
    const completedCourses = enrollments.data.filter(
      (e) => e.completionStatus === EnrollmentStatus.COMPLETED,
    ).length;
    ...
    const totalLearningHours =
      Math.round((totalLearningSeconds / 3600) * 10) / 10; // Precision to 1 decimal point
  ...
  private toEnrollmentDto(e: any): EnrollmentResponseDTO {
    return this.mapper.map<any, EnrollmentResponseDTO>(
      e,
      'Enrollment',
  ...
  async findByUserAndCourseRun(
    ...
    try {
      const item = await this.enrollmentRepository.findByUserAndCourseRun(
        userId,
        courseRunId,
      );
      if (!item) return null;
      ...
      this.logger.error(
        `Error fetching enrollment: ${error.message}`,
        error.stack,
      );
      return null;
    }
   ...
   */
  async findByUserAndCourseMaster(
    userId: string,
    courseMasterId: string,
  ): Promise<EnrollmentResponseDTO | null> {
    try {
      const item = await this.enrollmentRepository.findByUserAndCourseMaster(
        userId,
        courseMasterId,
      );
      if (!item) return null;
      ...
      this.logger.error(
        `Error fetching enrollment by course master: ${error.message}`,
        error.stack,
      );
      return null;
    }
   ...
   */
  async findAllByUserAndCourseMaster(
    userId: string,
    courseMasterId: string,
  ): Promise<EnrollmentResponseDTO[]> {
    try {
      const items =
        await this.enrollmentRepository.findAllByUserAndCourseMaster(
          userId,
          courseMasterId,
        );
      ...
      this.logger.error(
        `Error fetching enrollments by course master: ${error.message}`,
        error.stack,
      );
      return [];
    }
  ...
  async checkEnrollmentDetails(
    ...
    try {
      ...
      this.logger.error(
        `Error checking enrollment details: ${error.message}`,
        error.stack,
      );
      return { isEnrolled: false, enrollment: null, hasNewerVersion: false };
    }
  ...
  async createTrial(
    userId: string,
    input: TrialEnrollmentCreateDTO,
  ): Promise<EnrollmentResponseDTO> {
    ...
    const existing = await this.enrollmentRepository.findByUserAndCourseRun(
      userId,
      courseRunId,
    );
    if (existing) {
      throw new BadRequestException(
        'User already has an enrollment record for this course run',
      );
    }
    ...
    try {
      ...
      const versionId =
        courseRun.versionId ||
        (await this.courseRepository.getLatestVersion(course.id))?.id;
      ...
      this.logger.error(
        `Error creating trial enrollment: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async create(
    ...
    if (existing) {
      if (existing.completionStatus === EnrollmentStatus.PENDING_PAYMENT) {
        this.logger.log(
          `Found existing pending enrollment ${existing.id} for user ${userId} and run ${courseRunId}`,
        );
      ...
      const isMarkedExpired =
        existing.completionStatus === EnrollmentStatus.EXPIRED;
      ...
      this.logger.log(
        `Renewing enrollment ${existing.id} for user ${userId} and run ${courseRunId}`,
      );
    }
    ...
    if (course.type === 'live') {
      if (
        courseRun.enrollmentEnd &&
        new Date() > new Date(courseRun.enrollmentEnd)
      ) {
        throw new BadRequestException('Registration for this class has ended');
      }
      if (
        courseRun.maxStudents &&
        courseRun.totalStudents >= courseRun.maxStudents
      ) {
    ...
    try {
      ...
      const finalPrice =
        courseRun.price !== undefined && courseRun.price !== null
      ...
      if (result.completionStatus === EnrollmentStatus.IN_PROGRESS) {
        try {
          this.logger.log(
            `Fetching user ${userId} for enrollment notification`,
          );
          const response = await lastValueFrom(
            this.natsClient.send(
              { cmd: 'identity.users.findById' },
          ...
          if (user && user.email) {
            this.logger.log(
              `Emitting course_enrollment_success for ${user.email}`,
            );
            this.natsClient.emit(
              { cmd: 'course_enrollment_success' },
              {
            ...
            this.logger.log(
              `course_enrollment_success event emitted for ${existing ? 'renewal' : 'new enrollment'} of course ${course.id}`,
            );
          }
        } catch (error: any) {
          this.logger.error(
            `Failed to emit enrollment success event: ${error.message}`,
          );
        }
      }
  ...
  async checkAccess(
    ...
    if (enrollment.completionStatus === EnrollmentStatus.TRIAL) {
      ...
      const allowedLessons = await this.lessonRepository.findTopLessonsByCourse(
        courseMasterId,
        MAX_TRIAL_LESSONS,
      );
  ...
  async getAccessibleLessonIds(
    ...
    if (enrollment.completionStatus === EnrollmentStatus.TRIAL) {
      ...
      const MAX_TRIAL_LESSONS = 3;
      const lessons = await this.lessonRepository.findTopLessonsByCourse(
        courseMasterId,
        MAX_TRIAL_LESSONS,
      );
  ...
  async updateProgress(
    ...
    try {
      ...
      if (
        ...
        if (masterId) {
          this.certificateService
            .issueCertificate(enrollment.userId, masterId, enrollmentId)
            .catch((err: any) => {
              this.logger.error(
                `Failed to automatically issue certificate: ${err.message}`,
                err.stack,
              );
            });
        }

        return this.toEnrollmentDto({
          ...updated,
          completionStatus: EnrollmentStatus.COMPLETED,
          completedAt: new Date(),
        });
      ...
      this.logger.error(
        `Error updating enrollment progress: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async updateOrderId(
    ...
    try {
      ...
      this.logger.error(
        `Error updating enrollment order ID: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async upgradeVersion(
    ...
    try {
      ...
      this.logger.error(
        `Error upgrading enrollment version: ${error.message}`,
        error.stack,
      );
      throw error;
    }
   ...
   */
  async deleteByUserAndCourseRun(
    userId: string,
    courseRunId: string,
  ): Promise<EnrollmentResponseDTO> {
    try {
      ...
      this.logger.log(
        `Deleted enrollment ${enrollment.id} for user ${userId} and run ${courseRunId}`,
      );
   ...
   */
  private computeEnrollmentExpiry(
    course: any,
    courseRun?: any,
  ): Date | undefined {
    const now = new Date();
    ...
    if (course.type === 'live') {
      if (!courseRun) {
        throw new BadRequestException(
          'Course run ID is required for live courses',
        );
      }
