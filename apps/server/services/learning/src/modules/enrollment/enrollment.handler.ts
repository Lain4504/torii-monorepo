...
export class EnrollmentHandler {
  ...
  async checkEnrollment(
    @Payload() data: { userId: string; courseRunId: string },
  ) {
    return this.enrollmentService.checkEnrollmentDetails(
      data.userId,
      data.courseRunId,
    );
  }
  ...
  async createTrial(@Payload() data: { userId: string; courseRunId: string }) {
    return this.enrollmentService.createTrial(data.userId, {
  ...
  async updateProgress(
    @Payload() data: { id: string; completionPercentage: number },
  ) {
  ...
  async checkAccess(
    ...
    return this.enrollmentService.checkAccess(
      data.userId,
      data.courseMasterId,
      data.lessonId,
    );
  ...
  async getAccessibleLessonIds(
    @Payload() data: { userId: string; courseMasterId: string },
  ) {
    return this.enrollmentService.getAccessibleLessonIds(
      data.userId,
      data.courseMasterId,
    );
  }
  ...
  async delete(@Payload() data: { userId: string; courseRunId: string }) {
    return this.enrollmentService.deleteByUserAndCourseRun(
      data.userId,
      data.courseRunId,
    );
  }
  ...
  async upgradeVersion(
    ...
    return this.enrollmentService.upgradeVersion(
      data.userId,
      data.courseMasterId,
    );
  }
  ...
  async findByUserAdmin(
    @Payload() data: { userId: string; query?: EnrollmentQueryDTO },
  ) {
    ...
    return this.enrollmentService.findAll({
      ...baseQuery,
      userId,
    } as EnrollmentQueryDTO & { userId: string });
  }
}
