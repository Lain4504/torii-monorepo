...
describe('CourseMasterService', () => {
  ...
  describe('Drafting & Review flow', () => {
    ...
    it('should correctly initialize a CourseMaster as DRAFT upon creation', async () => {
      ...
      mockCourseMasterRepo.create.mockResolvedValue({
        id: 'cm-1',
        title: 'Test VOD',
        type: 'vod',
        status: CourseMasterStatus.DRAFT,
      });
      mockMapper.map = jest.fn().mockResolvedValue({
        id: 'cm-1',
        title: 'Test VOD',
        status: CourseMasterStatus.DRAFT,
      });
      ...
      expect(mockCourseMasterRepo.create).toHaveBeenCalledWith(
    ...
    it('should forbid lecturers from creating CourseMasters', async () => {
      await expect(
        service.create(lecturerRequester, { title: 'Test VOD' } as any),
      ).rejects.toThrow('Lecturers cannot create courses');
    });
    ...
    it('should transition DRAFT to PENDING_REVIEW on submitForSyllabusReview', async () => {
      ...
      mockCourseMasterRepo.update.mockResolvedValue({
        ...courseMaster,
      ...
      const result = await service.submitForSyllabusReview(
        adminRequester,
        'cm-1',
      );

      expect(mockCourseMasterRepo.update).toHaveBeenCalledWith('cm-1', {
        status: CourseMasterStatus.PENDING_REVIEW,
      });
    ...
    it('should transition PENDING_REVIEW back to CHANGES_REQUIRED on rejection', async () => {
      ...
      mockCourseMasterRepo.update.mockResolvedValue({
        ...courseMaster,
        status: CourseMasterStatus.CHANGES_REQUIRED,
      });
      mockCourseMasterRepo.getLatestMasterReview = jest
        .fn()
  ...
  describe('Clone-on-Edit versioning', () => {
    ...
    it('should trigger version publishing and VOD sync when APPROVED', async () => {
      ...
      mockCourseMasterRepo.update.mockResolvedValue({
        ...courseMaster,
        status: CourseMasterStatus.APPROVED,
      });
      mockCourseMasterRepo.getLatestMasterReview = jest
        .fn()
        .mockResolvedValue(null);
      mockMapper.map = jest
        .fn()
        .mockReturnValue({ id: 'cm-1', status: CourseMasterStatus.APPROVED });
      ...
      mockPrismaService.courseVersion = {
        updateMany: jest.fn(),
        update: jest.fn(),
      };
      mockPrismaService.courseRun = { updateMany: jest.fn() };

      await service.reviewSyllabus(adminRequester, 'cm-1', {
        outcome: 'APPROVED',
      });
    ...
    it('should create Clone-on-Edit deep copy Draft when triggering createDraftFromLatest', async () => {
      ...
      mockCourseMasterRepo.createVersion.mockResolvedValue({
        id: 'draft-v2',
        status: 'DRAFT',
        versionTag: 'v2.0',
      });
      ...
      mockModuleRepo.findByVersionId.mockResolvedValue([
        { id: 'mod-clone-source', title: 'Intro' },
      ]);
      mockModuleRepo.create.mockResolvedValue({ id: 'mod-clone-dest' });
      mockModuleItemRepo.findByModuleId.mockResolvedValue([
        { id: 'item-clone-source', title: 'L1' },
      ]);

      const newDraft = await service.createDraftFromLatest(
        adminRequester,
        'cm-1',
      );
  ...
  describe('Core Mechanics', () => {
    ...
    it('should findAll courses with pagination and filters', async () => {
      ...
      expect(mockCourseMasterRepo.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          skip: 0,
          take: 10,
    ...
    it('should findById and attach lecturer', async () => {
      ...
      mockCourseMasterRepo.getLecturer = jest
        .fn()
        .mockResolvedValue({ id: 'lecturer-1' });
      mockMapper.map = jest.fn().mockReturnValue({ id: 'cm-1' });
    ...
    it('should update course attributes and generate new slug if title changes', async () => {
      ...
      mockMapper.map = jest
        .fn()
        .mockReturnValue({ id: 'cm-1', title: 'New Title' });

      const result = await service.update(adminRequester, 'cm-1', {
        title: 'New Title',
      });

      expect(mockCourseMasterRepo.slugExists).toHaveBeenCalled();
      expect(mockCourseMasterRepo.update).toHaveBeenCalledWith(
        'cm-1',
        expect.objectContaining({
          title: 'New Title',
    ...
    it('should getCurriculum from snapshot if student is enrolled', async () => {
      ...
      mockEnrollmentService.findByUserAndCourseMaster = jest
        .fn()
        .mockResolvedValue({ versionId: 'ver-1' });
      mockCourseMasterRepo.getVersionById = jest.fn().mockResolvedValue({
        curriculumSnapshot: [
          {
            id: 'mod-1',
    ...
    it('should getCurriculum from draft tables if instructor/admin', async () => {
      ...
      mockModuleRepo.findByCourseId.mockResolvedValue([
        { id: 'mod-1', title: 'M1', orderIndex: 1 },
      ]);
      mockLessonRepo.findByModuleId.mockResolvedValue([
        {
          id: 'ls-1',
          title: 'L1',
          orderIndex: 1,
    ...
    it('should return getVersionHistory successfully', async () => {
      mockCourseMasterRepo.getVersions = jest
        .fn()
        .mockResolvedValue([
