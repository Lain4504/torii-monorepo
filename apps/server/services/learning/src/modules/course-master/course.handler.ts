...
export class CourseHandler {
  ...
  async create(
    @Payload() data: CourseMasterCreateDTO & { requester: Requester },
  ) {
  ...
  async update(
    @Payload()
    data: CourseMasterUpdateDTO & { id: string; requester: Requester },
  ) {
  ...
  async submitSyllabusReview(
    @Payload() data: { id: string; requester: Requester },
  ) {
    const { id, requester } = data;
  ...
  async updateLiveConfig(
    @Payload() data: { id: string; config: any; requester: Requester },
  ) {
    return this.courseMasterService.updateLiveConfig(
      data.requester,
      data.id,
      data.config,
    );
  ...
  async reject(
    @Payload() data: { id: string; reason: string; requester: Requester },
  ) {
  ...
  @MessagePattern({ cmd: 'learning.courseMaster.recalculate_stats' })
  async recalculate_stats(
    @Payload() data: { id?: string; courseMasterId?: string },
  ) {
