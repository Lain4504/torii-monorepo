...
import {
  EXAM_SERVICE_TOKEN,
  IExamService,
} from '@server/learning/interfaces/services/i-exam.service';
import {
  ExamQueryDTO,
  ExamCreateDTO,
  ExamUpdateDTO,
...
export class ExamHandler {
  ...
  async findAllWithStatus(
    @Payload() data: { query: ExamQueryDTO; requester: Requester },
  ) {
  ...
  async getUserSessions(
    @Payload() data: { requester: Requester; query: ExamSessionQueryDTO },
  ) {
  ...
  async saveAnswers(
    ...
    data: {
      sessionId: string;
      requester: Requester;
      dto: ExamSessionAnswersDTO;
    },
  ) {
    return this.examService.saveAnswers(
      data.sessionId,
      data.requester.sub,
      data.dto,
    );
  ...
  async submitSession(
    @Payload() data: { sessionId: string; requester: Requester },
  ) {
  ...
  async getAttemptDetails(
    ...
    return this.examService.getAttemptDetails(
      data.sessionId,
      data.requester.sub,
    );
  }
  ...
  async getExamById(
    @Payload() data: { examId: string; requester?: Requester },
  ) {
  ...
  async getExamSessions(
    ...
    return this.examService.getExamSessions(
      data.examId,
      data.requester.sub,
      data.query,
    );
  ...
  async updateAdmin(
    @Payload() data: ExamUpdateDTO & { id: string; requester: Requester },
  ) {
  ...
  async getQuizAttemptsAdmin(
    @Payload() data: { id: string; query: ExamSessionQueryDTO },
  ) {
