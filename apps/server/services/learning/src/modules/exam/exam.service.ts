...
export class ExamService implements IExamService {
  ...
  async findAllWithStatus(
    ...
    try {
      ...
      this.logger.log(
        `Fetching quizzes (exams) with filters: ${JSON.stringify(whereClause)}`,
      );
      ...
      this.logger.log(
        `Found ${total} quizzes, returning ${quizzes.length} quizzes`,
      );
        ...
        data: quizzes.map((q) => {
          ...
          return {
            ...
            sessionStatus: attempt
              ? (attempt.status as ExamSessionStatus)
              : undefined,
            sessionId: attempt?.id,
            courseRunId: q.courseRunId ?? undefined,
            score:
              attempt?.status === ExamSessionStatus.SUBMITTED
                ? this.calculateScore(attempt)
                : undefined,
            maxScore: q.totalQuestions,
            progress: attempt
              ? this.calculateProgress(attempt, q.totalQuestions)
              : undefined,
      ...
      this.logger.error(
        `Error fetching exams with status: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async startExam(
    ...
    try {
      ...
      const existingAttemptsCount = await this.examRepository.countAttempts({
        ...
        courseRunId: courseRunId || null,
        status: {
          in: [ExamSessionStatus.SUBMITTED, ExamSessionStatus.COMPLETED],
        },
      });
      ...
      this.logger.error(
        `Error starting exam ${examId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async saveAnswers(
    ...
    try {
      ...
      if (attempt.status !== ExamSessionStatus.IN_PROGRESS) {
        ...
        this.logger.warn(
          `Attempt ${sessionId} is already ${attempt.status}, ignoring save request`,
        );
        return this.mapper.map<any, ExamSessionResponseDTO>(
          attempt,
          'QuizAttempt',
      ...
      const quiz =
        attempt.quiz || (await this.examRepository.findById(attempt.quizId));

      // Check time limit
      const dataWithTime = data as ExamSessionAnswersDTO & {
        timeRemaining?: number;
      };
      let timeRemaining =
        dataWithTime.timeRemaining !== undefined
          ? dataWithTime.timeRemaining
      ...
      const updated = await this.examRepository.updateAttempt(sessionId, {
        ...
        flaggedQuestions:
          data.flaggedQuestions !== undefined
      ...
      this.logger.error(
        `Error saving answers for session ${sessionId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async submitSession(
    ...
    try {
      ...
      if (
        attempt.status === ExamSessionStatus.SUBMITTED ||
        attempt.status === ExamSessionStatus.COMPLETED
      ) {
        // Already submitted, return existing attempt
        this.logger.warn(
          `Attempt ${sessionId} already submitted, returning existing attempt`,
        );
        return this.mapper.map<any, ExamSessionResponseDTO>(
          attempt,
          'QuizAttempt',
      ...
      if (attempt.status !== ExamSessionStatus.IN_PROGRESS) {
        throw new BadRequestException(
          `Session is not in progress. Current status: ${attempt.status}`,
        );
      }
      ...
      if (isPassed && quiz) {
        this.handleQuizCompletion(
          ...
          this.logger.error(
            `Failed to handle quiz completion for user ${userId}: ${e.message}`,
            e.stack,
          );
        });
      }
      ...
      this.logger.error(
        `Error submitting session ${sessionId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  private async handleQuizCompletion(
    ...
    if (quizType === 'placement') {
      ...
      this.logger.log(
        `Placement result created for user ${userId}: ${recommendedLevel}`,
      );
      return;
    }
    ...
    if (!runId) {
      this.logger.warn(
        `Quiz ${quiz.id} has no courseRunId, skipping completion gating`,
      );
      return;
    }
    ...
    if (!enrollment) {
      this.logger.warn(
        `No enrollment for user ${userId} in courseRun ${runId}, skipping completion gating`,
      );
      return;
    }
    ...
    if (quizType === 'lesson' && quiz.lessonId) {
      ...
      this.logger.log(
        `LessonProgress updated: lesson ${quiz.lessonId} completed for enrollment ${enrollment.id}`,
      );
      ...
      this.logger.log(
        `Enrollment ${enrollment.id} marked completed via final course exam`,
      );
    }
  ...
  private async updateEnrollmentProgress(
    ...
    if (progressPercentage >= 100) {
      ...
      this.logger.log(
        `Enrollment ${enrollmentId} auto-completed: ${completedLessons}/${totalLessons} lessons done`,
      );
    } else {
      await this.prisma.enrollment.update({
        where: { id: enrollmentId },
  ...
  private async gradeAttempt(
    ...
    const quizQuestions = await this.examRepository.findQuizQuestions(
      attempt.quizId,
    );
    ...
    for (const questionId of questionIds) {
      ...
      if (!question) {
        this.logger.warn(
          `Question ${questionId} not found for attempt ${attempt.id}`,
        );
        continue;
      }
  ...
  private async generateExamQuestions(quiz: any): Promise<any[]> {
    ...
    const frozenQuestions = await this.examRepository.findQuizQuestions(
      quiz.id,
    );
    ...
    if (frozenQuestions.length > 0) {
      ...
      const questionDetails =
        await this.examRepository.findQuestionsByIds(questionIds);
      ...
      let questions = frozenQuestions
        ...
        .map((qq: any) => {
          ...
          return {
            ...
            options: q.options,
            audioUrl:
              qq.sectionType === ExamSectionType.LISTENING
                ? options?.audioUrl
                : undefined,
    ...
    this.logger.warn(
      `Quiz ${quiz.id} has no frozen QuizQuestions. Generating dynamically (draft mode preview).`,
    );
    ...
    for (const section of sections) {
      ...
      if (section.questionIds && section.questionIds.length > 0) {
        const allQuestions = await this.examRepository.findQuestionsByIds(
          section.questionIds,
        );
    ...
    if (attempt.startedAt) {
      ...
      if (Math.abs(calculatedTimeRemaining - storedTimeRemaining) > 5) {
        ...
        this.examRepository
          ...
          .catch((err) => {
            this.logger.warn(
              `Failed to update timeRemaining for attempt ${attempt.id}: ${err.message}`,
            );
          });
      }
    }
  ...
  async getUserSessions(
    ...
    try {
      const { page = 1, limit = 10, status, examId } = query;
      ...
      return {
        data: attempts.map((a: any) => {
          const sessionDto = this.mapper.map<any, ExamSessionResponseDTO>(
            a,
            'QuizAttempt',
            'ExamSessionResponseDTO',
          );
          ...
          const score =
            a.status === ExamSessionStatus.SUBMITTED ||
            a.status === ExamSessionStatus.COMPLETED
              ? a.score
                ? Number(a.score)
                : undefined
              : undefined;
  ...
  private checkPermission(requester: Requester, action: string): void {
    const hasPermission =
      ...
      requester.permissions?.includes('exam.manage');
    if (!hasPermission) {
  ...
  async create(
    ...
    try {
      ...
      const totalQuestions =
        sections.length > 0 ? this.calculateTotalQuestions(sections) : 0;
  ...
  async update(
    ...
    try {
      ...
      if ((dto as any).courseRunId !== undefined)
        updateData.courseRunId = (dto as any).courseRunId;
      ...
      this.logger.error(
        `Error updating exam ${examId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async delete(requester: Requester, examId: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Error deleting exam ${examId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async findById(examId: string): Promise<ExamResponseDTO> {
    try {
      ...
      this.logger.error(
        `Error fetching exam ${examId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
    ...
    try {
      ...
      for (const section of sections) {
        ...
        if (section.questionIds && section.questionIds.length > 0) {
          ...
          sectionQuestions = await this.examRepository.findQuestionsByIds(
            section.questionIds,
          );
          sectionQuestions = sectionQuestions.slice(
            0,
            section.questionCount || sectionQuestions.length,
          );
      ...
      if (quizQuestionRows.length === 0) {
        throw new BadRequestException(
          'No questions could be selected for this exam. Check that pools have active questions.',
        );
      }
      ...
      this.logger.log(
        `Exam ${examId} published by ${requester.sub}: ${quizQuestionRows.length} questions frozen`,
      );
      return this.buildExamResponseDTO(updated);
    } catch (error: any) {
      this.logger.error(
        `Error publishing exam ${examId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getQuizStatistics(examId: string): Promise<any> {
    try {
      ...
      const averageTimeMinutes =
        timeTaken.length > 0
          ? timeTaken.reduce((sum, time) => sum + time, 0) /
            timeTaken.length /
            60
          : 0;
      ...
      this.logger.error(
        `Error fetching quiz statistics for ${examId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getQuizAttempts(
    ...
    try {
      ...
      this.logger.error(
        `Error fetching quiz attempts for ${examId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getExamById(
    ...
    try {
      ...
      return {
        ...examDto,
        sessionStatus: userAttempt
          ? (userAttempt.status as ExamSessionStatus)
          : undefined,
        sessionId: userAttempt?.id,
        score:
          userAttempt?.status === ExamSessionStatus.SUBMITTED ||
          userAttempt?.status === ExamSessionStatus.COMPLETED
            ? this.calculateScore(userAttempt)
            : undefined,
        maxScore: quiz.totalQuestions,
        progress: userAttempt
          ? this.calculateProgress(userAttempt, quiz.totalQuestions)
          : undefined,
  ...
  async getExamSessions(
    ...
    try {
      ...
      this.logger.error(
        `Error fetching exam sessions for ${examId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getAttemptDetails(sessionId: string, userId: string): Promise<any> {
    try {
      ...
      const detailsData =
        await this.examRepository.findAttemptDetails(sessionId);
      ...
      this.logger.error(
        `Error fetching attempt details for ${sessionId}: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async addQuestionsFromPool(
    ...
    const existingQuizQuestions =
      await this.examRepository.findQuizQuestions(quizId);
    const existingQuestionIds = existingQuizQuestions.map(
      (qq) => qq.questionId,
    );

    const poolQuestions = await this.examRepository.findQuestionsByPool(
      poolId,
      limit,
      undefined,
      existingQuestionIds,
    ...
    if (poolQuestions.length === 0) {
      throw new BadRequestException(
        'No new questions found in the specified pool',
      );
    }

    let orderIndex =
      (await this.examRepository.getMaxQuizQuestionOrder(quizId)) + 1;
    ...
    this.logger.log(
      `Added ${poolQuestions.length} questions from pool ${poolId} to quiz ${quizId}`,
    );
  }
  ...
  async addBulkQuestions(
    ...
    const existingIds = new Set(
      existingQuizQuestions.map((qq) => qq.questionId),
    );
    ...
    this.logger.log(
      `Added ${newQuestionIds.length} questions to quiz ${quizId}`,
    );
  }
}
