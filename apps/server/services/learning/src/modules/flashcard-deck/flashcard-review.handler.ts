...
import {
  IFlashcardReviewService,
  FLASHCARD_REVIEW_SERVICE_TOKEN,
} from '@server/learning/interfaces/services/i-flashcard-review.service';
import {
  IFlashcardReviewSessionService,
  FLASHCARD_REVIEW_SESSION_SERVICE_TOKEN,
} from '@server/learning/interfaces/services/i-flashcard-review-session.service';
...
export class FlashcardReviewHandler {
  ...
  async getCardsDue(
    @Payload() data: { userId: string; query: GetCardsDueDTO },
  ) {
  ...
  async getUserProgress(
    @Payload() data: { userId: string; flashcardId: string },
  ) {
    return this.reviewService.getUserProgress(data.userId, {
  ...
  async startSession(
    @Payload() data: StartReviewSessionDTO & { userId: string },
  ) {
    const { userId, ...body } = data;
  ...
  async completeSession(
    @Payload() data: { sessionId: string; body: any; userId: string },
  ) {
    // Map durationSeconds from body if it exists
    const durationSeconds = data.body?.durationSeconds;
    return this.sessionService.completeSession(data.sessionId, data.userId, {
      durationSeconds,
    });
  }
  ...
  async getRecentSessions(
    @Payload() data: { userId: string; deckId?: string; limit?: number },
  ) {
