...
export class CertificateService implements ICertificateService {
  ...
  private toCertificateDto(c: Certificate): CertificateResponseDTO {
    return this.mapper.map<Certificate, CertificateResponseDTO>(
      c,
      'Certificate',
  ...
  ): Promise<CertificatePaginatedResponse> {
  ...
  async issueCertificate(
    ...
    try {
      ...
      const existing =
        await this.certificateRepository.findByEnrollmentId(enrollmentId);
      if (existing) {
      ...
      const course = await this.courseMasterRepository.findById(
        run.courseMasterId,
      );
      ...
      try {
        ...
        this.logger.warn(
          `Could not fetch user info for certificate: ${userId}`,
        );
      }
      ...
      page.drawText('This is to certify that', {
        x:
          (width -
            fontRegular.widthOfTextAtSize('This is to certify that', 14)) /
          2,
        y: height - 180,
        size: 14,
        font: fontRegular,
      ...
      page.drawText('Instructor', {
        x:
          120 +
          (sigLineWidth - fontItalic.widthOfTextAtSize('Instructor', 10)) / 2,
        y: sigY - 20,
        size: 10,
        font: fontItalic,
        color: navy,
      ...
      page.drawText('Director of Education', {
        x:
          ...
          120 +
          (sigLineWidth -
            fontItalic.widthOfTextAtSize('Director of Education', 10)) /
            2,
        y: sigY - 20,
        size: 10,
        font: fontItalic,
      ...
      this.logger.log(
        `Certificate issued: ${certificateCode} for user ${userId}`,
      );
      return this.toCertificateDto(created);
    } catch (error: any) {
      this.logger.error(
        `Error in issueCertificate: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async verifyCertificate(
    code: string,
  ): Promise<{ valid: boolean; certificate?: CertificateResponseDTO }> {
    const cert = await this.certificateRepository.findByCode(code);
