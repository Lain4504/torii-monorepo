...
describe('ModuleService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a module successfully and update course stats', async () => {
      ...
      expect(courseMasterService.recalculateStats).toHaveBeenCalledWith(
        'course-1',
      );
    ...
    it('should handle errors during creation', async () => {
      mockModuleRepository.getMaxOrderIndex.mockRejectedValue(
        new Error('DB Error'),
      );
  ...
  describe('update', () => {
    ...
    it('should update module successfully if instructor is assigned', async () => {
      ...
      expect(courseMasterService.isInstructor).toHaveBeenCalledWith(
        'inst-1',
        'course-1',
      );
    });
    ...
    it('should throw ForbiddenException if user is not assigned to course', async () => {
      ...
      await expect(
        service.update(requester as any, moduleId, updateDto),
    ...
    it('should throw ForbiddenException if user lacks "module.update" permission', async () => {
      ...
      await expect(
        service.update(weakRequester as any, moduleId, updateDto),
      ).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should throw ForbiddenException if user lacks delete permission', async () => {
      ...
      await expect(
        service.delete(weakRequester as any, moduleId),
      ).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('reorder', () => {
    ...
    it('should throw ForbiddenException if user lacks update permission', async () => {
      ...
      await expect(
        service.reorder(weakRequester as any, courseId, orders),
      ).rejects.toThrow(ForbiddenException);
    });
  });
