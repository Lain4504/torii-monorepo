...
describe('EnrollmentService', () => {
  ...
  describe('getLearnerStats', () => {
    it('should calculate stats correctly using watched duration from repository', async () => {
      ...
      mockEnrollmentRepository.countTotalLearningSeconds.mockResolvedValue(
        9000,
      );
      ...
      expect(
        enrollmentRepository.countTotalLearningSeconds,
  ...
  describe('create', () => {
    ...
    it('should create a PENDING_PAYMENT enrollment for paid course', async () => {
      ...
      expect(natsClient.emit).not.toHaveBeenCalledWith(
    ...
    it('should emit event for free course enrollment if finalPrice is 0', async () => {
      ...
      mockNatsClient.send.mockReturnValue(
        of({ user: { email: 'test@example.com', displayName: 'Test User' } }),
      );
    ...
    it('should calculate expiresAt for VOD course with expirationMonths', async () => {
      ...
      mockEnrollmentRepository.create.mockImplementation((data) =>
      ...
      expect(
        Math.abs(
          new Date((result as any).expiresAt).getTime() -
    ...
    it('should allow renewal of an EXPIRED enrollment', async () => {
      ...
      // Note: The service currently has a redundant create call at line 294
      // even if the enrollment exists. This should be fixed in the service code.
  ...
  describe('activateEnrollment', () => {
    ...
    it('should activate a pending enrollment successfully', async () => {
      ...
      expect(result.completionStatus).toBe(
        (EnrollmentStatus as any).IN_PROGRESS,
      );
    ...
    it('should throw NotFoundException if enrollment not found', async () => {
      mockEnrollmentRepository.findById.mockResolvedValue(null);
      await expect(service.activateEnrollment('invalid-id')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('updateProgress', () => {
    ...
    it('should throw BadRequestException for invalid percentage', async () => {
      await expect(service.updateProgress('enr-1', -1)).rejects.toThrow(
        BadRequestException,
      );
      await expect(service.updateProgress('enr-1', 101)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('deleteByUserAndCourse', () => {
    ...
    it('should throw NotFoundException if enrollment not found', async () => {
      mockEnrollmentRepository.findByUserAndCourse.mockResolvedValue(null);
      await expect(
        service.deleteByUserAndCourse('user-1', 'course-1'),
