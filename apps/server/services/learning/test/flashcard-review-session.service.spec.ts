...
describe('FlashcardReviewSessionService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<FlashcardReviewSessionService>(
      FlashcardReviewSessionService,
    );
  ...
  describe('completeSession', () => {
      ...
      mockReviewRepository.findReviews.mockResolvedValue([
        {
          ...
          timeSpent: 10,
          previousState: FlashcardState.NEW,
        },
        {
          quality: ReviewQuality.ZERO,
          timeSpent: 20,
          previousState: FlashcardState.LEARNING,
        },
      ]);
      mockReviewRepository.updateSession.mockResolvedValue({
        ...mockSession,
        completedAt: new Date(),
      });

      const result = await service.completeSession('sess-1', 'user-1', {
        durationSeconds: 30,
      });
    ...
    it('should throw RpcException if session not found', async () => {
      ...
      await expect(
        service.completeSession('sess-1', 'user-1', {}),
      ).rejects.toThrow(RpcException);
    });

    it('should throw RpcException if session not owned by user', async () => {
      mockReviewRepository.findSessionById.mockResolvedValue({
        ...mockSession,
        userId: 'other',
      });
      await expect(
