...
describe('QuestionService', () => {
    ...
    describe('create', () => {
        ...
        it('should throw BadRequestException if options missing for multiple choice', async () => {
            await expect(service.create(mockRequester as any, {
                questionText: 'Q',
                questionType: QuestionType.MULTIPLE_CHOICE,
                options: {},
                correctAnswer: 'a',
        ...
        it('should throw ForbiddenException if missing permission', async () => {
            ...
            await expect(service.create(requester as any, {} as any))
    ...
    describe('delete', () => {
        ...
        it('should throw BadRequestException if question in use', async () => {
            mockQuestionRepository.findById.mockResolvedValue({ ...mockQuestion, usageCount: 1 });
            await expect(service.delete(mockRequester as any, 'q-1')).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('update', () => {
        it('should update question successfully', async () => {
            mockQuestionRepository.findById.mockResolvedValue(mockQuestion);
            mockQuestionRepository.update.mockResolvedValue({ ...mockQuestion, questionText: 'New' });

            const result = await service.update(mockRequester as any, 'q-1', { questionText: 'New' });
