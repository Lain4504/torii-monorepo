...
describe('AssignmentService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw ForbiddenException if user has no permission', async () => {
      ...
      await expect(
        service.create(lowPrivRequester as any, dto as any),
    ...
    it('should throw BadRequestException on repository error', async () => {
      ...
      await expect(
        service.create(mockRequester as any, dto as any),
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException if assignment not found', async () => {
      ...
      await expect(
        service.update(mockRequester as any, 'non-existent', updateDto as any),
    ...
    it('should throw ForbiddenException if user is not the creator', async () => {
      ...
      await expect(
        service.update(mockRequester as any, 'assignment-1', updateDto as any),
      ).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('publish', () => {
    ...
    it('should throw BadRequestException if already published', async () => {
      ...
      await expect(
        service.publish(mockRequester as any, 'assignment-1'),
      ).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('findAll', () => {
    it('should return paginated assignments with user submission status', async () => {
      ...
      mockSubmissionRepository.findMany.mockResolvedValue([
        { assignmentId: 'assignment-1', status: 'SUBMITTED' },
      ]);
  ...
  describe('delete', () => {
    ...
    it('should throw BadRequestException if submissions exist', async () => {
      ...
      await expect(
        service.delete(mockRequester as any, 'assignment-1'),
      ).rejects.toThrow(BadRequestException);
    });
  });
});
