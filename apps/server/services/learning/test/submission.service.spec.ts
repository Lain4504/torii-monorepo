...
import { UserRole, SubmissionStatus, AssignmentStatus, AssignmentType } from '@workspace/schemas';
...
describe('SubmissionService', () => {
    ...
    describe('saveDraft', () => {
        it('should create a new draft if none exists', async () => {
            ...
            mockSubmissionRepository.create.mockResolvedValue({ ...mockSubmission, status: SubmissionStatus.DRAFT });

            const result = await service.saveDraft(mockRequester as any, '00000000-0000-0000-0000-000000000000', {
                textAnswer: 'Draft',
                fileUrls: []
            });
    ...
    describe('submit', () => {
        it('should submit assignment officially', async () => {
            ...
            const result = await service.submit(mockRequester as any, '00000000-0000-0000-0000-000000000000', {
                textAnswer: 'Final Answer',
                fileUrls: []
            });

            expect(result.status).toBe(SubmissionStatus.SUBMITTED);
            expect(natsClient.emit).toHaveBeenCalledWith('submission.submitted', expect.any(Object));
        });

        it('should throw BadRequestException if text answer missing for TEXT assignment', async () => {
            mockAssignmentRepository.findById.mockResolvedValue(mockAssignment);
            await expect(service.submit(mockRequester as any, '00000000-0000-0000-0000-000000000000', {
                textAnswer: '',
                fileUrls: []
            }))
    ...
    describe('grade', () => {
        it('should grade submission if requester has permission', async () => {
            ...
            mockAssignmentRepository.findById.mockResolvedValue(mockAssignment);
            mockSubmissionRepository.update.mockResolvedValue({ ...mockSubmission, status: SubmissionStatus.GRADED, score: 80 });

            const result = await service.grade(mockStaffRequester as any, 's-1', { score: 80, feedback: 'Good' });

            expect(result.status).toBe(SubmissionStatus.GRADED);
            expect(result.score).toBe(80);
            expect(natsClient.emit).toHaveBeenCalledWith('submission.graded', expect.any(Object));
        });

        it('should throw ForbiddenException if requester not instructor', async () => {
            mockSubmissionRepository.findById.mockResolvedValue(mockSubmission);
            mockAssignmentRepository.findById.mockResolvedValue(mockAssignment);
            await expect(service.grade(mockRequester as any, 's-1', { score: 80 }))
