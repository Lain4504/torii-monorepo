...
describe('TeachingScheduleService', () => {
  ...
  describe('assignSchedule', () => {
    it('should assign schedule if available', async () => {
      ...
      mockPrismaService.teachingSchedule.findUnique.mockResolvedValue(
        mockSchedule,
      ); // For generation
    ...
    it('should throw ConflictException if unavailable', async () => {
      ...
      await expect(
        service.assignSchedule(mockRequester as any, {
          courseMasterId: 'cm1',
          lecturerId: 'l-1',
          dayOfWeek: 1,
          startTime: '09:00',
  ...
  describe('removeSchedule', () => {
    it('should remove schedule and future sessions', async () => {
      ...
      expect(prisma.teachingSchedule.delete).toHaveBeenCalledWith({
        where: { id: 'ts-1' },
      });
    });
  });
