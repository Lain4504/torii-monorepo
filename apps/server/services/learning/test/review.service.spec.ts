...
describe('ReviewService', () => {
  ...
  describe('findByCourseId', () => {
    it('should return reviews for a specific course', async () => {
      ...
      mockReviewRepository.findManyByCourseId.mockResolvedValue(
        expectedReviews,
      );
      ...
      expect(repository.findManyByCourseId).toHaveBeenCalledWith(
  ...
  describe('getRatingDistribution', () => {
    ...
    it('should return empty distribution if error occurs', async () => {
      mockReviewRepository.findAllByCourseId.mockRejectedValue(
        new Error('DB Error'),
      );
  ...
  describe('create', () => {
    ...
    it('should throw if user not enrolled', async () => {
      ...
      await expect(service.create(userId, courseId, createDto)).rejects.toThrow(
        RpcException,
      ); // 403
    });
    ...
    it('should throw if user already reviewed', async () => {
      mockReviewRepository.findCourse.mockResolvedValue({ id: courseId });
      mockReviewRepository.findByUserAndCourse.mockResolvedValue({
        id: 'existing',
      });
