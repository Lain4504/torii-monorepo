...
describe('LessonMaterialService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        LessonMaterialService,
        {
          ...
          useValue: mockLessonMaterialRepository,
        },
    ...
    mockLessonMaterialRepository.findByLessonId.mockResolvedValue([
      mockMaterial,
    ]);
  });
  ...
  describe('findByLessonId - Content Protection', () => {
    ...
    it('Bài không phải preview, không có userId: nên ném ForbiddenException', async () => {
      ...
      await expect(service.findByLessonId('les-1', undefined)).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('Bài không phải preview, người dùng chưa enrolled: nên ném ForbiddenException', async () => {
      ...
      await expect(service.findByLessonId('les-1', requester)).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('Bài không phải preview, người dùng đã enrolled: nên trả về danh sách tài liệu', async () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0].id).toBe('mat-1');
      expect(result[0].fileAsset.fileUrl).toBe(
        'https://cdn.example.com/slide.pdf',
      );
    });
    ...
    it('Lesson không tồn tại (null): nên ném NotFoundException', async () => {
      ...
      await expect(
        service.findByLessonId('non-existent', requester),
