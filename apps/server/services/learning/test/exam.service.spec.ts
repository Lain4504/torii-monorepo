...
import { ExamStatus, ExamSessionStatus, UserRole, ExamType, ExamSectionType } from '@workspace/schemas';
...
describe('ExamService', () => {
    ...
    const mockQuiz = {
        ...
        totalQuestions: 10,
        maxAttempts: 3,
        sections: [{ type: 'READING', questionCount: 5, questionIds: ['q1', 'q2'] }],
        totalTime: 60,
    };
    ...
    describe('findAllWithStatus', () => {
        it('should return exams with session status', async () => {
            ...
            const result = await service.findAllWithStatus({ page: 1, limit: 10 }, 'user-1');
    ...
    describe('startExam', () => {
        it('should start a new exam session', async () => {
            ...
            mockExamRepository.findQuestionsByIds.mockResolvedValue([{ id: 'q1', questionText: 'Q1' }]);
        ...
        it('should throw NotFoundException if exam not found', async () => {
            ...
            await expect(service.startExam('exam-1', 'user-1')).rejects.toThrow(NotFoundException);
        });
    ...
    describe('saveAnswers', () => {
        it('should save answers successfully', async () => {
            ...
            mockExamRepository.updateAttempt.mockResolvedValue({ ...mockAttempt, answers: { q1: 'A' } });

            const result = await service.saveAnswers('session-1', 'user-1', { answers: { q1: 'A' } });
    ...
    describe('submitSession', () => {
        it('should submit and grade session', async () => {
            ...
            mockExamRepository.findQuestionsByIds.mockResolvedValue([{ id: 'q1', correctAnswer: 'A', questionType: 'multiple_choice' }]);
            mockExamRepository.findQuizQuestions.mockResolvedValue([{ questionId: 'q1', points: 1 }]);
            mockExamRepository.updateAttempt.mockResolvedValue({ ...mockAttempt, status: ExamSessionStatus.SUBMITTED, score: 0 });
    ...
    describe('create', () => {
        ...
        it('should create exam successfully', async () => {
            mockExamRepository.create.mockResolvedValue({ ...mockQuiz, title: 'New Exam' });
