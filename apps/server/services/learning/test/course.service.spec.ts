...
describe('CourseMasterService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a paid course successfully', async () => {
      ...
      mockCourseMasterRepository.create.mockResolvedValue({
        id: 'course-1',
        ...dto,
        status: 'draft',
      });
    ...
    it('should throw BadRequestException if paid course has invalid price', async () => {
      ...
      await expect(
        service.create(requester as any, invalidDto as any),
    ...
    it('should throw BadRequestException if Live course exceeds 6 months duration', async () => {
      ...
      await expect(
        service.create(requester as any, liveDto as any),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe('update', () => {
    ...
    it('should allow Admin/Staff with "course.update" and "course.publish" to update', async () => {
      ...
      mockCourseMasterRepository.update.mockResolvedValue({
        ...existing,
        ...updateDto,
      });
      mockCourseMasterRepository.slugExists.mockResolvedValue(false);
    ...
    it('should allow Instructor assigned to course to update (even without explicit course.update permission?)', async () => {
      ...
      mockCourseMasterRepository.getInstructors.mockResolvedValue([
        { userId: 'inst-1' },
      ]);
      mockCourseMasterRepository.update.mockResolvedValue({
        ...existing,
    ...
    it('should forbid Instructor NOT assigned to course (even with course.update)', async () => {
      ...
      await expect(
        service.update(requester as any, 'course-1', updateDto),
    ...
    it('should throw BadRequestException if updated durationWeeks exceeds 26 weeks', async () => {
      ...
      await expect(
        service.update(requester as any, 'course-1', invalidUpdateDto as any),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe('getCurriculum', () => {
    ...
    it('should show videoUrl if user is Enrolled', async () => {
      ...
      mockModuleRepository.findByCourseId.mockResolvedValue([
        { id: 'mod-1', orderIndex: 1 },
      ]);
      ...
      mockEnrollmentService.findByUserAndCourse.mockResolvedValue({
        id: 'enr-1',
      });
  ...
  describe('delete', () => {
    ...
    it('should soft delete if hardDelete is false', async () => {
      ...
      mockCourseMasterRepository.softDelete.mockResolvedValue({
        id: 'course-1',
      });

      await service.delete(requester as any, 'course-1', false);
      expect(courseMasterRepository.softDelete).toHaveBeenCalledWith(
        'course-1',
      );
    });
