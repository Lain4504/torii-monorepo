...
describe('BlogService', () => {
  ...
  describe('createBlog', () => {
    ...
    it('should throw BadRequestException if authorId is missing', async () => {
      await expect(
        service.createBlog({ ...createDto, authorId: undefined } as any),
    ...
    it('should throw NotFoundException if author does not exist', async () => {
      ...
      await expect(service.createBlog(createDto)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('findBlogById', () => {
    ...
    it('should throw NotFoundException if blog does not exist', async () => {
      ...
      await expect(service.findBlogById('non-existent')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('updateBlog', () => {
    ...
    it('should throw NotFoundException if blog does not exist', async () => {
      (repository.findById as jest.Mock).mockResolvedValue(null);
      await expect(service.updateBlog('non-existent', {})).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('publishBlog', () => {
    ...
    it('should throw BadRequestException if blog is already published', async () => {
      ...
      await expect(service.publishBlog('blog-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('deleteBlog', () => {
    ...
    it('should throw NotFoundException if blog does not exist', async () => {
      (repository.findById as jest.Mock).mockResolvedValue(null);
      await expect(service.deleteBlog('non-existent')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
