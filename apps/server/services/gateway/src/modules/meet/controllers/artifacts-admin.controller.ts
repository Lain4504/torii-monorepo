...
export class ArtifactsAdminController {
    ...
    async fetchArtifacts(
        ...
        try {
            const request = create(FetchArtifactsReqSchema, {
                ...
                type: body.type !== undefined ? body.type : RoomArtifactType.UNKNOWN_ARTIFACT,
                limit: body.limit || '20',
                from: body.from || '0',
            ...
            const artifacts = result.result?.artifactsList?.map((artifact: any) => ({
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : 'Error fetching artifacts',
            });
        }
    }
    ...
    async getArtifactInfo(
        ...
        try {
            ...
            const artifact = result.artifactInfo ? {
                ...
                metadata: toJson(result.artifactInfo.metadata.$typeName, result.artifactInfo.metadata),
            ...
            const roomInfo = result.roomInfo ? {
                ...
                joinedParticipants: parseInt(result.roomInfo.joinedParticipants || '0'),
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : 'Error getting artifact info',
            });
        }
    }
    ...
    async getDownloadToken(
        ...
        try {
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : 'Error generating download token',
            });
        }
    }
    ...
    async downloadArtifact(
        ...
        try {
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : 'Error downloading artifact',
            });
        }
    }
    ...
    async deleteArtifact(
        ...
        try {
            ...
            res.status(500).json({
                success: false,
                message: error instanceof Error ? error.message : 'Error deleting artifact',
            });
        }
    }
}
