...
export class SpeechToTextController {
    ...
    @Post('serviceStatus')
    async handleSpeechToTextTranslationServiceStatus(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        ...
        try {
            ...
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'speech.serviceStatus' }, { ...request, roomId: (req as any).roomId })
            );
    ...
    @Post('azureToken')
    async handleGenerateAzureToken(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        try {
            const request = fromBinary(GenerateAzureTokenReqSchema, bodyBuffer);
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'speech.generateAzureToken' }, {
                    ...request,
    ...
    @Post('userStatus')
    async handleSpeechServiceUserStatus(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        try {
            const request = fromBinary(SpeechServiceUserStatusReqSchema, bodyBuffer);
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'speech.userStatus' }, {
                    ...request,
                    roomId: (req as any).roomId,
                    userId: (req as any).requestedUserId
                })
    ...
    @Post('renewToken')
    async handleRenewAzureToken(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        try {
            const request = fromBinary(AzureTokenRenewReqSchema, bodyBuffer);
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'speech.renewToken' }, {
                    ...request,
                    roomId: (req as any).roomId,
                    userId: (req as any).requestedUserId
                })
