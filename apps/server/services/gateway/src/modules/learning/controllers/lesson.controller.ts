...
import {
  LessonSearchRequestDTO,
  lessonSearchRequestDTOSchema,
...
export class LessonController {
  ...
  async searchLessons(
    @Body(new ZodValidationPipe(lessonSearchRequestDTOSchema))
    dto: LessonSearchRequestDTO,
  ) {
    try {
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.lesson.search' }, dto),
      );
