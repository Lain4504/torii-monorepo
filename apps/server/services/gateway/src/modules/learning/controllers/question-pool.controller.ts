...
export class QuestionPoolController {
  ...
  async findAll(@Query() query: any) {
    try {
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.question-pool.findAll' }, query),
      );
  ...
  async getByCourse(@Param('courseMasterId') courseMasterId: string) {
    try {
      ...
      return errorResponse(
        error.message || 'Failed to fetch question pools by course',
      );
    }
  }
  ...
  async getByLesson(@Param('lessonId') lessonId: string) {
    try {
      ...
      return errorResponse(
        error.message || 'Failed to fetch question pools by lesson',
      );
    }
  }
  ...
  async getByJlptLevel(@Param('jlptLevel') jlptLevel: string) {
    try {
      ...
      return errorResponse(
        error.message || 'Failed to fetch question pools by JLPT level',
      );
    }
  }
