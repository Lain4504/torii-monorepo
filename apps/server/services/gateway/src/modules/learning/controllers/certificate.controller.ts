...
export class CertificateController {
  ...
  async findAll(
    ...
    try {
      ...
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.certificate.findAll' }, query),
      );
  ...
  async findById(@Param('id') id: string) {
    try {
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.certificate.findById' }, { id }),
      );
  ...
  async verify(@Param('code') code: string) {
    try {
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.certificate.verify' }, { code }),
      );
