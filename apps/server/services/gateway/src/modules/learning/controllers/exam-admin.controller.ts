...
export class ExamAdminController {
  ...
  async findAll(@Query() query: any) {
    try {
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.exam-admin.findAll' }, query),
      );
  ...
  async findById(@Param('id') id: string) {
    try {
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.exam-admin.findById' }, { id }),
      );
  ...
  async getStats(@Param('id') id: string) {
    try {
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.exam-admin.getStats' }, { id }),
      );
