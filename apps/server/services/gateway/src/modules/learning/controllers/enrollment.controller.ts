...
export class EnrollmentController {
  ...
  async checkGiftRecipient(
    ...
    try {
      ...
      const user = identityResponse?.data?.find(
  ...
  async findAll(@Query() query: any) {
    try {
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.enrollment.findAll' }, query),
      );
      return successPaginatedResponse(result);
    } catch (error: any) {
  ...
  async findById(@Param('id') id: string) {
    try {
      const result = await firstValueFrom(
        this.natsClient.send({ cmd: 'learning.enrollment.findById' }, { id }),
      );
  ...
  async checkEnrollment(
    @Param('courseRunId') courseRunId: string,
  ...
  async upgradeVersion(
    ...
    try {
      ...
      return errorResponse(
        error.message || 'Failed to upgrade enrollment version',
      );
    }
  }
