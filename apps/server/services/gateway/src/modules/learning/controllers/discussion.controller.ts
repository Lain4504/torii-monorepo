...
export class DiscussionController {
  ...
  @Get()
  async findAll(
    @Query() query: DiscussionTopicQueryDTO,
    @Req() req: ReqWithRequester,
  ) {
    try {
      const requester = req.requester;
      const result = await firstValueFrom(
        this.natsClient.send('discussion.findAll', {
          query,
  ...
  async findById(@Param('id') id: string, @Req() req: ReqWithRequester) {
    try {
      const requester = req.requester;
      const result = await firstValueFrom(
        this.natsClient.send('discussion.findById', {
          id,
          userId: requester?.sub,
        }),
      );
  ...
  @Post()
  async create(
    @Body() dto: DiscussionTopicCreateDTO,
    @Req() req: ReqWithRequester,
  ) {
    try {
      const requester = req.requester;
      const result = await firstValueFrom(
        this.natsClient.send('discussion.create', {
          dto,
          userId: requester?.sub,
        }),
      );
  ...
  async update(
    ...
    try {
      const requester = req.requester;
      const result = await firstValueFrom(
        this.natsClient.send('discussion.update', {
          id,
          dto,
          userId: requester?.sub,
        }),
  ...
  async delete(@Param('id') id: string, @Req() req: ReqWithRequester) {
    try {
      const requester = req.requester;
      const result = await firstValueFrom(
        this.natsClient.send('discussion.delete', {
          id,
          userId: requester?.sub,
        }),
      );
