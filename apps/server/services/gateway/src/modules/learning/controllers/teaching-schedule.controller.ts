...
export class TeachingScheduleController {
  ...
  async checkAvailability(
    ...
    const data = await firstValueFrom(
      this.natsClient.send(
        { cmd: 'learning.teachingSchedule.checkAvailability' },
        {
          lecturerId,
          dayOfWeek,
          startTime,
  ...
  async assignSchedule(
    ...
    const data = await firstValueFrom(
      this.natsClient.send(
        { cmd: 'learning.teachingSchedule.assign' },
        {
          ...dto,
          requester,
        },
  ...
  async findByRun(@Param('courseRunId') courseRunId: string) {
    const data = await firstValueFrom(
      this.natsClient.send(
        { cmd: 'learning.teachingSchedule.findByRun' },
        { courseRunId },
      ),
    );
    return successResponse(data);
  ...
  @Delete(':id')
  @Permissions('live_class.schedule')
  async removeSchedule(@Req() req: ReqWithRequester, @Param('id') id: string) {
    const requester = req.requester;
    await firstValueFrom(
      this.natsClient.send(
        { cmd: 'learning.teachingSchedule.remove' },
        {
          id,
          requester,
        },
  ...
  async createRequest(
    ...
    const data = await firstValueFrom(
      this.natsClient.send(
        { cmd: 'learning.teachingSchedule.createRequest' },
        {
          ...dto,
          requester,
        },
  ...
  async getPendingRequests(@Req() req: ReqWithRequester) {
    ...
    const data = await firstValueFrom(
      this.natsClient.send(
        { cmd: 'learning.teachingSchedule.getPendingRequests' },
        { requester },
      ),
    );
    return successResponse(data);
  ...
  async handleRequest(
    ...
    await firstValueFrom(
      this.natsClient.send(
        { cmd: 'learning.teachingSchedule.handleRequest' },
        {
          requestId,
          action,
          requester,
