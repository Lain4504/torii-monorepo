...
import {
  CourseMasterSearchRequestDTO,
  courseMasterSearchRequestDTOSchema,
...
export class CourseMasterController {
  ...
  async searchCourses(
    @Body(new ZodValidationPipe(courseMasterSearchRequestDTOSchema))
    dto: CourseMasterSearchRequestDTO,
  ...
  async advancedSearch(@Query() query: any) {
    const result = await firstValueFrom(
      this.natsClient.send(
        { cmd: 'learning.coursemaster.advancedSearch' },
        query,
      ),
    );
    return successPaginatedResponse(result);
  ...
  async getByType(@Param('type') type: 'vod' | 'live') {
    const result = await firstValueFrom(
      this.natsClient.send(
        { cmd: 'learning.coursemaster.getByType' },
        { type },
      ),
    );
  ...
  async getCourses(@Query() query: any, @Req() req: ReqWithRequester) {
    ...
    if (requester) {
      ...
      if (
        ...
        !permissions.includes('course.view_restricted') &&
        permissions.includes('course.instructor')
      ) {
  ...
  async updateLiveConfig(
    ...
    return successResponse(
      { course: result },
      'Live configuration updated successfully',
    );
  }
