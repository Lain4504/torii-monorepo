...
export class ModuleController {
    ...
    async create(
        ...
        const item = await firstValueFrom(
            this.nats.send({ cmd: 'academy.module.create' }, { ...dto, syllabusId, requesterId: req.requester?.sub }),
        );
    ...
    async delete(@Param('id', new ParseUUIDPipe()) id: string, @Req() req: ReqWithRequester) {
        const result = await firstValueFrom(
            this.nats.send({ cmd: 'academy.module.delete' }, { id, requesterId: req.requester?.sub }),
        );
