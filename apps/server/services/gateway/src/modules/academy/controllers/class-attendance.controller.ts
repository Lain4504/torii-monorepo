...
export class ClassAttendanceController {
    ...
    async create(
        ...
        const item = await firstValueFrom(
            this.nats.send({ cmd: 'academy.classAttendance.create' }, { ...dto, requesterId: req.requester?.sub }),
        );
        return successResponse({ item });
    }
    ...
    async update(
        ...
        const item = await firstValueFrom(
            this.nats.send({ cmd: 'academy.classAttendance.update' }, { id, input: dto, requesterId: req.requester?.sub }),
        );
        return successResponse({ item });
    }
    ...
    async delete(@Param('id', new ParseUUIDPipe()) id: string, @Req() req: ReqWithRequester) {
        const result = await firstValueFrom(
            this.nats.send({ cmd: 'academy.classAttendance.delete' }, { id, requesterId: req.requester?.sub }),
        );
        return successResponse(result);
    }
}
