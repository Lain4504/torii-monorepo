...
export class SyllabusController {
    ...
    async create(
        ...
        const item = await firstValueFrom(
            this.nats.send({ cmd: 'academy.syllabus.create' }, { ...dto, requesterId: req.requester?.sub }),
        );
        return successResponse({ item });
    }
    ...
    async clone(
        ...
        const item = await firstValueFrom(
            this.nats.send({ cmd: 'academy.syllabus.clone' }, { sourceSyllabusId: id, ...dto, requesterId: req.requester?.sub }),
        );
        return successResponse({ item });
    }
    ...
    async publish(@Param('id', new ParseUUIDPipe()) id: string, @Req() req: ReqWithRequester) {
        const item = await firstValueFrom(
            this.nats.send({ cmd: 'academy.syllabus.publish' }, { id, requesterId: req.requester?.sub }),
        );
        return successResponse({ item });
    }
    ...
    @Post(':id/lock')
    @Permissions('academy.content.write')
    async lock(@Param('id', new ParseUUIDPipe()) id: string, @Req() req: ReqWithRequester) {
        const item = await firstValueFrom(
            this.nats.send({ cmd: 'academy.syllabus.lock' }, { id, requesterId: req.requester?.sub }),
        );
        return successResponse({ item });
    }
}
