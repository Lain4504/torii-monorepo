...
jest.mock(
  '@server/communication/interfaces/repositories',
  () => {
...
jest.mock(
  '@server/communication/interfaces/services',
  () => {
...
describe('TicketService (Exhaustive)', () => {
  ...
  describe('createTicket', () => {
    ...
    describe('Refund Validation', () => {
      ...
      it('should throw if courseId is missing in metadata for refund', async () => {
        await expect(
          service.createTicket(USER_ID, { type: TicketType.REFUND } as any),
        ).rejects.toThrow(BadRequestException);
      });

      it('should throw if user is not enrolled', async () => {
        natsClient.send.mockReturnValue(of({ isEnrolled: false }));
        await expect(service.createTicket(USER_ID, refundDto)).rejects.toThrow(
      ...
      it('should handle NATS check error', async () => {
        natsClient.send.mockReturnValue(
          throwError(() => new Error('NATS Fail')),
        );
        await expect(service.createTicket(USER_ID, refundDto)).rejects.toThrow(
          'Could not verify enrollment status',
        );
      });
    });
  });
  ...
  describe('getTicketById', () => {
    ...
    it('should throw NotFoundException if missing', async () => {
      ...
      await expect(service.getTicketById(TICKET_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('updateTicketStatus', () => {
    it('should throw if ticket is already finalized', async () => {
      ...
      await expect(
        service.updateTicketStatus(TICKET_ID, 'staff-1', {
    ...
    describe('Refund Processing (Edges & Complex Flow)', () => {
      ...
      beforeEach(() => {
        ...
        natsClient.send.mockImplementation((pattern: any) => {
          ...
          if (cmd === 'learning.enrollment.check')
            return of({
              isEnrolled: true,
              enrollment: { enrollmentDate: new Date().toISOString() },
            });
          if (cmd === 'billing.order.findAll')
            return of({
              data: [{ id: ORDER_ID, metadata: { courseId: COURSE_ID } }],
            });
          if (cmd === 'learning.enrollment.delete')
            return of({ id: 'en-1', finalPrice: 1000, senderId: USER_ID });
          if (cmd === 'billing.user_balance.add') return of({ success: true });
          if (cmd === 'identity.users.findById')
            return of({
              user: { email: 'test@user.com', displayName: 'User' },
            });
          if (cmd === 'learning.course.findById')
      ...
      it('should process full flow with senderId', async () => {
        natsClient.send.mockImplementation((pattern: any) => {
          if (pattern.cmd === 'learning.enrollment.delete')
            return of({ finalPrice: 500, senderId: 'sender-001' });
          return of({
            isEnrolled: true,
        ...
        await service.updateTicketStatus(TICKET_ID, 'staff-1', {
          status: TicketStatus.RESOLVED,
        });
        expect(natsClient.send).toHaveBeenCalledWith(
          { cmd: 'billing.user_balance.add' },
          expect.objectContaining({ userId: 'sender-001' }),
        );
      });
      ...
      it('should skip email if user results verify email is missing', async () => {
        ...
        expect(Logger.prototype.warn).toHaveBeenCalledWith(
          expect.stringContaining('Could not send refund email'),
        );
      });

      it('should handle "not found" enrollment exception gracefully', async () => {
        natsClient.send.mockImplementation((pattern: any) => {
          if (pattern.cmd === 'learning.enrollment.check')
        ...
        expect(Logger.prototype.warn).toHaveBeenCalledWith(
          expect.stringContaining('Enrollment not found during refund'),
        );
      ...
      it('should handle NATS fetch user/course details failure gracefully', async () => {
        natsClient.send.mockImplementation((pattern: any) => {
          ...
          if (cmd === 'learning.enrollment.delete')
            return of({ finalPrice: 100, senderId: USER_ID });
          return of({});
        });
        ...
        expect(Logger.prototype.error).toHaveBeenCalledWith(
          expect.stringContaining(
            'Failed to fetch user/course details via NATS',
          ),
        );
        expect(emailService.sendEmail).not.toHaveBeenCalled();
      ...
      it('should handle audit log emission failure', async () => {
        natsClient.emit.mockImplementation(() => {
        ...
        expect(Logger.prototype.error).toHaveBeenCalledWith(
          expect.stringContaining('Failed to emit audit log'),
        );
      });
      ...
      it('should skip balance refund if finalPrice is zero', async () => {
        ...
        expect(natsClient.send).not.toHaveBeenCalledWith(
          { cmd: 'billing.user_balance.add' },
          expect.anything(),
        );
      });
