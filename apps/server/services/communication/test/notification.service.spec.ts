...
describe('NotificationService (Exhaustive - Test Only Fix)', () => {
  ...
  describe('findAll', () => {
    ...
    it('should throw BadRequestException on repository error', async () => {
      ...
      await expect(
        service.findAll(USER_ID, { page: 1, limit: 10 }),
    ...
    it('should default to page 1 and limit 10 for non-numeric strings', async () => {
      const query: any = { page: 'abc', limit: 'xyz' };
      notificationRepository.count.mockResolvedValue(0);
      notificationRepository.findMany.mockResolvedValue([]);
  ...
  describe('markAsRead', () => {
    it('should update and return notification as read', async () => {
      notificationRepository.findByIdAndUserId.mockResolvedValue(
        mockNotification,
      );
    ...
    it('should throw BadRequestException on unexpected update error', async () => {
      ...
      notificationRepository.update.mockRejectedValue(
  ...
  describe('markAllAsRead', () => {
    ...
    it('should throw BadRequestException on error', async () => {
      ...
      await expect(service.markAllAsRead(USER_ID)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('getUnreadCount', () => {
    ...
    it('should throw BadRequestException on error', async () => {
      ...
      await expect(service.getUnreadCount(USER_ID)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('create', () => {
    ...
    it('should respect custom sentVia if provided', async () => {
      const dto: any = {
        userId: USER_ID,
        title: 'T',
        message: 'M',
  ...
  describe('delete', () => {
    ...
    it('should throw NotFoundException if notification missing', async () => {
      notificationRepository.findByIdAndUserId.mockResolvedValue(null);
      await expect(service.delete(NOTIFICATION_ID, USER_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should throw BadRequestException on unexpected delete error', async () => {
      ...
      notificationRepository.delete.mockRejectedValue(
  ...
  describe('handleSendNotification', () => {
    ...
    it('should handle repository failure gracefully (catch block)', async () => {
      notificationRepository.create.mockRejectedValue(new Error('DB Fail'));
      const payload: any = {
        recipientId: 'r1',
        type: 'COMMENT_REPLY',
        payload: { title: 'T', body: 'B', metadata: {} },
      ...
      expect(Logger.prototype.error).toHaveBeenCalledWith(
        expect.stringContaining('Error handling send_notification event'),
        expect.anything(),
      );
    });
  });
  ...
  describe('handleCommentReply', () => {
    ...
    it('should fallback to "bài viết" if blog title is missing', async () => {
      prisma.blog.findUnique.mockResolvedValue({
        id: 'b1',
        authorId: 'staff-1',
        title: '',
      }); // Empty title
      prisma.user.findUnique.mockResolvedValue({ displayName: 'Author' });
      ...
      expect(notificationRepository.create).toHaveBeenCalledWith(
        expect.objectContaining({
          message: expect.stringContaining('trong bài viết "bài viết"'),
        }),
      );
    });
  });
