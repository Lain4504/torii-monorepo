...
export class RedisWebhookService {
    ...
    async addWebhookData(roomId: string, val: Buffer | string): Promise<void> {
        try {
            ...
            this.logger.error(`AddWebhookData failed for roomId ${roomId}: ${error.message}`);
            ...
            if (error.message.includes('unknown command')) {
                ...
                this.logger.warn(`HEXPIRE not supported, field TTL not set for roomId ${roomId}`);
            } else {
                throw error;
            }
        }
    ...
    async getWebhookData(roomId: string): Promise<Buffer | null> {
        try {
            ...
            this.logger.error(`GetWebhookData failed for roomId ${roomId}: ${error.message}`);
            throw error;
        }
    }
    ...
    async deleteWebhookData(roomId: string): Promise<void> {
        try {
            ...
            this.logger.error(`DeleteWebhookData failed for roomId ${roomId}: ${error.message}`);
            throw error;
        }
    }
}
