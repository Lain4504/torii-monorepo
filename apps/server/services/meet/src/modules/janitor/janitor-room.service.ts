    ...
    async activeRoomChecker(): Promise<void> {
        ...
        for (const room of activeRooms) {
            ...
            if (userIds.length === 0) {
                ...
                if (now > valid) {
                    this.logger.log(`Closing empty room as it reached empty timeout: ${room.roomId}, created: ${createdAt}, timeout: ${emptyTimeout}, now: ${now}`);
    ...
    async checkRoomWithDuration(): Promise<void> {
        try {
            ...
            for (const [roomId, r] of Object.entries(rooms)) {
                ...
                if (now > valid) {
                    this.logger.log(`Room exceeded duration limit. Ending room: ${roomId}, duration: ${r.duration}, started: ${r.startedAt}`);
                    await this.roomEndService.endRoom(create(RoomEndReqSchema, {
                        roomId: roomId
                    }));
                }
