...
export class FileNatsController {
    ...
    async handleConvertWhiteboard(@Payload() data: { roomId: string; roomSid: string; filePath: string }): Promise<any> {
        return this.fileService.convertAndBroadcastWhiteboardFile(data.roomId, data.roomSid, data.filePath);
    }

    @MessagePattern({ cmd: 'file.getByType' })
    async handleGetFilesByType(@Payload() req: GetRoomUploadedFilesReq): Promise<any> {
