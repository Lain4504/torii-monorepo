...
describe('AchievementService', () => {
  ...
  describe('getUserAchievements', () => {
    ...
    it('should throw error if ensureAchievementsExist fails', async () => {
      ...
      await expect(service.getUserAchievements(mockUserId)).rejects.toThrow(
        'Upsert Failed',
      );
    });
  ...
  describe('checkStreakAchievements', () => {
    it('should unlock streak achievements if milestone reached', async () => {
      const unlockSpy = jest
        .spyOn(service as any, 'unlockAchievement')
  ...
  describe('checkLessonAchievements', () => {
    ...
    it('should handle database error when counting lessons', async () => {
      ...
      await expect(service.checkLessonAchievements(mockUserId)).rejects.toThrow(
        'Count Error',
      );
    });
  });
  ...
  describe('checkCourseAchievements', () => {
    ...
    it('should handle database error when counting courses', async () => {
      ...
      await expect(service.checkCourseAchievements(mockUserId)).rejects.toThrow(
        'Course DB Error',
      );
    });
  });
  ...
  describe('checkFlashcardAchievements', () => {
    ...
    it('should handle database error when counting reviews', async () => {
      prisma.flashcardReview.count.mockRejectedValue(
        new Error('Flashcard DB Error'),
      );
      await expect(
        service.checkFlashcardAchievements(mockUserId),
      ).rejects.toThrow('Flashcard DB Error');
    });
  });
  ...
  describe('unlockAchievement', () => {
    ...
    it('should handle database error while checking existing unlock', async () => {
      ...
      prisma.achievement.upsert.mockResolvedValue({});
      prisma.achievement.findUnique.mockResolvedValue(mockAchievement);
      prisma.userAchievement.findUnique.mockRejectedValue(
        new Error('Selection Error'),
      );

      await expect(
        service.unlockAchievement(mockUserId, 'STREAK_3'),
      ).rejects.toThrow('Selection Error');
    });
    ...
    it('should handle error during userAchievement upsert', async () => {
      ...
      prisma.userAchievement.upsert.mockRejectedValue(
    ...
    it('should handle failure in applying rewards (gamification upsert)', async () => {
      ...
      prisma.userGamification.upsert.mockRejectedValue(
        new Error('Rewards Error'),
      );

      await expect(
        service.unlockAchievement(mockUserId, 'STREAK_3'),
      ).rejects.toThrow('Rewards Error');
    });
  });
});
