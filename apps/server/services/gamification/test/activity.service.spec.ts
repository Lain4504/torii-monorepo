...
describe('ActivityService', () => {
  ...
  describe('recordActivity', () => {
    ...
    it('should reward small XP for incorrect quiz answers', async () => {
      await service.recordActivity(mockUserId, 'QUIZ_ANSWER' as any, {
        isCorrect: false,
      });
    ...
    it('should check achievements based on activity type', async () => {
      ...
      expect(achievementService.checkLessonAchievements).toHaveBeenCalledWith(
        mockUserId,
      );
      ...
      expect(achievementService.checkQuizAchievements).toHaveBeenCalledWith(
        mockUserId,
        100,
        'N3',
      );
      ...
      expect(
        achievementService.checkFlashcardAchievements,
    ...
    it('should check streak achievements only on milestones', async () => {
      ...
      expect(achievementService.checkStreakAchievements).toHaveBeenCalledWith(
        mockUserId,
        3,
      );
    });
    ...
    it('should handle database error when checking existing activity', async () => {
      ...
      await expect(
        service.recordActivity(mockUserId, activityType as any),
    ...
    it('should handle failure in achievement services', async () => {
      achievementService.checkLessonAchievements.mockRejectedValue(
        new Error('Achievement Error'),
      );

      await expect(
        service.recordActivity(mockUserId, 'LESSON_COMPLETE' as any),
      ).rejects.toThrow('Achievement Error');
    });
  });
  ...
  describe('getWeeklyActiveDates', () => {
    ...
    it('should handle database error when fetching active dates', async () => {
      ...
      await expect(service.getWeeklyActiveDates(mockUserId)).rejects.toThrow(
        'DB Error',
      );
    });
  });
  ...
  describe('getHistory', () => {
    ...
    it('should handle database error when fetching history', async () => {
      prisma.gamificationHistory.findMany.mockRejectedValue(
