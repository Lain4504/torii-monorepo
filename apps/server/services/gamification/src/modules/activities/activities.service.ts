...
import { PROFILES_SERVICE_TOKEN, ACHIEVEMENTS_SERVICE_TOKEN } from '@server/gamification/interfaces/services';
import type { IProfilesService, IAchievementsService } from '@server/gamification/interfaces/services';
...
export class ActivitiesService implements IActivitiesService {
    ...
    constructor(
        @Inject(ACTIVITIES_REPOSITORY_TOKEN) private readonly activitiesRepository: IActivitiesRepository,
    ...
    async recordActivity(
        ...
        const existing = await this.activitiesRepository.findDailyActivity(userId, today, activityType);
        ...
        if (xpGain > 0) {
            gamification = await this.profilesService.updateXP(userId, xpGain, activityType);
        } else {
        ...
        if (streakResult.streakUpdated) {
            ...
            if (streakResult.isMilestone) {
                await this.achievementsService.checkStreakAchievements(userId, streakResult.newStreak);
            }
        }
        ...
        switch (activityType) {
            ...
            case 'QUIZ_ANSWER':
                if (meta?.score && meta?.jlptLevel) {
                    await this.achievementsService.checkQuizAchievements(userId, meta.score, meta.jlptLevel);
                }
                break;
    ...
    async getHistory(userId: string, query: { page?: any, limit?: any, type?: any }) {
        ...
        const [data, total] = await this.activitiesRepository.findHistory(userId, skip, limit, query.type);
