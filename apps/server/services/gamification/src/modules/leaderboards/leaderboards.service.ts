...
export class LeaderboardsService implements ILeaderboardsService {
    ...
    constructor(
        @Inject(LEADERBOARDS_REPOSITORY_TOKEN) private readonly leaderboardsRepository: ILeaderboardsRepository,
    ...
    async getWeeklyLeaderboard(userId?: string): Promise<LeaderboardDto> {
        const topStreaks = await this.leaderboardsRepository.getTopByWeeklyStreak(100);
        ...
        if (userId) {
            ...
            if (index !== -1) {
                ...
                const profile = await this.profilesService.getGamificationProfile(userId);
                const rank = await this.leaderboardsRepository.getUserWeeklyRank(userId);
                currentUser = {
                    id: profile.userId,
                    displayName: '',
                    avatarUrl: null,
