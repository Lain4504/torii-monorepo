...
export class ProfilesService implements IProfilesService {
  ...
  async getStreakStatus(userId: string): Promise<StreakStatusDto> {
    ...
    const willBreakTomorrow =
      !isActiveToday && gamification.lastActiveDate !== yesterday;
  ...
  async markStreakToastShown(userId: string): Promise<void> {
    ...
    const secondsUntilEndDay = Math.max(
      1,
  ...
  async recordActivity(userId: string): Promise<{
    ...
    if (!gamification.lastActiveDate) {
      ...
      const daysMissed = this.getDaysDifference(
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      await tx.userGamification.update({
        ...
        data: {
          ...
          weeklyActiveCount:
            lastActiveDate && this.isThisWeek(lastActiveDate)
              ? { increment: 1 }
              : 1,
          monthlyActiveCount:
            lastActiveDate && this.isThisMonth(lastActiveDate)
              ? { increment: 1 }
              : 1,
        },
      });
  ...
  async checkStreaksDaily(): Promise<void> {
    ...
    const usersAtRisk =
      await this.profilesRepository.findUsersAtRiskOfStreakReset(twoDaysAgo);
    ...
    for (const gamification of usersAtRisk) {
      if (gamification.freezeCount > 0) {
        ...
        await this.profilesRepository.update(gamification.userId, {
          currentStreak: 0,
        });
      }
    }
  ...
  async updateXP(
    ...
    try {
      ...
      if (newLevel > gamification.level) {
        ...
        this.natsClient.emit('user.level_up', {
          userId,
          level: newLevel,
          xp: totalXp,
        });
      ...
      this.natsClient.emit('user.xp_gained', {
        userId,
        xpGained: xpGain,
        totalXp: totalXp,
      });
