...
import type { UserAchievementDto, AchievementUnlockedEvent } from '@workspace/schemas';
...
export class AchievementsService implements IAchievementsService {
    ...
    constructor(
        @Inject(ACHIEVEMENTS_REPOSITORY_TOKEN) private readonly achievementsRepository: IAchievementsRepository,
    ...
    async getUserAchievements(userId: string): Promise<UserAchievementDto[]> {
        ...
        const userAchievements = await this.achievementsRepository.findByUserId(userId);
    ...
    async checkLessonAchievements(userId: string): Promise<void> {
        ...
        if (lessonsCompleted >= 1) await this.unlockAchievement(userId, 'FIRST_LESSON');
        if (lessonsCompleted >= 50) await this.unlockAchievement(userId, 'LESSON_50');
    }
    ...
    async checkCourseAchievements(userId: string): Promise<void> {
        ...
        if (coursesCompleted >= 1) await this.unlockAchievement(userId, 'FIRST_COURSE');
        if (coursesCompleted >= 5) await this.unlockAchievement(userId, 'COURSE_5');
    }

    async checkQuizAchievements(userId: string, score: number, jlptLevel: string): Promise<void> {
        if (score >= 100) {
    ...
    async checkFlashcardAchievements(userId: string): Promise<void> {
        const reviewCount = await this.prisma.flashcardReview.count({ where: { userId } });
        if (reviewCount >= 100) await this.unlockAchievement(userId, 'FLASHCARD_100');
        if (reviewCount >= 1000) await this.unlockAchievement(userId, 'FLASHCARD_1000');
    }

    async unlockAchievement(userId: string, achievementCode: string): Promise<void> {
        const achievement = await this.achievementsRepository.findByCode(achievementCode);
        if (!achievement || !achievement.isActive) return;

        const existing = await this.achievementsRepository.findUserAchievement(userId, achievement.id);
        if (existing?.isUnlocked) return;

        await this.achievementsRepository.upsertUserAchievement(userId, achievement.id, {
            isUnlocked: true,
            unlockedAt: new Date(),
        });
    ...
    private async applyRewards(userId: string, rewards: Record<string, any>): Promise<void> {
