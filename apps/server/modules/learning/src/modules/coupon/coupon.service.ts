...
export class CouponService implements ICouponService {
    ...
    async create(requester: Requester, dto: CouponCreateDTO): Promise<CouponResponseDTO> {
        ...
        try {
            ...
            // Validate dates
            const validFrom = typeof dto.validFrom === 'string' ? new Date(dto.validFrom) : dto.validFrom;
            const validUntil = typeof dto.validUntil === 'string' ? new Date(dto.validUntil) : dto.validUntil;
            ...
            if (dto.maxDiscountAmount && dto.discountType === CouponDiscountType.FIXED_AMOUNT) {
                throw new BadRequestException('maxDiscountAmount is only applicable for percentage discount type');
            }
            if (dto.maxDiscountAmount && dto.maxDiscountAmount !== null && dto.maxDiscountAmount <= 0) {
                throw new BadRequestException('maxDiscountAmount must be greater than 0');
            }

            // Validate usageLimit
            if (dto.usageLimit !== undefined && dto.usageLimit !== null && dto.usageLimit <= 0) {
    ...
    async update(requester: Requester, couponId: string, dto: CouponUpdateDTO): Promise<CouponResponseDTO> {
        ...
        try {
            ...
            if (dto.code && dto.code !== coupon.code) {
                const codeExists = await this.couponRepository.codeExists(dto.code, couponId);
                if (codeExists) {
                    throw new BadRequestException(`Coupon code ${dto.code} already exists`);
                }
            }
            ...
            if (dto.validFrom) {
                validFrom = typeof dto.validFrom === 'string' ? new Date(dto.validFrom) : dto.validFrom;
            }
            if (dto.validUntil) {
                validUntil = typeof dto.validUntil === 'string' ? new Date(dto.validUntil) : dto.validUntil;
            }
            ...
            if (discountType === CouponDiscountType.PERCENTAGE && Number(discountValue) > 100) {
                throw new BadRequestException('Percentage discount cannot exceed 100%');
            }

            // Validate maxDiscountAmount: only applicable for PERCENTAGE type
            if (dto.maxDiscountAmount !== undefined && (dto.discountType ?? coupon.discountType) === CouponDiscountType.FIXED_AMOUNT) {
                throw new BadRequestException('maxDiscountAmount is only applicable for percentage discount type');
            }
            ...
            if (dto.usageLimit !== undefined && dto.usageLimit !== null && dto.usageLimit < coupon.usageCount) {
                throw new BadRequestException(`Usage limit (${dto.usageLimit}) cannot be less than current usage count (${coupon.usageCount})`);
            }
            if (dto.usageLimit !== undefined && dto.usageLimit !== null && dto.usageLimit <= 0) {
                throw new BadRequestException('Usage limit must be greater than 0');
            }
     ...
     * Delete coupon
     */
    async delete(requester: Requester, couponId: string): Promise<{ message: string }> {
        // Check permissions
     ...
     */
    async validateCoupon(request: CouponValidateRequestDTO): Promise<CouponValidateResponseDTO> {
        try {
            // 1. Find coupon
            ...
            if (request.userId) {
                const userUsageCount = await this.couponRepository.countUserUsage(coupon.id, request.userId);
     ...
     */
    async calculateDiscount(request: CouponCalculateDiscountRequestDTO): Promise<CouponCalculateDiscountResponseDTO> {
        try {
