...
export class ModuleHandler {
    ...
    async findByCourseId(@Payload() data: { courseId: string, userId: string }) {
    ...
    async update(@Payload() data: ModuleUpdateDTO & { id: string, userId: string }) {
    ...
    async reorder(@Payload() data: { courseId: string, moduleOrders: { id: string; orderIndex: number }[], userId: string }) {
        const { courseId, moduleOrders, userId } = data;
        const requester = { sub: userId, role: 'INSTRUCTOR' as any, permissions: [] };
