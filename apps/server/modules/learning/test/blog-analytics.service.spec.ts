...
describe('BlogAnalyticsService', () => {
    ...
    describe('generateDailyBlogInteractionStats', () => {
        ...
        it('should gracefully handle errors during processing of a single blog', async () => {
            mockPrismaService.blog.findMany.mockResolvedValue([mockBlog, { ...mockBlog, id: 'blog-2' }]);
            mockPrismaService.$queryRaw.mockRejectedValueOnce(new Error('Internal Error'));
            mockPrismaService.$queryRaw.mockResolvedValue([]);
