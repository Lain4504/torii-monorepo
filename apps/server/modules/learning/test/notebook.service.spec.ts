...
describe('NotebookService', () => {
    ...
    describe('createNotebook', () => {
        it('should create notebook if name not duplicate', async () => {
            ...
            const result = await service.createNotebook({ userId: 'user-1', name: 'New' });
        ...
        it('should throw RpcException if name duplicate', async () => {
            ...
            await expect(service.createNotebook({ userId: 'user-1', name: 'Existing' }))
    ...
    describe('addEntry', () => {
        it('should add entry to notebook', async () => {
            ...
            const result = await service.addEntry('nb-1', { word: 'W', meaning: 'M' }, 'user-1');
        ...
        it('should throw RpcException if not owner', async () => {
            mockNotebookRepository.findById.mockResolvedValue({ ...mockNotebook, userId: 'other' });
            await expect(service.addEntry('nb-1', { word: 'W', meaning: 'M' }, 'user-1'))
                .rejects.toThrow(RpcException);
        });
    });
