...
describe('CourseInstructorService', () => {
    ...
    beforeEach(async () => {
        ...
        courseInstructorRepository = module.get(COURSE_INSTRUCTOR_REPOSITORY_TOKEN);
    ...
    describe('assignLecturer', () => {
        const dto = { courseId: 'course-1', lecturerId: 'lect-1', role: 'INSTRUCTOR' };

        it('should assign lecturer successfully', async () => {
            mockPrismaService.course.findUnique.mockResolvedValue({ id: 'course-1' });
            mockPrismaService.user.findUnique.mockResolvedValue({ id: 'lect-1', role: 'LECTURER' });
            ...
            const result = await service.assignLecturer(mockRequester as any, dto as any);

            expect(result.id).toBe(mockInstructor.id);
            expect(courseInstructorRepository.assign).toHaveBeenCalled();
            expect(natsClient.emit).toHaveBeenCalledWith({ cmd: 'course.instructor.assigned' }, expect.any(Object));
        });
        ...
        it('should throw ForbiddenException if requester is not admin or staff', async () => {
            ...
            await expect(service.assignLecturer(studentRequester as any, dto as any))
                .rejects.toThrow(ForbiddenException);
        });

        it('should throw NotFoundException if course does not exist', async () => {
            mockPrismaService.course.findUnique.mockResolvedValue(null);
            await expect(service.assignLecturer(mockRequester as any, dto as any))
                .rejects.toThrow(NotFoundException);
        });

        it('should throw BadRequestException if user is not a lecturer', async () => {
            mockPrismaService.course.findUnique.mockResolvedValue({ id: 'course-1' });
            mockPrismaService.user.findUnique.mockResolvedValue({ id: 'lect-1', role: 'LEARNER' });
            await expect(service.assignLecturer(mockRequester as any, dto as any))
    ...
    describe('getInstructorsByCourse', () => {
        it('should return instructors for a course', async () => {
            mockPrismaService.course.findUnique.mockResolvedValue({ id: 'course-1' });
            mockCourseInstructorRepository.findByCourseId.mockResolvedValue([mockInstructor]);
            mockPrismaService.user.findUnique.mockResolvedValue({ id: 'lect-1', displayName: 'John' });
    ...
    describe('unassignLecturer', () => {
        it('should unassign lecturer successfully', async () => {
            ...
            mockCourseInstructorRepository.unassign.mockResolvedValue({ id: 'instr-1' });

            const result = await service.unassignLecturer(mockRequester as any, 'instr-1');
        ...
        it('should throw NotFoundException if assignment not found', async () => {
            ...
            await expect(service.unassignLecturer(mockRequester as any, 'non-existent'))
                .rejects.toThrow(NotFoundException);
        });
    });
});
