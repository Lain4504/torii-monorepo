...
describe('SrsAlgorithmService', () => {
    ...
    describe('calculateNextReview', () => {
        it('should return initial learning values for Quality 2 (Good) on a new card', () => {
            const result = service.calculateNextReview(0, 2.5, ReviewQuality.TWO, FlashcardState.NEW);
            expect(result.newInterval).toBe(1);
            expect(result.newState).toBe(FlashcardState.LEARNING);
        });

        it('should increase interval for Quality 2 (Good) on second review', () => {
            const result = service.calculateNextReview(1, 2.5, ReviewQuality.TWO, FlashcardState.LEARNING);
        ...
        it('should graduate to REVIEW state when interval >= 7', () => {
            const result = service.calculateNextReview(6, 2.5, ReviewQuality.TWO, FlashcardState.LEARNING);
        ...
        it('should reset interval to 0 and state to RELEARNING on Quality 0 (Again)', () => {
            const result = service.calculateNextReview(10, 2.5, ReviewQuality.ZERO, FlashcardState.REVIEW);
        ...
        it('should use easyBonus for Quality 3 or 4 (Easy)', () => {
            const result = service.calculateNextReview(10, 2.5, ReviewQuality.THREE, FlashcardState.REVIEW);
