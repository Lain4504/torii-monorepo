...
describe('WishlistService', () => {
    ...
    beforeEach(async () => {
        ...
        repository = module.get(WishlistRepository) as jest.Mocked<WishlistRepository>;
    });
    ...
    describe('create', () => {
        ...
        it('should throw error if userId is missing', async () => {
            await expect(service.create({ ...createInput, userId: undefined as any })).rejects.toThrow('UserId is required');
        });

        it('should throw error if courseId is missing', async () => {
            await expect(service.create({ ...createInput, courseId: undefined as any })).rejects.toThrow('CourseId is required');
        });
        ...
        it('should return existing wishlist if P2002 error occurs', async () => {
            ...
            expect(repository.findByUserAndCourse).toHaveBeenCalledWith(mockUserId, mockCourseId);
            expect(result).toEqual(mockWishlistResponse);
        });
    ...
    describe('toggle', () => {
        ...
        it('should throw error on database error', async () => {
            repository.findByUserAndCourse.mockRejectedValue(new Error('Database error'));

            await expect(service.toggle(mockUserId, mockCourseId)).rejects.toThrow('Database error');
        });
    });
