...
describe('NotificationService (Exhaustive - Test Only Fix)', () => {
    ...
    describe('create', () => {
        ...
        it('should respect custom sentVia if provided', async () => {
            const dto: any = { userId: USER_ID, title: 'T', message: 'M', sentVia: ['email', 'push'] };
            notificationRepository.create.mockResolvedValue({ ...mockNotification, ...dto });
    ...
    describe('handleSendNotification', () => {
        ...
        it('should handle repository failure gracefully (catch block)', async () => {
            ...
            const payload: any = { recipientId: 'r1', type: 'COMMENT_REPLY', payload: { title: 'T', body: 'B', metadata: {} } };
            
            // Should not throw
            await expect(service.handleSendNotification(payload)).resolves.not.toThrow();
            expect(Logger.prototype.error).toHaveBeenCalledWith(expect.stringContaining('Error handling send_notification event'), expect.anything());
        });
    });
    ...
    describe('handleCommentReply', () => {
        ...
        it('should fallback to "bài viết" if blog title is missing', async () => {
            prisma.blog.findUnique.mockResolvedValue({ id: 'b1', authorId: 'staff-1', title: '' }); // Empty title
