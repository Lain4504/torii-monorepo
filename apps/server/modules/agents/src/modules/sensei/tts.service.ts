...
export class TTSService {
    ...
    async getAudioBase64(text: string, voice: string = 'google-translate'): Promise<string> {
        this.logger.log(`[TTS] Request received - Voice: ${voice}, Text: ${text.substring(0, 30)}...`);
        ...
        if (voice && voice.includes('Neural')) {
            ...
            try {
                ...
                this.logger.log(`[TTS] Edge TTS succeeded. Audio length: ${result.length}`);
                return result;
            } catch (error) {
                this.logger.error(`Edge TTS failed for voice ${voice}: ${error.message}. Falling back to Google TTS.`);
            }
        } else {
    ...
    private async getEdgeAudio(text: string, voice: string): Promise<string> {
        const tempFile = path.join(os.tmpdir(), `tts-${Date.now()}-${Math.random().toString(36).substring(7)}.mp3`);
