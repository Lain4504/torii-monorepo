...
import { NatsRoomEventsService } from '@server/meet/handlers/nats-room-events.service';
...
export class RoomUserService {
    ...
    async getWajlcJoinToken(req: any): Promise<{ token: string }> {
        ...
        this.logger.log(`Request to generate join token: room=${roomId}, user=${userId}, name=${userName}, admin=${isAdmin}`);
        ...
        try {
            ...
            if (!req.userInfo.userMetadata.exUserId || req.userInfo.userMetadata.exUserId === '') {
            ...
            if (meta.roomFeatures?.autoGenUserId) {
                ...
                const status = await this.natsUserInfo.getRoomUserStatus(roomId, req.userInfo.userId);
                if (status === 'online') {
                    this.logger.warn('Same user found in online status, removing before re-generating token');
    ...
    private async waitForUserToBeOffline(roomId: string, userId: string): Promise<void> {
        ...
        this.logger.log(`Waiting for user ${userId} to be offline in room ${roomId}`);
    ...
    private assignLockSettingsToUser(meta: any, req: any): void {
        ...
        this.applyDefaultLockSettings(meta.defaultLockSettings, req.userInfo.userMetadata.lockSettings);
    }
    ...
    async updateUserLockSettings(data: {
        ...
        this.logger.log(`Updating lock settings: room=${data.roomId}, user=${data.userId}, service=${data.service}, direction=${data.direction}`);
        ...
        try {
            if (data.userId === 'all') {
                ...
                await this.updateAndBroadcastUserLock(data.roomId, data.userId, data.service, data.direction);
                return { status: true, msg: 'User lock settings updated' };
            }
        } catch (error) {
            this.logger.error(`Failed to update user lock settings: ${error.message}`);
    ...
    private async handleUpdateAllUsersLockSettings(data: {
        ...
        try {
            ...
            this.logger.error(`Failed to update default room lock settings: ${error.message}`);
        }
        ...
        for (const userId of userIds) {
            ...
            try {
                await this.updateAndBroadcastUserLock(data.roomId, userId, data.service, data.direction);
            } catch (error) {
                this.logger.error(`Failed to update user ${userId} lock settings during all-user change: ${error.message}`);
            }
        }
    }
    ...
    private async updateDefaultRoomLockSettings(data: {
        ...
        this.assignNewLockSetting(data.service, data.direction, metadata.defaultLockSettings);

        // Update the room metadata and broadcast the change
        await this.natsRoomEvents.updateAndBroadcastRoomMetadata(data.roomId, metadata);
    }
    ...
    async handleMuteUnMuteTrack(data: {
        ...
        this.logger.log(`Request to mute/unmute track: room=${data.roomId}, user=${data.userId}, muted=${data.muted}`);
    ...
    private async muteUnmuteAllMic(data: {
        ...
        const participants = await this.livekitService.loadParticipants(data.roomId);
            ...
            for (const track of participant.tracks || []) {
                if (track.source === TrackSource.MICROPHONE) {  // ✅ Using LiveKit enum
                    try {
                        ...
                        this.logger.log(`${data.muted ? 'Muted' : 'Unmuted'} microphone for ${participant.identity}`);
                    } catch (error) {
                        this.logger.error(
        ...
        this.logger.log('Successfully processed mute/unmute all microphones request');
    }
    ...
    async handleSwitchPresenter(data: {
        ...
        this.logger.log(`Switching presenter in room: ${data.roomId}, task: ${data.task}`);
    ...
    private async createNewPresenter(req: any): Promise<void> {
        ...
        this.logger.log(`Request to check for new presenter: room=${roomId}, user=${userId}`);

        // Find current presenter
        const presenter = await this.findCurrentPresenter(roomId);
        if (presenter) {
            this.logger.log(`Session already has an online presenter (${presenter}), skipping`);
            return;
        }
    ...
    private async switchPresenter(req: {
        ...
        this.logger.log(`Request to switch presenter: room=${req.roomId}, userId=${req.userId}, task=${req.task}`);
        ...
        if (req.task === SwitchPresenterTask.PROMOTE) {
            ...
            try {
                ...
                this.logger.warn(`Could not find current presenter to demote: ${error.message}`);
            }
        ...
        try {
            ...
            this.logger.error(`Failed to promote new presenter ${newPresenterId}: ${error.message}`);
            throw error;
        }
    ...
    private async findCurrentPresenter(roomId: string): Promise<string | null> {
        ...
        for (const userId of userIds) {
            const isPresenter = await this.natsUserInfo.isUserPresenter(roomId, userId);
            if (isPresenter) {
                return userId;
            }
        }
    ...
    private async updatePresenterStatus(roomId: string, userId: string, isPresenter: boolean): Promise<void> {
        ...
        try {
            await this.natsUser.updateUserKeyValue(roomId, userId, 'is_presenter', String(isPresenter));
        } catch (error) {
            throw new Error(`Failed to update user key-value for ${userId}: ${error.message}`);
        }
        ...
        try {
            ...
            throw new Error(`Failed to get user metadata for ${userId}: ${error.message}`);
        }
