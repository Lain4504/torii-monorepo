...
export class AnalyticsService {
    ...
    private async handleUserTypeEvents(d: AnalyticsDataMsg): Promise<void> {
        ...
        const key = this.redisAnalytics.getAnalyticsUserKeyPrefix(d.roomId, d.userId);
        await this.insertEventData(d, key);
    }
     ...
     */
    private async insertEventData(d: AnalyticsDataMsg, key: string): Promise<void> {
        ...
        if (d.eventValueInteger === undefined && d.eventValueString === undefined) {
            ...
            // INCRBY
            await this.redisAnalytics.incrementAnalyticsVal(eventKey, Number(d.eventValueInteger));
        } else if (d.eventValueString !== undefined) {
            // SET
            await this.redisAnalytics.addAnalyticsStringType(eventKey, d.eventValueString);
        }
    }
    ...
    private async handleFirstTimeUserJoined(d: AnalyticsDataMsg, key: string): Promise<void> {
        ...
        if (d.extraData) {
            try {
                ...
                this.logger.warn(`Failed to unmarshal user metadata for analytics: ${error.message}`);
            }
        }
        ...
        await this.redisAnalytics.addAnalyticsUser(d.roomId, d.userId, userInfoJson);

        // Also record the join event
        const userEventKey = this.redisAnalytics.getAnalyticsUserKeyPrefix(d.roomId, d.userId);
    ...
    async prepareToExportAnalytics(roomId: string, sid: string, metaJson: string): Promise<void> {
        ...
        if (!metaJson || !sid) {
            this.logger.warn(`Metadata or SID is empty for room ${roomId}, skipping analytics export`);
            return;
        }
        ...
        try {
            ...
            if (natsInfo && natsInfo.roomSid !== sid) {
                this.logger.log(`Room ${roomId} was likely re-created, skipping analytics export for sid ${sid}`);
                return;
            }
            ...
            if (!room) {
                this.logger.warn(`Could not find ended room ${sid} in DB, skipping analytics export`);
                return;
            }
     ...
     */
    private async exportAnalyticsToJson(room: any, metadata: RoomMetadata): Promise<Uint8Array | null> {
        const roomInfo = create(AnalyticsRoomInfoSchema, {
            roomId: room.roomId,
            roomTitle: room.roomTitle,
            roomTotalUsers: '0',
            roomCreation: Math.floor(room.created.getTime() / 1000).toString(),
            roomEnded: room.ended ? Math.floor(room.ended.getTime() / 1000).toString() : Math.floor(Date.now() / 1000).toString(),
            roomDuration: '0',
            enabledE2ee: metadata.roomFeatures?.endToEndEncryptionFeatures?.isEnabled ?? false,
            events: [],
        });
        ...
        const roomKeyPrefix = this.redisAnalytics.getAnalyticsRoomKeyPrefix(room.roomId);
        ...
        for (const [userId, userInfoJson] of Object.entries(users)) {
            const redisUserInfo = fromJsonString(AnalyticsRedisUserInfoSchema, userInfoJson);
            ...
            const userKeyPrefix = this.redisAnalytics.getAnalyticsUserKeyPrefix(room.roomId, userId);
            for (const key of allKeys) {
        ...
        if (metadata.roomFeatures?.enableAnalytics) {
            ...
            resultData = new TextEncoder().encode(toJsonString(AnalyticsResultSchema, result));
        }
     ...
     */
    private async processEventKey(key: string, prefix: string): Promise<AnalyticsEventData | null> {
        const eventNameWithPrefix = key.substring(prefix.length + 1);
        let eventName = '';

        if (eventNameWithPrefix.startsWith('ANALYTICS_EVENT_ROOM_')) {
            eventName = eventNameWithPrefix.replace('ANALYTICS_EVENT_ROOM_', '').toLowerCase();
        } else if (eventNameWithPrefix.startsWith('ANALYTICS_EVENT_USER_')) {
            eventName = eventNameWithPrefix.replace('ANALYTICS_EVENT_USER_', '').toLowerCase();
        } else {
            return null;
        }
        ...
        if (type === 'hash') {
            const hashVals = await this.redisAnalytics.getAnalyticsAllHashTypeVals(key);
            for (const [t, v] of Object.entries(hashVals)) {
                eventData.values.push(create(AnalyticsEventValueSchema, {
                    time: t,
                    value: v,
                }));
            }
     ...
     */
    private async createAnalyticsArtifact(roomTableId: number, roomId: string, roomSid: string, jsonData: Uint8Array): Promise<any> {
        const fileName = `${roomId}-${Date.now()}-analytics.json`;
        const { relativePath, absolutePath } = await this.artifactsService.buildPath(fileName, roomId, RoomArtifactType.MEETING_ANALYTICS);
