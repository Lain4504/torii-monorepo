/**
 * Polls Controller - Microservice Controller
 *
...
export class PollsController {
    ...
    async handleCountPollTotalResponses(@Payload() data: { roomId: string; pollId: string }) {
        try {
            const responses = await this.pollsService.getPollTotalResponses(data.roomId, data.pollId);
            return {
                status: true,
                msg: 'success',
                pollId: data.pollId,
    ...
    async handleUserSelectedOption(@Payload() data: { roomId: string; pollId: string; userId: string }) {
        try {
            const voted = await this.pollsService.userSelectedOption(data.roomId, data.pollId, data.userId);
            return {
                status: true,
                msg: 'success',
                pollId: data.pollId,
    ...
    async handleGetPollResponsesDetails(@Payload() data: { roomId: string; pollId: string }) {
        try {
            const responses = await this.pollsService.getPollResponsesDetails(data.roomId, data.pollId);
            return {
                status: true,
                msg: 'success',
                pollId: data.pollId,
            ...
            this.logger.error(`Error getting poll responses details: ${error.message}`);
            return {
                status: false,
                msg: error.message,
            };
    ...
    async handleGetResponsesResult(@Payload() data: { roomId: string; pollId: string }) {
        try {
            const result = await this.pollsService.getResponsesResult(data.roomId, data.pollId);
            return {
                status: true,
                msg: 'success',
                pollId: data.pollId,
            ...
            this.logger.error(`Error getting poll responses result: ${error.message}`);
            return {
                status: false,
                msg: error.message,
            };
