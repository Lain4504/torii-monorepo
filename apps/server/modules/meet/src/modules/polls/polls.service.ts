...
export class PollsService {
    ...
    async userSubmitResponse(r: SubmitPollResponseReq): Promise<void> {
        const log = this.logger;
        log.log(`request to submit poll response for room: ${r.roomId}, user: ${r.userId}, poll: ${r.pollId}`);
    ...
    async closePoll(r: ClosePollReq): Promise<void> {
        const log = this.logger;
        log.log(`request to close poll for room: ${r.roomId}, user: ${r.userId}, poll: ${r.pollId}`);
    ...
    async userSelectedOption(roomId: string, pollId: string, userId: string): Promise<string> {
        const allRespondents = await this.redisPollService.getPollAllRespondents(roomId, pollId);
    ...
    async getPollResponsesDetails(roomId: string, pollId: string): Promise<Record<string, string>> {
        let result = await this.redisPollService.getPollCountersByPollId(roomId, pollId);
    ...
    async getResponsesResult(roomId: string, pollId: string): Promise<PollResponsesResult> {
        const log = this.logger;
        log.log(`request to get response result for room: ${roomId}, poll: ${pollId}`);

        const pStr = await this.redisPollService.getPollInfoByPollId(roomId, pollId);
        if (!pStr) {
        ...
        const counters = await this.redisPollService.getPollCountersByPollId(roomId, pollId);
        if (!counters) {
        ...
        for (const o of p.options) {
            ...
            opts.push(create(PollResponsesResultOptionsSchema, {
