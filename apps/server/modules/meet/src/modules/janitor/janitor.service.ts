...
export class JanitorService implements OnApplicationBootstrap, OnApplicationShutdown {
    ...
    private async runLeaderElectionLoop() {
        while (!this.isShutdown) {
            try {
                const { acquired, lockValue } = await this.redisLock.acquireJanitorLeaderLock(this.leaderLockTTL);
                if (acquired) {
                    this.logger.log(`Acquired janitor leader lock (val: ${lockValue}). Starting tasks.`);
                ...
                this.logger.error(`Failed to check for janitor leader lock: ${error.message}`);
                await new Promise(resolve => setTimeout(resolve, this.leaderRenewal));
            }
        }
    }
    ...
    private startRenewalTicker() {
        ...
        this.renewalTicker = setInterval(async () => {
            ...
            try {
                // Renew lock
                const renewed = await this.redisLock.renewJanitorLeadershipLock(this.leaderLockVal, this.leaderLockTTL);
                if (!renewed) {
                    this.logger.warn('Failed to renew janitor leader lock, stopping tasks.');
                    this.stopJanitorTasks();
                    this.leaderLockVal = '';
