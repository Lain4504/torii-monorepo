...
export class RecordingInfoService {
    ...
    async fetchRecordings(req: FetchRecordingsReq): Promise<FetchRecordingsResult> {
        ...
        const recordingsList: RecordingInfo[] = artifacts.map(a => {
            ...
            const size = meta.fileInfo?.fileSize ? parseFloat(meta.fileInfo.fileSize as string) : 0;
            ...
            return create(RecordingInfoSchema, {
                ...
                roomCreationTime: a.roomInfo ? Math.floor(a.roomInfo.created.getTime() / 1000).toString() : '0',
            });
        });
    ...
    async fetchRecording(recordId: string): Promise<RecordingInfo> {
        ...
        return create(RecordingInfoSchema, {
            ...
            roomCreationTime: artifact.roomInfo ? Math.floor(artifact.roomInfo.created.getTime() / 1000).toString() : '0',
        });
    }
