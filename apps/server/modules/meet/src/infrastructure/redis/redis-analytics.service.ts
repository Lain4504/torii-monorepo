...
export class RedisAnalyticsService {
    ...
    async analyticsScanKeys(pattern: string): Promise<string[]> {
        ...
        try {
            do {
                const [newCursor, scannedKeys] = await this.redis.scan(cursor, 'MATCH', pattern, 'COUNT', 100);
                cursor = newCursor;
                keys = keys.concat(scannedKeys);
            } while (cursor !== '0');
            return keys;
        } catch (error) {
            this.logger.error(`AnalyticsScanKeys failed for pattern ${pattern}: ${error.message}`);
            throw error;
        }
    }
    ...
    async analyticsGetAllUsers(roomId: string): Promise<Record<string, string>> {
        ...
        try {
            ...
            this.logger.error(`AnalyticsGetAllUsers failed for room ${roomId}: ${error.message}`);
            throw error;
        }
    }
     ...
     */
    async getAnalyticsAllHashTypeVals(key: string): Promise<Record<string, string>> {
        try {
            return await this.redis.hgetall(key);
        } catch (error) {
            this.logger.error(`GetAnalyticsAllHashTypeVals failed for key ${key}: ${error.message}`);
            throw error;
        }
    }
    ...
    async getAnalyticsStringTypeVal(key: string): Promise<string | null> {
        try {
            ...
            this.logger.error(`GetAnalyticsStringTypeVal failed for key ${key}: ${error.message}`);
            throw error;
        }
    }
    ...
    async analyticsGetKeyType(key: string): Promise<string> {
        try {
            ...
            this.logger.error(`AnalyticsGetKeyType failed for key ${key}: ${error.message}`);
            throw error;
        }
    }
}
