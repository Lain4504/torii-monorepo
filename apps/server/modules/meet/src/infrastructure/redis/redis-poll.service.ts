...
export class RedisPollService {
    ...
    async addPollResponse(r: SubmitPollResponseReq): Promise<void> {
        ...
        try {
            ...
            multi.hincrby(respondentsKey, `${r.selectedOption}${POLL_COUNT_SUFFIX}`, 1);
    ...
    async getPollAllRespondents(roomId: string, pollId: string): Promise<string[] | null> {
        try {
    ...
    async getPollCountersByPollId(roomId: string, pollId: string): Promise<Record<string, string> | null> {
        try {
    ...
    async getPollInfoByPollId(roomId: string, pollId: string): Promise<string | null> {
        try {
