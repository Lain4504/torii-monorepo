...
export class InsightsHandler {
    ...
    async handleChatTranslationConfigure(@Payload() data: any) {
        return await this.insightsService.chatTranslationConfigure(data.roomId, data);
    }
    ...
    async handleAIMeetingSummarizationConfig(@Payload() data: any) {
        try {
            return await this.insightsService.meetingSummarizationConfigure(data.roomId, data);
        } catch (error) {
