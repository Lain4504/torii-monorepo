...
export class NatsCacheService implements OnModuleDestroy {
    ...
    // Room files cache
    private readonly roomFilesStore = new Map<string, Map<string, RoomUploadedFileMetadata>>();
    ...
    addRoomWatcher(kv: any, bucket: string, roomId: string): void {
        ...
        if (!this.roomUsersInfoStore.has(roomId)) {
            this.roomUsersInfoStore.set(roomId, new Map<string, CachedUserInfoEntry>());
        }

        this.logger.log(`NATS unified KV watcher for room started: ${roomId}, bucket: ${bucket}`);
     ...
     */
    private updateRoomFilesCache(entry: any, roomId: string, fileId: string): void {
        const val = entry.value ? new TextDecoder().decode(entry.value) : '';
        if (!this.roomFilesStore.has(roomId)) {
            this.roomFilesStore.set(roomId, new Map<string, RoomUploadedFileMetadata>());
        }
     ...
     */
    getCachedRoomFile(roomId: string, fileId: string): RoomUploadedFileMetadata | null {
     ...
     */
    getAllCachedRoomFiles(roomId: string): Record<string, RoomUploadedFileMetadata> | null {
        const roomStore = this.roomFilesStore.get(roomId);
    ...
    private updateRecorderCache(entry: any): void {
        ...
        if (!recorder) {
            recorder = { recorderId, maxLimit: 0n, currentProgress: 0n, lastPing: 0n };
     ...
     */
    private updateRoomInfoCache(field: string, val: string, roomId: string): void {
     ...
     */
    private updateUserInfoAndStatusCache(key: string, val: string, revision: number, roomId: string): void {
        const parts = NatsService.parseUserKey(key);
        if (!parts) return;
        ...
        if (!userEntry) {
            userEntry = { userInfo: {}, status: '', lastPingAt: 0, isBlacklisted: false };
        ...
        switch (field) {
            ...
            case 'disconnected_at':
                ...
                break;
            case 'last_ping_at':
                if (userEntry) userEntry.lastPingAt = parseInt(this.convertTextToUint64(val), 10);
                break;
            case 'is_blacklisted':
     ...
     */
    getCachedRoomUserStatus(roomId: string, userId: string): CachedRoomUserStatusEntry | null {
     ...
     */
    isUserBlacklistedFromCache(roomId: string, userId: string): { isBlacklisted: boolean; found: boolean } {
        const roomStore = this.roomUsersInfoStore.get(roomId);
        if (roomStore) {
            const entry = roomStore.get(userId);
