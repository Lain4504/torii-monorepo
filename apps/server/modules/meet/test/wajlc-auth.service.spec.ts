...
import { WajlcAuthService, WajlcTokenClaims } from '../src/modules/auth/wajlc-auth.service';
import { NatsUserInfoService } from '../src/interfaces/nats/nats-user-info.service';
...
jest.mock('@server/shared/utils/access_token', () => ({
    generateWajlcJWTAccessToken: (...args) => mockGenerateWajlcJWTAccessToken(...args),
}));
...
describe('WajlcAuthService', () => {
    ...
    describe('generateWajlcJoinToken', () => {
        ...
        it('should throw error if generation fails', () => {
            ...
            expect(() => service.generateWajlcJoinToken(claims)).toThrow('Generation failed');
        });
    });
    ...
    describe('verifyWajlcAccessToken', () => {
        ...
        it('should throw error if verification fails', () => {
            ...
            expect(() => service.verifyWajlcAccessToken(token)).toThrow('Invalid token claims');
        });
    });
    ...
    describe('renewWajlcToken', () => {
        it('should renew token if user is in room', async () => {
            ...
            jest.spyOn(service, 'verifyWajlcAccessToken').mockReturnValue(claims as any);
            ...
            expect(mockNatsUserInfoService.getRoomUserStatus).toHaveBeenCalledWith('r1', 'u1');
        ...
        it('should throw error if user is not in room', async () => {
            ...
            await expect(service.renewWajlcToken(oldToken)).rejects.toThrow('user not found');
        });
        ...
        it('should throw error if getRoomUserStatus throws', async () => {
            ...
            jest.spyOn(service, 'verifyWajlcAccessToken').mockReturnValue(claims as any);
            mockNatsUserInfoService.getRoomUserStatus.mockRejectedValue(new Error('NATS error'));

            await expect(service.renewWajlcToken(oldToken)).rejects.toThrow('NATS error');
        });
    });
