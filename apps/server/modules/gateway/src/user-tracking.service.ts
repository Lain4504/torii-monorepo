...
export class UserTrackingService {
    ...
    async onAfterUserJoined(roomId: string, userId: string) {
        ...
        try {
            ...
            const currentStatus = await this.natsService.getRoomUserStatus(roomId, userId);
    ...
    async onAfterUserDisconnected(roomId: string, userId: string) {
        ...
        try {
            ...
            setTimeout(() => this.handleDelayedOffline(roomId, userId, userInfo), 5000);
        } catch (error) {
            this.logger.error(`Error in onAfterUserDisconnected: ${error.message}`);
        }
    }

    private async handleDelayedOffline(roomId: string, userId: string, userInfo: any) {
        try {
    ...
    private async broadcastUserJoined(roomId: string, userId: string, userInfo: any) {
        try {
            this.logger.log(`Broadcasting USER_JOINED for ${userId} in room ${roomId}`);
    ...
    private async broadcastUserDisconnected(roomId: string, userId: string, userInfo: any) {
        try {
            ...
            this.logger.error(`Error broadcasting USER_DISCONNECTED: ${error.message}`);
        }
    }

    private async broadcastUserOffline(roomId: string, userId: string, userInfo: any) {
        try {
