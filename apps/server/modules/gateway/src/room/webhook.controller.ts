import { Controller, Post, Headers, Body, Inject, Logger, UnauthorizedException } from '@nestjs/common';
...
export class WebhookController {
    ...
    async handleWebhook(@Headers('authorization') authHeader: string, @Body() body: any) {
        ...
        try {
            ...
            const event = await this.liveKitService.getWebhookReceiver().receive(body, authHeader);

            this.logger.log(`Received Webhook Event: ${event.event} for room ${event.room?.name}`);
