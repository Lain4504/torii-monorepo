...
export class NatsConnectionListener implements OnModuleInit {
  ...
  private async subscribeToUsersConnEvents() {
    ...
    while (retries < 10) {
      ...
      if (nc && !nc.isClosed()) {
        try {
          ...
          this.logger.error(
            `Failed to subscribe to connection events: ${e.message}`,
          );
        }
      }
    ...
    this.logger.error(
      'Could not subscribe to NATS connection events after retries',
    );
  }
  ...
  private async handleUserConnectionEvent(
    ...
    try {
      ...
      this.logger.debug(
        `Connection event - type: ${eventData.type}, isConnect: ${isConnect}`,
      );

      // DEBUG: Log full event structure
      this.logger.debug(
        `Full event data: ${JSON.stringify(eventData, null, 2)}`,
      );
      ...
      if (!roomId || !userId) {
        this.logger.warn(
          `Missing roomId or userId in token claims. roomId=${roomId}, userId=${userId}`,
        );
        this.logger.debug(
          `Available claims: ${Object.keys(decoded).join(', ')}`,
        );
        return;
      }
      ...
      this.logger.log(
        `User ${isConnect ? 'connected' : 'disconnected'}: ${userId} in room ${roomId}`,
      );
