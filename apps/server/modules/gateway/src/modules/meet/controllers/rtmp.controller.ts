import { Controller, Post, Body, Inject, Logger, HttpCode, HttpStatus, Res } from '@nestjs/common';
    ...
    async handleRtmpWebhook(@Body() body: any, @Res() res: Response) {
        ...
        try {
            ...
            if (buffer) {
                ...
                try {
                    ...
                    if (task === RecordingTasks.START_RTMP) {
                        isActive = true;
                    } else if (task === RecordingTasks.STOP_RTMP || task === RecordingTasks.END_RTMP) {
                        isActive = false;
                    } else {
                ...
                if (roomId && action) {
                    ...
                    // Unknown format
                    this.logger.warn(`Received unknown body format: ${JSON.stringify(body).slice(0, 100)}...`);
            ...
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'room.updateRTMP' }, {
                    roomId: roomId,
                    isActive: isActive,
                    nodeId: nodeId
                })
            ...
            if (result && result.success) {
                ...
                this.logger.warn(`Failed to update RTMP status for room ${roomId}: ${result?.message}`);
                if (isActive) {
