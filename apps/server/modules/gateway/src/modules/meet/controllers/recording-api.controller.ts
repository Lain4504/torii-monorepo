    ...
    async handleRecorderTasks(
        ...
        try {
            ...
            switch (request.task) {
                case RecordingTasks.START_RECORDING:
                    if (rInfo.isRecording) {
                        sendCommonProtobufResponse(res, false, 'notifications.recording-already-running');
                        return;
                    }
                    break;
                case RecordingTasks.STOP_RECORDING:
                    if (!rInfo.isRecording) {
                        sendCommonProtobufResponse(res, false, 'notifications.recording-not-running');
                        return;
                    }
                    break;
                case RecordingTasks.START_RTMP:
                    ...
                    if (rInfo.isActiveRtmp) {
                        sendCommonProtobufResponse(res, false, 'notifications.rtmp-already-running');
                        return;
                    }
                    break;
                case RecordingTasks.STOP_RTMP:
                    if (!rInfo.isActiveRtmp) {
                        sendCommonProtobufResponse(res, false, 'notifications.rtmp-not-running');
                        return;
                    }
                    break;
            }
            ...
            sendCommonProtobufResponse(
                res,
                false,
                error instanceof Error ? error.message : 'Error processing recorder task',
            );
        }
    }
}
