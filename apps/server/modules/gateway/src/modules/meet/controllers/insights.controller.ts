...
export class InsightsController {
    ...
    @Post('supportedLangs')
    async handleGetSupportedLangs(@Body() bodyBuffer: Buffer, @Res() res: Response) {
        try {
            const request = fromBinary(InsightsGetSupportedLanguagesReqSchema, bodyBuffer);
    ...
    async handleTranscriptionConfigure(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        if (!(req as any).isAdmin) return sendCommonProtobufResponse(res, false, 'only admin can perform this task');

        try {
            const request = fromBinary(InsightsTranscriptionConfigReqSchema, bodyBuffer);
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'insights.transcription.configure' }, { ...request, roomId: (req as any).roomId })
            );
    ...
    async handleEndTranscription(@Req() req: Request, @Res() res: Response) {
        ...
        try {
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'insights.transcription.end' }, { roomId: (req as any).roomId })
            );
            sendCommonProtobufResponse(res, result.status, result.msg);
        } catch (error) {
            sendCommonProtobufResponse(res, false, error.message);
    ...
    async handleTranscriptionUserSession(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        try {
            const request = fromBinary(InsightsTranscriptionUserSessionReqSchema, bodyBuffer);
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'insights.transcription.userSession' }, {
                    ...request,
    ...
    async handleChatTranslationConfigure(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        ...
        try {
            const request = fromBinary(InsightsChatTranslationConfigReqSchema, bodyBuffer);
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'insights.translation.chat.configure' }, { ...request, roomId: (req as any).roomId })
            );
            sendCommonProtobufResponse(res, result.status, result.msg);
        } catch (error) {
            sendCommonProtobufResponse(res, false, error.message);
    ...
    async handleExecuteChatTranslation(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        try {
            const request = fromBinary(InsightsTranslateTextReqSchema, bodyBuffer);
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'insights.translation.chat.execute' }, {
                    ...request,
                    roomId: (req as any).roomId,
                    userId: (req as any).requestedUserId
                })
    ...
    async handleAITextChatConfigure(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        if (!(req as any).isAdmin) return sendCommonProtobufResponse(res, false, 'only admin can perform this task');

        try {
            const request = fromBinary(InsightsAITextChatConfigReqSchema, bodyBuffer);
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'insights.ai.textChat.configure' }, { ...request, roomId: (req as any).roomId })
            );
            sendCommonProtobufResponse(res, result.status, result.msg);
        } catch (error) {
            sendCommonProtobufResponse(res, false, error.message);
    ...
    async handleExecuteAITextChat(@Req() req: Request, @Body() bodyBuffer: Buffer, @Res() res: Response) {
        try {
            const request = fromBinary(InsightsAITextChatContentSchema, bodyBuffer);
            const result = await firstValueFrom(
                this.natsClient.send({ cmd: 'insights.ai.textChat.execute' }, {
                    ...request,
                    roomId: (req as any).roomId,
                    userId: (req as any).requestedUserId
                })
