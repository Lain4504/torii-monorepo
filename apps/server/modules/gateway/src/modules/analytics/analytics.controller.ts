...
export class AnalyticsController {
    ...
    async getPlatformOverview() {
        try {
            ...
            const [identityStats, learningStats, billingStats, ticketStats] = await Promise.all([
                firstValueFrom(this.natsClient.send({ cmd: 'identity.analytics.overview' }, {})).catch(() => ({ totalUsers: 0, activeToday: 0 })),
                firstValueFrom(this.natsClient.send({ cmd: 'learning.analytics.overview' }, {})).catch(() => ({ totalCourses: 0, totalEnrollments: 0, popularCourses: [], pendingApprovals: 0, activeRooms: 0 })),
                firstValueFrom(this.natsClient.send({ cmd: 'billing.analytics.overview' }, {})).catch(() => ({ totalRevenue: 0, recentSales: [], growthData: [], revenueByLevel: [] })),
                firstValueFrom(this.natsClient.send({ cmd: 'communication.analytics.tickets' }, {})).catch(() => ({ pendingCount: 0, refundCount: 0 }))
            ]);
