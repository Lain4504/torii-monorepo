...
describe('AuditLogService', () => {
    ...
    describe('log', () => {
        ...
        it('should catch and log errors during creation', async () => {
            ...
            expect(loggerSpy).toHaveBeenCalledWith('Failed to create audit log:', error);
            expect(consoleSpy).toHaveBeenCalledWith('❌ Audit log creation failed:', error);
            ...
            const mockLogs = [
                {
                    ...
                    entityId: 'course-1',
                    createdAt: new Date(),
                    user: { id: 'user-1', email: 'test@example.com', displayName: 'Test User', role: 'USER' },
                },
            ];
            const total = 1;
    ...
    describe('getEntityActivity', () => {
        it('should return entity activity', async () => {
            ...
            expect(repository.findByEntity).toHaveBeenCalledWith(entity, entityId, 20);
            expect(result).toEqual(mockActivity);
        });
    });
});
