...
import { AppConfigService, EncryptionService, PrismaService } from '@server/shared';
...
describe('TwoFactorAuthService', () => {
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: TWO_FACTOR_AUTH_REPOSITORY_TOKEN, useValue: mockTwoFactorAuthRepository },
    ...
    describe('generateTotpSecret', () => {
        ...
        it('should throw BadRequestException if 2FA already enabled', async () => {
            twoFactorAuthRepository.findByUserId.mockResolvedValue({ isEnabled: true });
            await expect(service.generateTotpSecret(mockUser.id)).rejects.toThrow(BadRequestException);
        });

        it('should throw NotFoundException if user not found', async () => {
            twoFactorAuthRepository.findByUserId.mockResolvedValue(null);
            prisma.user.findUnique.mockResolvedValue(null);
            await expect(service.generateTotpSecret(mockUser.id)).rejects.toThrow(NotFoundException);
        });
    });
    ...
    describe('enableTotp', () => {
        ...
        it('should throw BadRequestException for invalid code', async () => {
            ...
            await expect(service.enableTotp(mockUser.id, 'secret', 'wrong-code')).rejects.toThrow(BadRequestException);
        });
    });
    ...
    describe('verifyTotp', () => {
        ...
        it('should throw UnauthorizedException if account locked', async () => {
            const lockedAuth = { ...mockTwoFactorAuth, lockedUntil: new Date(Date.now() + 30000) };
            ...
            await expect(service.verifyTotp(mockUser.id, '123456')).rejects.toThrow(UnauthorizedException);
        });
    });
    ...
    describe('disable2FA', () => {
        it('should disable 2FA', async () => {
            ...
            expect(twoFactorAuthRepository.update).toHaveBeenCalledWith(mockUser.id, expect.objectContaining({ isEnabled: false }));
        });
    });
