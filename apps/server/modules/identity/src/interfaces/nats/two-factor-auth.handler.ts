...
export class TwoFactorAuthHandler {
    ...
    async enableTotp(@Payload() data: { userId: string; secret: string; code: string }) {
        return this.twoFactorAuthService.enableTotp(data.userId, data.secret, data.code);
    }
    ...
    async regenerateBackupCodes(@Payload() data: { userId: string }) {
        const backupCodes = await this.twoFactorAuthService.regenerateBackupCodes(data.userId);
        return { backupCodes };
    }
