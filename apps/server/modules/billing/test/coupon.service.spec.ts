...
describe('CouponService', () => {
    ...
    describe('validateCoupon', () => {
        ...
        it('should return valid if coupon exists and and meets all conditions', async () => {
            ...
            const result = await service.validateCoupon('TEST-CODE', userId, orderAmount);
        ...
        it('should return invalid if coupon not found', async () => {
            ...
            const result = await service.validateCoupon('INVALID', userId, orderAmount);
        ...
        it('should return invalid if coupon is not active', async () => {
            couponRepository.findByCode.mockResolvedValue({ ...mockCoupon, status: CouponStatus.INACTIVE } as any);
    ...
    describe('redeemCoupon', () => {
        ...
        it('should redeem coupon successfully', async () => {
            ...
            expect(couponRepository.incrementUsage).toHaveBeenCalledWith(mockCoupon.id);
        });

        it('should throw ConflictException if lock not acquired', async () => {
            redisClient.set.mockResolvedValue(null);

            await expect(service.redeemCoupon(code, userId, orderAmount)).rejects.toThrow(ConflictException);
        });
    ...
    describe('getCouponsForUser', () => {
        ...
        it('should return available coupons for user', async () => {
            const mockCoupons = [
                ...
                { id: 'c2', userId: null, status: CouponStatus.ACTIVE, userUsageLimit: 1 }, // Public
                { id: 'c3', userId: null, status: CouponStatus.ACTIVE, usageLimit: 10, usageCount: 10 }, // Global limit reached
            ];
        ...
        it('should filter out coupons where user reached limit', async () => {
            const mockCoupons = [
                { id: 'c2', userId: null, status: CouponStatus.ACTIVE, userUsageLimit: 1 },
            ];
