...
export class NatsAuthService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    if (!accountSeed || !xKeySeed) {
      this.logger.warn(
        'NATS Authority Keys missing. Auth Callout service will not start.',
      );
      return;
    }
    ...
    try {
      ...
      if (!publicKey.startsWith('A')) {
        throw new Error(
          `Invalid Account Seed: public key must start with 'A', got '${publicKey[0]}'`,
        );
      }
    ...
    try {
      ...
      if (xKey) {
        if (!this.xKeyKp) {
          msg.respond(null, {
            headers: undefined,
            error: { code: 500, description: 'xKey not supported' },
          });
          return;
        }
        try {
          ...
          msg.respond(null, {
            error: { code: 500, description: 'Decryption error' },
          });
          return;
        }
      }
      ...
      // Generate User JWT
      const userJwt = await this.generateUserJwt(
        userNkey,
        pnmClaims,
        authToken,
      );
      ...
      const userNkey = e.decoded?.sub || '';
      const serverId = e.decoded?.iss || '';
  ...
  async generateUserJwt(
    ...
    const consumers = [
      ...
      {
        stream: roomId,
        durable: `sysPublic:${userId}`,
        filter: `${roomId}:sysPublic.>`,
      },
      {
        stream: roomId,
        durable: `sysPrivate:${userId}`,
        filter: `${roomId}:sysPrivate.${userId}.>`,
      },
      {
        stream: roomId,
        durable: `whiteboard:${userId}`,
        filter: `${roomId}:whiteboard.>`,
      },
      {
        stream: roomId,
        durable: `dataChannel:${userId}`,
        filter: `${roomId}:dataChannel.>`,
      },
    ];
