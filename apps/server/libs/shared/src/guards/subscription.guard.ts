...
export class SubscriptionGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (!requester || !requester.sub) {
            this.logger.warn('[SubscriptionGuard] No requester information found in request');
        ...
        if (!activeSubscription) {
            this.logger.warn(`[SubscriptionGuard] User ${userId} does not have an active subscription`);
        ...
        const subscriptionOffering = (activeSubscription as any).class.offeringLinks
            .find((oc: any) => oc.offering.type === 'SUBSCRIPTION')?.offering;
