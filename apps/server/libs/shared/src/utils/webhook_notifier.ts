...
import { CommonNotifyEvent, CommonNotifyEventSchema } from '@workspace/protocol';
...
export class WebhookNotifier {
    ...
    addInNotifyQueue(
        ...
        for (const url of urls) {
            this.worker.submit(async () => {
                ...
                try {
                    const statusCode = await this.sendWebhookRequest(event, apiKey, apiSecret, url);
                    this.logger.log({
                        ...logFields,
    ...
    private async sendWebhookRequest(
        ...
        const encoded = JSON.stringify(toJson(CommonNotifyEventSchema, event, {
