...
export function prepareDefaultRoomFeatures(r: CreateRoomReq): void {
    ...
    if (!rf.externalMediaPlayerFeatures) {
        ...
        // backward compatibility
        if (rf.externalMediaPlayerFeatures.allowedExternalMediaPlayer !== undefined) {
 ...
 * @param r - CreateRoomReq to configure
 */
export function setDefaultRoomSettings(s: RoomDefaultSettings | null, r: CreateRoomReq): void {
    if (!s) {
        return;
    }
    ...
    } else if (s.maxNumBreakoutRooms !== undefined && s.maxParticipants! > maxNum) {
        s.maxNumBreakoutRooms = maxNum;
    }
