 ...
 */
export function sendCommonProtobufResponse(res: Response, status: boolean, msg: string): void {
 ...
 */
export function sendProtobufResponse(res: Response, schema: any, message: any): void {
 ...
 * @param msg - Response message
 */
export function sendCommonProtoJsonResponse(res: Response, status: boolean, msg: string): void {
    const response = create(CommonResponseSchema, {
        status,
        msg,
    });
 ...
 * @param message - Protobuf message to send
 */
export function sendProtoJsonResponse(res: Response, schema: any, message: any): void {
 ...
 */
export function getFilesFromDir(path: string, ext: string, sortOrder: string): string[] {
    try {
        const entries = readdirSync(path, { withFileTypes: true });
        let files: string[] = [];

        for (const entry of entries) {
            if (entry.isFile() && extname(entry.name) === ext && checkIfAllowedFilePrefix(entry.name)) {
