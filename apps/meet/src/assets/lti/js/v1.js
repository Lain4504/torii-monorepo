...
const joinSession = async () => {
  ...
  if (res.status) {
    let url = window.WAJLC_SERVER_URL + '/?access_token=' + res.token;
...
const downloadRecording = async (e) => {
  ...
  if (res.status) {
    const url =
      window.WAJLC_SERVER_URL + '/download/recording/' + res.token;
    window.open(url, '_blank');
  } else {
    alert(res.msg);
  }
...
const callLtiApi = async (path, body) => {
  const url = window.WAJLC_SERVER_URL + '/lti/v1/api' + path;
  const output = {
    status: false,
    msg: '',
  };
