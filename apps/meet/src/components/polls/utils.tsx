...
const RespondentItem = ({ user }: { user: Respondents }) => {
  ...
  return (
    <p
      className="text-xs w-[156.5px] font-medium text-foreground flex items-center gap-1 px-[14px]"
      key={user.userId}
    >
      <span className="w-[18px] h-[18px] rounded-md bg-primary flex items-center justify-center text-primary-foreground text-[8px] font-medium">
        {initials}
      </span>
      {user.name}
    </p>
...
export const getFormatedRespondents = (respondents: Respondents[]) => {
  ...
  respondentChunks.forEach((users, i) => {
    elms.push(
      <div
        className="grid gap-2 content-start border-r border-border py-2 w-max last:border-none"
        key={`chunk-${i}`}
      >
        {users.map((user) => (
